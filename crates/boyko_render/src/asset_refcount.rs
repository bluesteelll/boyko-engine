...
/// [`validate_asset_refs`] found to have TRANSITIONED this frame — keyed by the
...
/// param — one slot read per transitioning row, and a row the query yields is
/// live for the whole body (SCH7). At apply, [`EcsMaster::enable`] /
/// [`EcsMaster::disable`]'s own `live_inland` resolve (null + generation, one
/// slot read) drops a dead entity (a despawn racing this frame — a silent
/// no-op) or an id a later spawn re-occupied under a bumped generation.
///
...
/// this command, toggled `F`'s bit instead of `E`'s (unification plan 02,
...
/// +16 : kind: AssetRefKind (1 B)
/// +17 : stale: bool (1 B)
/// +18 : pad (6 B)
/// +24 : end
/// ```
#[repr(C)]
struct SetStaleCommand {
    /// The row's entity, resolved from the matched archetype's entity-id
    /// column through [`Entities`] at gather time.
    entity: Entity,
...
impl Command for SetStaleCommand {
    fn apply(self, world: &mut EcsMaster) {
        // No resolve of our own: `enable` / `disable` read the slot once and
        // drop a dead or generation-stale handle there (H-06). The bit op
        // resolves the row via the live inland (never a captured enqueue-time
        // row), so a swap-remove that moved another entity is honored.
        match (self.kind, self.stale) {
            (AssetRefKind::Mesh, true) => world.enable::<RenderStale>(self.entity),
            (AssetRefKind::Mesh, false) => world.disable::<RenderStale>(self.entity),
            (AssetRefKind::Material, true) => world.enable::<MaterialStale>(self.entity),
            (AssetRefKind::Material, false) => world.disable::<MaterialStale>(self.entity),
        }
    }
}

// Layout pin (house style, the A9 discipline): one `Entity` + two 1-B tags,
// `#[repr(C)]` — 24 B, 8-aligned. A silent widening would change the
// per-command arena cost.
const _: () = assert!(size_of::<SetStaleCommand>() == 24);
const _: () = assert!(align_of::<SetStaleCommand>() == 8);

/// Enqueues one [`SetStaleCommand`] for the row at `id`, resolved to the live
/// handle through `entities` so the generation travels with the command (H-06).
/// A row the query yields is live for the whole body (SCH7), so the resolve
/// cannot miss; the handle it returns carries the CURRENT generation, which is
/// what lets apply tell this row from a later occupant of the same id.
#[inline]
fn push_stale(
    cmd: &mut Commands,
    entities: &Entities,
    ...
    if let Some(entity) = entity {
        cmd.add(SetStaleCommand { entity, kind, stale });
    }
}
...
pub fn validate_asset_refs(
    ...
    for (id, _) in q_mesh_orphan.iter_entities() {
        push_stale(&mut cmd, &entities, id, AssetRefKind::Mesh, true);
        cursor.orphan_rows_flagged += 1;
    }
    for (id, _) in q_mat_orphan.iter_entities() {
        push_stale(&mut cmd, &entities, id, AssetRefKind::Material, true);
        cursor.orphan_rows_flagged += 1;
    }
    ...
    for (id, (&MeshHandle(slot), &MeshRefGen(g))) in q_mesh_stale.iter_entities() {
        if g != GEN_UNSYNCED && !is_stale(&mesh_assets, slot, g) {
            push_stale(&mut cmd, &entities, id, AssetRefKind::Mesh, false);
        }
    }
    ...
    for (id, (&MaterialHandle(slot), &MaterialRefGen(g))) in q_mat_stale.iter_entities() {
        if g != GEN_UNSYNCED && !is_stale(&material_assets, u32::from(slot), g) {
            push_stale(&mut cmd, &entities, id, AssetRefKind::Material, false);
        }
    }
...
/// expressible. The FURTHER edges — validation before
/// `boyko_render::gather_mesh_draws` / `gather_shadow_casters` (they filter on
/// the [`RenderStale`] / [`MaterialStale`] bits it writes), registered by a
/// LATER, separate closure in the composing host
/// (`boyko_app::plugins::EnginePlugins::build`), and after `visibility_sync`'s
/// apply window — cannot be `SystemKey` edges (a key for a system that does not
/// exist yet at this plugin's build time is unobtainable, and `SystemKey` is
/// unnameable outside `boyko_ecs`; the documented `CsmPlugin`/`ShadowAtlasPlugin`/
/// `LightingPlugin` cross-plugin limitation). They are pinned BY NAME, twice
/// over, and the two pins are independent:
///
/// * Prereq (c): `validate_asset_refs` is the sole member of
...
///   regardless of add-order and of the composing host, and is provable in a
///   bare `App` through the builder's cycle detection
///   (`tests/asset_validate_schedule_edge.rs`) — a mutation that drops the
///   `.after_set` turns that test red, which an add-order pin could never offer.
/// * R4b: `validate_asset_refs` also joins [`VisibilitySet::Validate`], its own
///   phase between `VisibilitySet::Sync` and `VisibilitySet::Read`; the composing
///   host configures `VisibilitySet::Validate.after(VisibilitySet::Sync)` and
///   `VisibilitySet::Read.after(VisibilitySet::Validate)` (`EnginePlugins` does;
///   each is pinned by a `boyko_app/tests/host_orders_*.rs` cycle gate). This
///   orders EVERY `Read` member after the validation — the instance packs too,
///   which the consumer helpers do not cover — and the validation after the
///   visibility sync. Add-order is NOT a pin: the executor orders two systems
///   that have no path between them by its wave packing, which any unrelated
///   edge can reshuffle (measured on the mesh gather vs. `visibility_sync`).
///
/// A host composing this plugin by hand declares the two `VisibilitySet` edges;
/// the fence-gate proof in [`retire_deferred_frees`]' doc ("after `apply`, before
/// any gather") rests on the validate → gather order, which either pin supplies.
/// The two sets may share the member: `boyko-B9004` rejects a shared member only
/// between sets ordered AGAINST EACH OTHER, and nothing orders `AssetValidateSet`
/// relative to a `VisibilitySet` phase.
#[derive(Default)]
...
impl Plugin for AssetRefcountPlugin {
    fn build(&self, app: &mut App) {
        ...
        app.add_systems_cfg(|b| {
            ...
            // `VisibilitySet::Validate`, its own phase between `Sync` and `Read` (R4b), AND
            // `AssetValidateSet`, the consumer-side seam (prereq (c)) — see the plugin's doc
            // for why both, and why the shared member is legal. It writes the `RenderStale` /
            // `MaterialStale` bits the gathers filter on, so a gather that runs before this
            // system's apply window draws a stale row for one more frame. Membership only;
            // the composing host orders `Validate.after(Sync)` and `Read.after(Validate)`,
            // and the gather helpers chain `.after_set(AssetValidateSet)`. It cannot join
            // `Read` as well: two ordered sets that share a member are rejected
            // (`boyko-B9004`).
            b.add_system(validate_asset_refs)
                .after(apply)
                .in_set(VisibilitySet::Validate)
                .in_set(AssetValidateSet);
        });
    }
