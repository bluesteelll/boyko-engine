...
//   1. LegacyQuery::with_component_ids -- rebuilds Vec<&Archetype> per call (Q-011).
...
// Manual Component impls so LegacyQuery::with::<(QBenchPos, QBenchVel)> can exercise
...
/// Baseline: LegacyQuery::with_component_ids rebuilds Vec<&Archetype> on every call.
    ...
    for &n in &[1_000usize, 10_000, 100_000] {
        ...
        group.bench_with_input(BenchmarkId::new("entity_count", n), &n, |b, _| {
            b.iter(|| {
                ...
                let query = LegacyQuery::with_component_ids(
...
///   - `with_typed`: cold `LegacyQuery::with::<(QBenchPos, QBenchVel)>(master)` construction.
...
///   - `with_component_ids`: equivalent path via `LegacyQuery::with_component_ids`, bypassing
...
fn bench_query_one_shot(c: &mut Criterion) {
    ...
    for &n in &[1_000usize, 10_000, 100_000] {
        ...
        group.bench_with_input(BenchmarkId::new("with_typed", n), &n, |b, _| {
            b.iter(|| {
                let query = LegacyQuery::with::<(QBenchPos, QBenchVel)>(ecs.archetype_master());
                black_box(query.len())
            });
        });
