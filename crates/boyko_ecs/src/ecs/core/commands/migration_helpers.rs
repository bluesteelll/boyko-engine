...
pub(crate) fn merged_archetype_id<B: Bundle>(
    ...
    for &cid in source_ids.iter() {
        debug_assert!(len < MAX_MIGRATION_COLUMNS, "migration union exceeds MAX_COMPONENTS");
        combined[len] = cid;
        len += 1;
    }
...
pub(crate) fn migrate_entity_insert<B: Bundle>(
    ...
    //   * The reborrows live for the rest of this function; we hold no
    //     overlapping `&mut self` after this point.
    ...
    bundle.for_each_component_bytes(|id, bytes| {
        ...
        unsafe {
            let slot_ptr =
                bundle_base.add(bundle_count * bundle_stride) as *mut BundleSlot<'_>;
    ...
    let pushed =
        target.create_entity_with_ticks(entity.id(), &mut new_row, combined_slice, current_tick);
    ...
    match source.move_out_entity(InlandPoolId(source_row)) {
        ...
        RemoveOutcome::Swapped { moved_entity } => {
            ...
            // Fix its EntityInland.unit_index.
    ...
    // Step 6: update `entity`'s `EntityInland` to point at target's slot.
...
pub(crate) fn migrate_entity_remove<C: Component>(
    ...
    // the C value would leak (no destructor would ever run on its bytes).
    {
        ...
        //     `move_out_entity` swap-removes it bytewise (no drop).
