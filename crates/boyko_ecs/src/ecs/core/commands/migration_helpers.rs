...
pub(crate) fn migrate_entity_attach_ids_with_bytes(
    ...
    // dead — Phase 2 can mint `world_ptr` with no live reborrow (SAFETY-1).
    {
        ...
        // minted (O3).
        //
        // KE14 D1 (tag-attach face), applied to this D9 sibling exactly as A3
        // applied it to `migrate_entity_attach_ids`: the declaration record was
        // the walk here, so ANY attach onto an entity whose archetype declares
        // a dense id resolved a pool that cannot exist.
        for &retained_cid in source.table_component_ids() {
            let src_pool = source
                .component_pools()
                .get_pool(retained_cid)
