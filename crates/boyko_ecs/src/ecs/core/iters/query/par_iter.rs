    ...
    F: QueryFilter,
{
    ...
    {
        for_each_impl::<D, F, Body>(self.state, self.world, self.batching, self.meta, body, false);
    }
}
    ...
    F: QueryFilter,
{
    ...
    where
        Body: Fn(D::Item<'_>) + Send + Sync,
    {
        for_each_impl::<D, F, Body>(self.state, self.world, self.batching, self.meta, body, true);
    }
}
