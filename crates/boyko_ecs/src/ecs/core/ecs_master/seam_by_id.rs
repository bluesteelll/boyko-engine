...
impl EcsMaster {
    ...
    fn add_by_id_expanding_requires(
        ...
        component_registry::for_each_required_id_excluding(&[id], |req_id| {
            // PRESENT ⇒ SKIP, decided on `all_component_ids()` — the DECLARATION
            // RECORD — and NOT on `has_component_id` or `table_component_ids()`.
            // Both of those filter the non-signature ids out, so a dense required
            // component the entity ALREADY carries would read as absent and be
            // re-constructed into the attach set, which is the phantom-column trap
            // the `caller_is_table` guard above exists to avoid.
            //
            // ⚠ A6 merge: this comment used to cite `migrate_entity_insert`'s Step 2b
            // as the precedent. That site now tests `table_component_ids()` (A3 /
            // KE14 D1), because it resolves a POOL from the id; this one only asks
            // whether the entity has the component, resolves nothing, and therefore
            // wants the unfiltered list — the same reading `merged_archetype_id_dyn`
            // makes for its own union seed.
            //
            ...
            let already = unsafe {
                let archetype = &*inland.archetype_ptr();
                archetype.all_component_ids().contains(&req_id)
            };
            if already {
                return;
            }
