//! Kernel gate — **no walk that resolves a `ComponentPool` may read an archetype's
//! DECLARATION RECORD.**
//!
//! # The false premise these tests pin
//!
//! `Archetype::all_component_ids()` is NOT the archetype signature. `create_by_ids`
...
//! `StorageKind::Dense` id stays in that record forever, even though it
...
//!
//! ⚠ A6 merge: the accessor these tests were written against, `component_ids()`, no
//! longer exists. A3 / KE14 D1 split it into `all_component_ids()` (the record above)
//! and `table_component_ids()` (the signature-storage subsequence, which every
//! pool-resolving walk now reads). The defect class and these tests' subjects are
//! unchanged; only the name of the list each assertion observes moved. See the
//! RESOLVER-half banner below for the one place where the two sides asserted
//! different things.
//!
...
/// Returns the DECLARATION RECORD of the archetype the entity currently lives in — the id
/// list `create_by_ids` stored verbatim, non-signature ids included.
///
/// ⚠ A6 merge: this was `component_ids()`. A3 / KE14 D1 split that accessor in two, and the
/// split is what these tests now have to say out loud: `all_component_ids()` is the record,
/// [`table_ids_of`] is the pool-bearing subsequence, and only the second one is walked by
/// anything that resolves a `ComponentPool`.
...
/// Returns the SIGNATURE-STORAGE subsequence — every id here owns a `ComponentPool`, and this
/// is the list every retained-copy and pool-resolving walk reads on this tree.
fn table_ids_of(ecs: &EcsMaster, e: Entity) -> Vec<ComponentId> {
    let arch_id = ecs.entity_archetype_id(e).expect("entity is live");
    ecs.archetype_master()
        .get_archetype(arch_id)
        .expect("archetype is live")
...
// ════════════════════════════════════════════════════════════════════════════
// The RESOLVER half — a newly-minted archetype must carry no non-signature id in the
// POOL-BEARING list, whatever its declaration record says.
//
// ⚠ A6 merge — this half was REPHRASED, not weakened, and the two sides are worth stating.
// The lane asserted `!component_ids().contains(dense)`: it screened both `_dyn` resolvers so
// the seed itself was filtered, because an unfiltered seed manufactured the next victim for
// the retained walks above. The line (A3 / KE14 D1) took the other half of the same fix — it
// moved every pool-resolving walk onto `table_component_ids()` and then DELIBERATELY kept the
// poolless ids in the declaration record, so a dense / bitset declarer's `flags (…)` still
// reaches the derived archetype (KE10). Under the line's fix the lane's screen is not merely
// redundant, it would delete that propagation — so the screen goes and the assertion moves to
// the list the walks actually read. What is asserted is the SAME property (`no non-signature
// id where a pool is resolved`) against the accessor that now carries it; the record is
// observed too, and its retention is shown INERT by the data assertions that follow.
// ════════════════════════════════════════════════════════════════════════════
...
fn attach_resolver_does_not_seed_a_non_signature_id_into_a_new_archetype() {
    ...
    let table = table_ids_of(&ecs, e);
    let record = retained_ids_of(&ecs, e);
    assert!(
        !table.contains(&A7Dense::component_id()),
        "the attach resolver put a non-signature id into the POOL-BEARING list of the \
         newly-minted archetype — every retained-copy walk resolves `get_pool` out of this \
         list, so any entity that later dedups into the archetype is a victim; table list \
         was {table:?}, declaration record was {record:?}"
    );
    assert!(
        table.contains(&A7Pos::component_id()) && table.contains(&tag.component_id()),
        "the signature ids must still all be there; table list was {table:?}"
    );
    assert!(
...
fn detach_resolver_does_not_seed_a_non_signature_id_into_a_new_archetype() {
    ...
    let record = retained_ids_of(&ecs, e);
    assert!(
        !table.contains(&A8Dense::component_id()),
        "the detach resolver carried a non-signature id into the POOL-BEARING list of the \
         newly-minted archetype; table list was {table:?}, declaration record was {record:?}"
    );
    assert!(
        table.contains(&A8Pos::component_id()) && table.contains(&tag2.component_id()),
        "the surviving signature ids must all be there; table list was {table:?}"
    );
