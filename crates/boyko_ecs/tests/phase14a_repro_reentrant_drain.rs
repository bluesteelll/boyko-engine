...
#[cfg_attr(
    miri,
    ignore = "miri-slow: Phase 9.1-era ignore, NOT re-validated since Phase 9.3a made every executor wait site Miri-cooperative (schedule.rs carries #[cfg(miri)] yield_now() at both drain sites); miri_schedule_parallel runs the same 2-worker Schedule::run green under Miri, so this is LIKELY REMOVABLE -- but nobody has re-run it. The reason given here until 2026-09-10 was 'unverifiable on this box (Miri needs the nightly MSVC toolchain, which was removed)', and that blocker DOES NOT EXIST: MEASURED 2026-09-18, both nightlies carry miri -- nightly-x86_64-pc-windows-gnu is miri 0.1.0 (8925ea358a 2026-08-20) and nightly-x86_64-pc-windows-msvc is miri 0.1.0 (a36d05efab 2026-09-09). The 2026-09-10 record this replaced put the msvc one at 2026-05-29, three months older, and explained the project's gnu pin by that gap; the gap has INVERTED (msvc is now three weeks newer); the pin then stayed on receipt continuity until 2026-09-21, when rung AH moved every Miri recipe to nightly-x86_64-pc-windows-msvc with the receipts under docs/threadpool/receipts/ re-derived on both nightlies, see .cargo/config.toml. What remains is unrun, not unrunnable. miri_phase14a covers the enqueue-then-drain path single-threaded meanwhile."
)]
