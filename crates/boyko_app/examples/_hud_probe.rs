...
/// probe override
fn hud_distance() -> f32 { std::env::var("BOYKO_HUD_DIST").ok().and_then(|v| v.parse().ok()).unwrap_or(HUD_DISTANCE) }
...
fn setup(
    ...
    //    the default material slot 0 the same way (`runner.rs:273`).
    for mesh in [
        floor_mesh,
        wall_ns_mesh,
        wall_ew_mesh,
    ...
    commands.spawn(FlyCameraBundle {
        transform: Transform::from_translation(Vec3::new(0.0, 1.0, -3.2)),
        global: GlobalTransform::IDENTITY,
        ...
        fly: FlyCamera { yaw: 0.0, pitch: -0.04, speed: 7.0, sensitivity: 0.0 },
    });
...
fn spawn_hud_panel(
    ...
    // (`bindless.rs:150-168`), so the instant the panel is minified even slightly the GPU
    // blends in mip 1 and the glyph strokes go soft. A HUD is the one surface that is
    // never meant to be minified: a single level makes "sample mip 0" the only thing the
    // sampler can do, and the 1:1 sizing below makes that the right thing.
...
fn bake_hint_texture() -> Option<TextureData> {
    ...
    for px in rgba.chunks_exact_mut(4) {
        px.copy_from_slice(&[10, 12, 16, 255]);
    }
