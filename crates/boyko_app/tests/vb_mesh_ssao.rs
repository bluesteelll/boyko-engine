//! Rung R9b (docs/R9-VB-SPLIT-PLAN.md) — the VisibilityBuffer geo/shade SPLIT golden dump
//! (`vb_geo` → SSAO(VB_THIN) → à-trous → `vb_shade_split`): the `[vb_mesh]` five-sphere scene
//! with `SsaoConfig::High`, which arms the boot resolver's `mesh_geo_shade_split`.
//!
...
/// **The VisibilityBuffer geo/shade SPLIT golden dump (rung R9b).** The SAME five-sphere
/// `[vb_mesh]` scene with `SsaoConfig::High` — the boot resolver arms `mesh_geo_shade_split`
/// (then frozen by `RenderPathFrozenConsumers`), so the frame runs `vb_geo → ssao(VB_THIN) →
/// à-trous×3 → vb_shade_split` instead of the fused `vb_resolve` — the owner's visual
/// sign-off gate for rung R9b (`goldens/PINS.toml`'s `[vb_mesh_ssao]`). NOT byte-compared to
/// any deferred SSAO pin: the split's geometric interpolated normal differs from the deferred
/// mapped-normal gather by design.
///
...
fn vb_mesh_ssao_screenshot_dump() {
    ...
    // Rung R9b: request `VisibilityBuffer × Mesh` + SSAO High — both inserted AFTER
    // `add_plugins` (the post-plugins owner-override precedent), so the boot resolver sees
    // `ssao_on = true` and commits the split.
