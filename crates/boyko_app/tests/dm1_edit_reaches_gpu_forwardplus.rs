//! DM1 red-first test (1) on **ForwardPlus x Both**: an authored material edit made after boot reaches the
...
#[test]
#[ignore = "gpu-windowed: needs a windowed GPU device; DM1 red-first gate (1) on ForwardPlus x Both, run with --test-threads=1"]
fn dm1_edit_reaches_gpu_forwardplus() {
    dm1_common::edit_reaches_gpu::run("dm1_edit_reaches_gpu_forwardplus", RenderPath::ForwardPlus);
}
