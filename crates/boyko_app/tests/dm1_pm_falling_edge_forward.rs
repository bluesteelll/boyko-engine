//! DM1 red-first test (4) on **Forward x Mesh**: live defect D-2, the `PerInstanceMaterial` ring's
...
#[test]
#[ignore = "gpu-windowed: needs a windowed GPU device; DM1 red-first gate (4) / D-2 on Forward x Mesh, run with --test-threads=1"]
fn dm1_pm_falling_edge_forward() {
    dm1_common::pm_falling_edge::run("dm1_pm_falling_edge_forward", RenderPath::Forward);
}
