...
/// 2. ⚠️ **DEAD as of EM2′ (`0afcbd7d`).** This fact used to read "this kernel
///    does not recycle [`EntityId`]" (MEASURED at A1: spawn `[0..5]`, despawn
///    four, the next six ids are `[6..11]`; recycled = none), and it was the
///    load-bearing one — with no id reuse there was no id under which fact 1
///    could be undone. `entity_ids_are_not_recycled_today` was the tripwire, and
///    it FIRED: ids now recycle on the deferred route. Its successors
///    `entity_ids_are_recycled_on_this_kernel` and
///    `a_completion_pair_never_outlives_its_frame_so_a_recycle_cannot_replay_it`
///    (`tests/miri_a1_tween.rs`) pin the new fact and gate the consequence
///    directly. The key is still safe, but on facts 1 and 3 ALONE — a
///    schedule-shaped guarantee where this was a kernel-shaped one. Carrying the
///    generation is escalated in `docs/OPEN-QUESTIONS.md` (it needs an
///    `Entity`-yielding `iter_entities_mut`, a kernel change).
...
/// That pricing stands on the bytes; what changed is the risk it was weighed
/// against, which is why the question is re-opened rather than re-answered here.
...
/// consumed at `query/filter.rs:1205`, `:1225`, `:1493` and `:1503`. *(`schedule.rs:152`
