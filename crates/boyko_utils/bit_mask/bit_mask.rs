...
impl<T: BitStorage> BitMask<T> {
    ...
    pub fn activate_category(&mut self, category: u32) -> Result<()> {
            ...
            category as usize < self.elements.len(),
            "Category index out of bounds: index is {}, but length is {}",
            category, self.elements.len()
        );
    ...
    pub fn get_category_elements(&self, category: u32) -> Result<&BitSet<T>> {
        self.elements.get(category as usize).ok_or_else(|| 
    ...
    pub fn get_category_elements_mut(&mut self, category: u32) -> Result<&mut BitSet<T>> {
        self.elements.get_mut(category as usize).ok_or_else(|| 
            anyhow!("Category index out of bounds: index is {}, but length is {}", 
                  category, self.elements.len())
        )
    }
...
// Implement reference versions
impl<T: BitStorage> BitAnd<&BitMask<T>> for BitMask<T> {
    type Output = Self;
...
impl<T: BitStorage> BitAnd<BitMask<T>> for &BitMask<T> {
    ...
    #[inline]
    fn bitand(self, rhs: BitMask<T>) -> Self::Output {
        self.intersection(&rhs)
    }
}
...
impl<T: BitStorage> BitOr<&BitMask<T>> for BitMask<T> {
    type Output = Self;
...
impl<T: BitStorage> BitOr<BitMask<T>> for &BitMask<T> {
    ...
    #[inline]
    fn bitor(self, rhs: BitMask<T>) -> Self::Output {
        self.union(&rhs)
    }
}
...
impl<T: BitStorage> BitXor<&BitMask<T>> for BitMask<T> {
    type Output = Self;
...
impl<T: BitStorage> BitXor<BitMask<T>> for &BitMask<T> {
    ...
    #[inline]
    fn bitxor(self, rhs: BitMask<T>) -> Self::Output {
        self.symmetric_difference(&rhs)
    }
}
...
impl<T: BitStorage> Sub<&BitMask<T>> for BitMask<T> {
    type Output = Self;
...
impl<T: BitStorage> Sub<BitMask<T>> for &BitMask<T> {
    ...
    #[inline]
    fn sub(self, rhs: BitMask<T>) -> Self::Output {
        self.difference(&rhs)
    }
}
