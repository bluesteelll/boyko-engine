...
bool m2_brick_span(float3 p, float3 rd, float3 cell_min, float brick_world, out float t_enter, out float t_exit) {
    ...
    // The brick-AABB ray-span clip body is MACHINE-GENERATED by boyko_shaderdsl::emit (Increment 5b —
    // the first COMPUTED-bool-returning leaf, `return tmax > tmin;`), traced from the single source
    // boyko_shaderdsl::brick::m2_brick_span_body. DO NOT hand-edit: re-run
    //   cargo run -p boyko_shaderdsl --features emit --bin emit_field
    // and re-splice between these sentinels. The `bool m2_brick_span(...) {` signature + the closing
    // `}` stay hand-written (framing b); the cmp-.spv gates byte-identity.
    float tmin = 0.0;
    float tmax = 1.0e30;
    [unroll]
    for (uint a = 0u; a < 3u; ++a) {
...
float m2_brick_cubic_hit(Texture3D<float> atlas, SamplerState atlas_smp,
    ...
    // SINGLE-SOURCED (Increment 5c): the DDA marcher body is MACHINE-GENERATED by
    // `boyko_shaderdsl::cubic_hit::m2_brick_cubic_hit_body` over the `EmitCf` backend. EMIT-ONLY:
    // `m2_corner` → `atlas.SampleLevel` cannot run on the CPU, so the body is never instantiated over
    // `EvalCf` and there is NO eval sweep — the cmp-`.spv` is the SOLE byte-identity oracle. The
    // signature, the `t_exit <= t_enter` early-out, and the `const uint W` decl above stay
    // hand-written (framing b). Do NOT hand-edit between the sentinels: re-run
    //   cargo run -p boyko_shaderdsl --features emit --bin emit_field
    // and re-splice. The `sdf_field_edsl_sync` test pins this body to the generator; a hand-edit
    // fails CI. (The 3-way `else if` is `else { if }`, and the `lo_g` ctor is single-line — both
    // .spv-neutral reflows of the generator's text, proven by the cmp-`.spv`.)
...
int select_level(float3 p) {
    ...
    // The clip-map LOD scan body is MACHINE-GENERATED by boyko_shaderdsl::emit (Increment 5a —
    // the first signed-int-returning leaf), traced from the single source
    // boyko_shaderdsl::levels::select_level_body. DO NOT hand-edit: re-run
    //   cargo run -p boyko_shaderdsl --features emit --bin emit_field
    // and re-splice between these sentinels. The `int select_level(float3 p) {` signature + the
    // closing `}` stay hand-written (framing b); the cmp-.spv gates byte-identity.
    [unroll]
