...
// Render P7 POLISH: the SSAO bilateral-blur kernel. The raw `gSsao` is a no-blur HBAO-lite
// gather (2 slices × 4 discrete step radii) → VISIBLE CONCENTRIC RINGS on a broad contact-AO
// region (a mesh floor around an SDF occluder). The fix is an inline NxN WEIGHTED blur of the
// AO INSIDE the resolve's `ssao_mode != 0` combine (NO new pass): `ssao_blurred` is the
// weighted mean of the (2*R+1)² neighbour taps whose `gViewT` is within `SSAO_BLUR_DEPTH_TOL`
// of the CENTER's (a HARD bilateral DEPTH gate — the blur does NOT bleed AO across the
// mesh↔SDF silhouette, where `view_t` jumps far more than the tol). Change C (Render P7 POLISH)
// replaced the old hard-binary UNIFORM average with a SMOOTH per-tap weight `w_spatial *
// w_depth` — both `clamp01(1 - x²/sigma²)` polynomials (no `pow`/`exp`) standing in for a
// gaussian — so taps near the sigma radius fade toward zero weight instead of counting fully.
// The center always passes its own gate with weight 1, so the weight sum is ≥ 1. `SSAO_BLUR_R
// == 5` → an 11×11 tap window (WIDE enough to dissolve the residual low-frequency angular-
// undersampling blobs the smaller 7×7 box left — the owner-visible "blotchy" contact shadow;
// tuned on-GPU with `SSAO_BLUR_SPATIAL_SIGMA == 7.0` so the polynomial weight spans most of the
// window instead of collapsing it). `SSAO_BLUR_DEPTH_TOL == 0.1` view-t units stays WITHIN a flat
// surface (the mesh floor has constant `view_t`) yet rejects the silhouette. The HOST mirror is
// `golden_ssao_blur` (compute.rs), byte-mirror-friendly (integer/`abs`/compare/`mul`/`div`/
// `clamp` only — no transcendental); GPU == host within the existing ±2/255.
static const int   SSAO_BLUR_R              = 7;
static const float SSAO_BLUR_DEPTH_TOL      = 1.0;
static const float SSAO_BLUR_SPATIAL_SIGMA  = 100.0; // px; ~flat over the 15x15 (bell far wider than R)
static const float SSAO_BLUR_DEPTH_SIGMA    = 1.0;  // view_t; RESIDUAL falloff scale
// The slope-aware depth-gate gradient clamp: the blur predicts each tap's view_t from the
// center's clamped local gradient (min-magnitude one-sided differences) and gates the RESIDUAL
// `vt - view_t - dz_pred` — the bilateral band follows a SLOPED floor / CURVED surface instead
// of truncating the kernel (the raw-`dz` gate collapsed the kernel to a near-1D sliver on any
// surface whose view_t drifts more than the tolerance across the kernel radius). The clamp
// bounds a silhouette/background one-sided step (up to the 1e30 sentinel) so a
// cross-silhouette tap can never be "predicted" back inside the band.
static const float SSAO_BLUR_GRAD_CLAMP     = 0.1;  // view_t per pixel; plane-fit slope cap
...
void main(uint3 tid : SV_DispatchThreadID) {
    ...
    if (is_sdf_lit) {
        ...
        if (ssao_mode != SSAO_MODE_OFF) {
            // Render P7 POLISH: the inline BILATERAL blur of `gSsao` (replaces the single
            // center tap — kills the discrete-step RINGS). Change C weights the (2*R+1)²
            // neighbour AO taps by `w_spatial * w_depth` (both `clamp01(1 - x²/sigma²)`
            // polynomials — a transcendental-free gaussian stand-in), instead of the old
            // hard-binary UNIFORM average; a neighbour whose `gViewT` is beyond
            // `SSAO_BLUR_DEPTH_TOL` of the center's is still REJECTED outright (the silhouette
            // guard is unchanged — a neighbour across the mesh↔SDF edge has a far `view_t` and
            // never contributes, so the blur never bleeds AO over the silhouette). The center
            // always passes its own gate with weight 1, so the weight sum is ≥ 1 (no
            // divide-by-zero). `w_spatial` is computed HERE (hand-written glue, it owns
            // `dx`/`dy`) and threaded into the generated tap span below.
            //
            // Render P7 POLISH Track 2: the per-tap gate+weight+accumulate and the tail combine
            // below are MACHINE-GENERATED by `boyko_shaderdsl::emit::emit_hlsl_ssao_blur_tap()` /
            // `emit_hlsl_ssao_blur_combine()` (tracing `boyko_shaderdsl::ssao::ssao_blur_tap_body`
            // / `ssao_blur_combine_body` over the Emit backend) — the SAME single-sourced body
            // whose `<EvalCf>` instantiation (`ssao::ssao_blur_body`) is the host AO oracle
            // `golden_ssao_blur` locks against. The `(2*SSAO_BLUR_R+1)²` loop nest itself (a
            // SIGNED, symmetric-range, `<=`-terminated header) stays HAND-WRITTEN — no `Cf`
            // loop-emit facet can print it — as do the bounds `continue`, the `gViewT`/`gSsao`
            // `Load` calls, and the `w_spatial` weight (the SAME seam discipline the SSAO
            // horizon-step's `Pp` reconstruct uses). The `ssao_blur_edsl_sync` test pins both
            // committed spans to the generator; a hand-edit of either span fails CI.
            // The slope-aware (plane-fit) gate gradient — min-magnitude ONE-SIDED view_t
            // differences from the 4 direct neighbours (coordinate-CLAMPED loads: an image-edge
            // "neighbour" reads the center's own texel, a zero diff — the host oracle's bounds
            // check supplies the same zero), clamped to ±SSAO_BLUR_GRAD_CLAMP. Min-magnitude
            // picks the in-surface side at a silhouette; the tie keeps the +side (the host
            // mirror's `if p.abs() > m.abs() { m } else { p }`). Hand-written glue (it owns the
            // Loads), mirrored bit-for-bit by `golden_ssao_blur` / `ssao_blur_body`'s walk.
            float grad_xp = gViewT.Load(int2(min(coord.x + 1, (int)w - 1), coord.y)) - view_t;
            float grad_xm = view_t - gViewT.Load(int2(max(coord.x - 1, 0), coord.y));
            float grad_yp = gViewT.Load(int2(coord.x, min(coord.y + 1, (int)h - 1))) - view_t;
            float grad_ym = view_t - gViewT.Load(int2(coord.x, max(coord.y - 1, 0)));
            ...
            for (int dy = -SSAO_BLUR_R; dy <= SSAO_BLUR_R; ++dy) {
                for (int dx = -SSAO_BLUR_R; dx <= SSAO_BLUR_R; ++dx) {
                    ...
                    float s = gSsao.Load(c).r;
                    float w_spatial = saturate(1.0 - float(dx*dx + dy*dy) / (SSAO_BLUR_SPATIAL_SIGMA * SSAO_BLUR_SPATIAL_SIGMA));
                    float dz_pred = dzdx * float(dx) + dzdy * float(dy);
                    // === GENERATED ssao_blur_tap BEGIN ===
                    float dz = vt - view_t - dz_pred;
