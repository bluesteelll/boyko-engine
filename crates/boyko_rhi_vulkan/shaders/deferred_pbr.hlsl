...
void main(uint3 tid : SV_DispatchThreadID) {
    ...
    if (is_sdf_lit) {
        ...
        if (ssao_mode != SSAO_MODE_OFF) {
            ...
            //
            // Render P7 POLISH Track 2: the per-tap gate+accumulate and the tail combine below
            // are MACHINE-GENERATED by `boyko_shaderdsl::emit::emit_hlsl_ssao_blur_tap()` /
            ...
            // loop-emit facet can print it — as do the bounds `continue` and the `gViewT`/`gSsao`
            // `Load` calls (the SAME seam discipline the SSAO horizon-step's `Pp` reconstruct
            // uses). The `ssao_blur_edsl_sync` test pins both committed spans to the generator; a
            // hand-edit of either span fails CI.
            float ssao_sum = 0.0;
            float ssao_cnt = 0.0;
