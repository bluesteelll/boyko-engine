...
// === GENERATED NORMAL BEGIN (boyko_shaderdsl::emit) ===
// Surface normal via central differences of `sdf` (the gradient of the WHOLE
// edit-list field). MACHINE-GENERATED by tracing the generic normal leaf
// `boyko_shaderdsl::normal::sdf_normal_body` over the `Emit` backend — the SAME
// single source whose `Eval` backend (`impl FieldScalar for f32`) IS the host normal
// `boyko_sdf_math::sdf_edit_list_normal`. The GRAD_H swizzle offsets (`e.xyy` /
// `e.yxy` / `e.yyx`) are emitted TEXTUALLY off `float2 e = float2(GRAD_H, 0.0)` and
// `sdf` is CALLED (not inlined: it owns the edit-list `[loop]`). Regenerate with:
//     cargo run -p boyko_shaderdsl --features emit --bin emit_field
// Do NOT hand-edit this body — edit `boyko_shaderdsl::normal` and re-emit. The
// generated body is character-identical to the prior hand-frozen `sdf_normal`, so it
// compiles to SPIR-V BYTE-IDENTICAL to `sdf_field_probe.baseline.dis` (the
// `field_probe_gate` GATE-1 tripwire is the empirical proof).
...
// === GENERATED NORMAL END (boyko_shaderdsl::emit) ===
