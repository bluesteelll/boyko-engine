...
//!
//! **Track 1 scope**: this is a CPU-only drift gate (no `.hlsl`/`.spv` change — the shader still
//! carries the hand-written box blur verbatim; splicing the eDSL-generated span into
//! `deferred_pbr.hlsl` is a separate, byte-identity-gated rung). `boyko_shaderdsl` is a
//! DEV-dependency of `boyko_rhi_vulkan` (the shipped lib must not link the eDSL), so this guard
//! lives in `tests/`, mirroring `ssao_edsl_sync.rs`'s host-mirror tests.
//!
