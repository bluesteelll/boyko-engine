...
//! distance `falloff` range gate, the `sampleCos = dot(delta,dir) / max(length(delta),
//! SSAO_EPS)` horizon cosine, and the `hc = max(hc, sampleCos * falloff)` running horizon
//! max) is MACHINE-GENERATED by `boyko_shaderdsl::emit::emit_hlsl_ssao()` — the eDSL whose
