...
//! as a DEDICATED PASS (`sdf_mesh_shadow.comp`, plan Rev 10 DP1..DP5). Precisely: only the MARCH
//! moved to that pass — the CONSUME half (the `[[vk::binding(10, 0)]] gSdfTerm` declaration and
//! the mode-gated `.Load` + `min`-combines, DP2) is compiled into the shipping `.spv` of exactly
//! these rows (`vb_resolve.comp.hlsl:165/309`, `vb_shade.comp.hlsl:204/470`,
//! `vb_shade_split.comp.hlsl:203/478`; only the non-shipping `-D VB_SV0_KILL=1` A/B removes it).
//! A sweep of SV0 therefore DOES edit and re-bless these rows' sources. The gate survives both
//! turns because the rows it enumerates **ship regardless of either implementation** — the
