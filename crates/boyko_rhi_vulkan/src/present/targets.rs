...
impl DeferredSets {
    ...
    fn build(
        ...
        // rung-1b binding 20 = the HWRT soft-shadow-params UBO.
        #[cfg(feature = "hwrt")]
    ...
    unsafe fn destroy(self, ctx: &VulkanContext) {
        ...
        unsafe {
            ...
            // TAA-under-VB: the `viewt_from_depth_rz` set RING, `Option`-guarded (present only
            // when `scene.viewt_from_vb_depth` was armed — `VisibilityBuffer × Mesh` with TAA
            // on). Built AFTER `vb_set0_tex_froxel` and BEFORE the HWRT resolve ring (so
            // destroyed between them, reverse acquisition).
            if let Some(rz) = self.viewt_from_vb_depth_set {
                for g in rz {
                    RhiDevice::destroy_bind_group(ctx, g);
                }
            }
