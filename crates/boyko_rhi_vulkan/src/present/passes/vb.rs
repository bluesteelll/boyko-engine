...
impl Renderer<'_> {
    ...
    pub(crate) unsafe fn record_vb(
        ...
        // === Dynamic-materials DM1: the material-table upload, at the position `declare_vb_graph`
        // declared it (right after `light_upload`, before every VB table reader). Recorded ONLY on
        // an upload frame. ===
        if let Some(material_upload) = plan.material_upload {
            ...
            ts.cmd();
            self.record_vb_pass(material_upload, cmd, targets, forward, vb, scene, fi);
