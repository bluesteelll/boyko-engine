...
impl Renderer<'_> {
    ...
    pub(crate) unsafe fn record_forward(
            ...
            )
        {
            ...
            //
            // On every current ForwardPlus boot this `match` always takes the `None` arm:
            ...
            // `None` on a ForwardPlus boot and this whole `if let` block does not execute. This
            ...
            // future ForwardPlus froxel-cull wiring landing without a matching push/dispatch
            // update here) ahead of that wiring existing.
