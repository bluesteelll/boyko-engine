...
//! `&EcsMaster`.
//!
//! # Source 1 is `table_component_ids()`, and the filter is the rung
//!
//! `Archetype::all_component_ids()` is **not** the archetype signature. It is the raw id
//! list handed to whichever call first minted that archetype, and it **retains** `Bitset`
//! and `Dense` ids; only the signature *mask* is filtered. `table_component_ids()` is the
//! signature-storage subsequence of it — the kernel's own `is_signature_storage` screen,
//! applied once at the mint funnel (A3 / KE14 D1) instead of once per consumer — and it is
...
use boyko_ecs::ecs::core::component::component_registry::{
    MAX_COMPONENTS, ResidencyKind, get_layout, residency_class,
};
...
fn visit_components<F: FnMut(IdEntry)>(
    ...
    // The `&Archetype` is confined to this block and dies before source 2 runs.
    {
        ...
        for &id in archetype.table_component_ids() {
            // NOT a tidy-up: the DECLARATION record (`all_component_ids()`) retains Bitset
            // and Dense ids, and an unfiltered read reports them on entities that do not
            // carry them, because archetype dedup keys on the filtered mask. See this
            // module's header. The filter is `table_component_ids()` itself: A3 / KE14 D1
            // moved the per-consumer `is_signature_storage` screen into the accessor, so
            // this walk asserts the same property through the kernel's own subsequence
            // rather than re-deriving it per turn.
