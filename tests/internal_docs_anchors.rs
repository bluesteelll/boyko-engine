...
//!   form (`(65~`, FEATURE_MAP.md:758); dropping `(-[0-9]+)?` also reports 24, because one is
...
fn ignore_markers_are_measured_by_what_they_silence() {
    ...
    for (doc, scan) in &scans {
        ...
        if !scan.stale_ignored.is_empty() {
            report.push_str(&format!(
                "docs/{doc}: marker(s) that silence nothing\n{}\n",
...
fn rs_line_citations_written_inside_rust_sources_are_bounds_checked() {
    ...
    /// shrunk under the citation — `boyko_macros/src/lib.rs` (562 lines), `data.rs` (980) and
    /// `ecs_master.rs` (1917, the god-file the refactoring campaign split).
    ...
    const UNBINDABLE_RS_MAX: usize = 176;
...
///
/// Making it a comparison was measured and is not available at this revision. Of the **18** markers
/// in the corpus, **7** carry a machine-readable command inside the SAME three-line window the
/// proximity guard reads — the two `#[ignore` rows and the two `REFLECTION-PLAN-CORE.md`
/// self-count rows of the twins, the F17 row of `REFLECTION-PLAN-CORE.md`, and the F22 and
...
/// about a marker would become one. Even over those seven a parser would not be a gate: F22 writes
/// its population in prose (*"over `crates/`"*) rather than in the command, and the two `#[ignore`
/// rows print a `wc -l` pipeline this vocabulary has no shape for. **7** markers sit on lines whose
...
///   Live in the corpus: **903** and **3283** for the two `seam_by_id` files, **1916** for
...
/// * **The census's OWN figures, quoted back into the documents** — 242 bound `.rs` citations, 174
///   unbindable, 5 dead, 179 formerly dropped, 12 known-stale. ⚠️ The first read **240** until
...
/// times.** Both `OPEN-QUESTIONS.md` twins cite THIS file by line — **8 per twin, 16 between
/// them** at this revision, four of them at numbers a recent round moved — and **no check in
...
/// besides. Every edit to this file that moves a line silently rots those 16, and this round
...
fn documents_that_count_the_tree_are_re_measured() {
    ...
    /// only three of the eighteen markers have a machine-readable command beside them at all.
...
fn figures_this_file_writes_about_the_tree_are_re_derived() {
    ...
    // this — the god-file has no trailing newline, so the two answers are 1916 and 1917, and a gate
