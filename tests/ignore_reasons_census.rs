...
/// Floor on ignore sites found. The tree held 311 (168 plain + 143 `cfg_attr`) when measured on
/// 2026-09-19 on the colored-solver-default lane; this is well below it and exists only to catch a
...
/// its red-first tests, after growing for weeks, then rose by one with that lane's release-only
/// SIMD on/off test) — do NOT read it as the current count, and
