...
| **R2c-tail (b)** ⚠️ **OPEN, owner-scoped** | Only the VB raster site is armed. The 3 remaining camera sites (`forward.rs` ×2, `gbuffer.rs`) are untouched — **deliberate: the owner scoped this campaign to VB for now**. The 6 shadow sites stay unculled on their own merits (an off-screen caster still casts into view, and CSM cascade fitting CONSUMES the caster set, so culling casters feeds back into the fit and moves pixels) | per-site, when the owner widens the scope | per-site |
