...
| **D7** — the `.ui` `#[derive(UiVocab)]` registration table (architecture rung 1) | **soft, and only for the equivalence gate** | The construct emits Rust, never `.ui` text, so D7 does not gate any rung here. *(2026-08-26, [`UI-PLAN-SPRITES-DECISIONS.md` **S-D20**](UI-PLAN-SPRITES-DECISIONS.md#s-d20--the-s6-pre-build-audit-the-cursor-hole-closes-with-a-hook-and-six-of-the-rungs-own-sentences-did-not-survive-the-tree) (7) — **D7 has no owning document.** This row, [`UI-PLAN-SPRITES.md` §0](UI-PLAN-SPRITES.md#0--what-this-plan-owns-and-what-it-does-not), [`UI-PLAN-ANIMATION.md` §8](UI-PLAN-ANIMATION.md#8--dependencies-stated-explicitly) and `UI-PLAN-INTERACTION.md:518-521` each name a different owner or none, and no rung in any ladder lands it. SCOPE call filed in `docs/OPEN-QUESTIONS.md`. **Coordinate read at its target 2026-08-28 by the tenth pass and re-read by the eleventh: [`UI-PLAN-ANIMATION.md` §8 Dependencies](UI-PLAN-ANIMATION.md#8--dependencies-stated-explicitly)**, the D7 row itself *(read that day as `:3105`, before the 2026-08-28 split moved §8 into the index file; the two struck coordinates below are left as bare numbers on purpose — they are records that a referent was **deleted**, and an anchor would assert the opposite)*. It read `:846`, which no longer resolves; the twins were re-aimed to `:2530` on 2026-08-28 and this copy was missed because that sweep's scope was "documents the landing edited" — **and `:2530` was itself wrong — read on the part-10 tree as prose about an unrelated axis — so the sweep that missed this copy had nothing correct to propagate to it**. ⚠️ *(Part 11, 2026-08-28: that description had since rotted too — read at `:2530` on the tree part 11 inherited, it was a different sentence again, with the prose part 10 quoted 200 lines below. **The description of a discarded coordinate is an anchor, and until this pass the campaign had never re-read one.**)* The `UI-PLAN-INTERACTION.md` coordinate beside it is also re-aimed: `:501` is a component table row; the statement that D7 has no owning document is `:518-521`.)* It gates **how wide the three-way equivalence corpus can be**: a component `.ui` cannot spell has no third leg. See U11 and §13 Q2. |
...
*(precondition note, 2026-08-21 at the S5 pre-build audit — [`UI-PLAN-SPRITES-DECISIONS.md` **S-D16**](UI-PLAN-SPRITES-DECISIONS.md#s-d16--the-flipbook-writes-uispritesheetindex-and-it-must-a-dense-changedc-inside-or-is-measurably-dead). **The
...
*(field-list note, 2026-08-21 — [`UI-PLAN-SPRITES-DECISIONS.md` **S-D12 (2)**](UI-PLAN-SPRITES-DECISIONS.md#2-the-source-side-split-is-an-authored-border_uv-f32-4-in-fractions-of-the-current-sub-rect): `UiNineSlice` gained
