...
| **D7** — the `.ui` `#[derive(UiVocab)]` registration table (architecture rung 1) | **soft, and only for the equivalence gate** | The construct emits Rust, never `.ui` text, so D7 does not gate any rung here. *(2026-08-26, `UI-PLAN-SPRITES.md` S-D20 (7) — **D7 has no owning document.** This row, `UI-PLAN-SPRITES.md` §0, `UI-PLAN-ANIMATION.md:846` and `UI-PLAN-INTERACTION.md:501` each name a different owner or none, and no rung in any ladder lands it. SCOPE call filed in `docs/OPEN-QUESTIONS.md`.)* It gates **how wide the three-way equivalence corpus can be**: a component `.ui` cannot spell has no third leg. See U11 and §13 Q2. |
