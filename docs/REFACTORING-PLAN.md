...
**⚠ CORRECTION (investigation 2026-07) — SSAO does NOT collapse via spec-const.** The SSAO slice/step loops are **`[unroll]`** (`sdf_ssao_medium.comp.hlsl:271/292/331`), NOT `[loop]`. DXC `[unroll]` requires a COMPILE-TIME bound; a spec-const is set at pipeline-CREATE, so DXC can't unroll it → the spec-const version would fall back to a DYNAMIC loop = a real per-invocation perf REGRESSION. So the SSAO variant files ARE justified (the "measured==shipped, zero unroll cost" rationale IS valid here — 3 unroll counts genuinely need 3 `.spv`). SSAO's ONLY dedup: delete the dead byte-identical `sdf_ssao.comp.hlsl` (unreferenced dup), and confirm the 3 are eDSL-emit-GENERATED (single-sourced body) rather than hand-maintained. Do NOT spec-const-collapse SSAO ([[feedback-hybrid-perf-decides]] — perf wins). Rung-1a's "collapse SSAO/DDGI variant-explosion" intent holds for DDGI (`[loop]`), NOT SSAO (`[unroll]`) — a per-loop-attribute call.
