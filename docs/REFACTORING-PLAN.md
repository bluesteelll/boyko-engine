...
**The rationale is sound but does NOT justify FILE duplication — FOR `GI_MAX_IT`.** A **specialization constant** delivers the *identical* property WHEN the loop is `[loop]` (already dynamic): the trip count resolves at *pipeline-create*, structurally identical to the baked const, from **one** `.hlsl`. `GI_MAX_IT`'s loop is `[loop]` (verified) — so its collapse is free (SHIPPED @3c10826, `SHADOW_RAY_COUNT` `[loop]` precedent).

**⚠ CORRECTION (investigation 2026-07) — SSAO does NOT collapse via spec-const.** The SSAO slice/step loops are **`[unroll]`** (`sdf_ssao_medium.comp.hlsl:271/292/331`), NOT `[loop]`. DXC `[unroll]` requires a COMPILE-TIME bound; a spec-const is set at pipeline-CREATE, so DXC can't unroll it → the spec-const version would fall back to a DYNAMIC loop = a real per-invocation perf REGRESSION. So the SSAO variant files ARE justified (the "measured==shipped, zero unroll cost" rationale IS valid here — 3 unroll counts genuinely need 3 `.spv`). Do NOT spec-const-collapse SSAO ([[feedback-hybrid-perf-decides]] — perf wins). Rung-1a's "collapse SSAO/DDGI variant-explosion" intent holds for DDGI (`[loop]`), NOT SSAO (`[unroll]`) — a per-loop-attribute call.

**⚠⚠ SECOND CORRECTION (investigation 2026-07, verified) — there is NO dead `sdf_ssao.comp.hlsl` to delete.** The file is the LIVE single-source base: `emit_ssao_variants.rs:39` reads it and swaps only its 6-line `static const SSAO_*` tuning header per preset to WRITE the 3 `sdf_ssao_{low,medium,high}.comp.hlsl`; `ssao_edsl_sync.rs:100/188/228` uses it as the byte-identity anchor (asserts Medium == base) and as one of the eDSL horizon-step sync targets. All four `.hlsl` are 22720 bytes (the presets' numeric literals are equal-length; only Medium is byte-IDENTICAL in content). Deleting the base would break BOTH the emitter (`base_path`) and the sync gate (`base_hlsl_path`). **SSAO is ALREADY optimally single-sourced (1 base + 3 generated variants + a byte-identity/eDSL sync gate + correct `[unroll]` perf preservation) — NO ACTION. The A-1 "delete dead file" line was wrong.**

**Fix (A1):** collapse `GI_MAX_IT` (4→1) via spec-const (SHIPPED @3c10826, −4 `.hlsl`/`.spv`). **SSAO stays 4 files** (`[unroll]` — spec-const would regress perf; already single-sourced, no dead file). Net **−4 `.hlsl`, −4 `.spv`, −~60 registry lines** (not −8/−8 — the SSAO half was ruled out on perf grounds).
...
|---|---|---|---|---|
| A-1 | Collapse `GI_MAX_IT` (4→1) via spec-const (SSAO ruled out: `[unroll]`; no dead file) | spec-const (infra ready) | −4 files, −4 `.spv`, −~60 reg lines | golden `58f6c6c3` (GI-OFF) + `ddgi_probe_gi_arm` smoke (GI-ON) — SHIPPED @3c10826 |
| A-2 | Kill the 35 hand-counted embed sizes | **`embed_spirv!` macro** deriving `SpirvBlob<{ include_bytes!($path).len() }>` (NO build.rs needed — the size comes from the file, written once) | −35 hand-counts + the whole drift failure mode | golden `58f6c6c3` ±hwrt + eDSL sync — **SHIPPED @b32f48a** |
| A-3 | Shrink `compute.rs` | move the ~3017 test lines (10 `#[cfg(test)] mod`) → `compute/tests.rs` | 6743 → 3739 | golden `58f6c6c3` ±hwrt — **SHIPPED @b32f48a** (std430_params extraction deferred; the test move + A-2 already halved the file) |
| A-4 | Variant-manifest doc for the stay-separate `SHADOW_STAGE×HWRT×MV` matrix | doc | — | **SHIPPED → [SHADER-VARIANT-MANIFEST.md](SHADER-VARIANT-MANIFEST.md)** |
| A-5 | Keep eDSL for math bodies (no change) | — | — | — |

**A-2 note:** the original plan proposed a `build.rs` manifest codegen. Investigation found each embed was
ALREADY `SpirvBlob<N>(*include_bytes!(path))` — the bytes come from the file; `N` was only a hand-counted
drift guard. A `macro_rules! embed_spirv` deriving `N` from `{ include_bytes!($path).len() }` kills every
hand-count with ZERO codegen machinery, and (unlike an inline `SpirvBlob<{…len()}>`) writes `$path` ONCE so
the length and the bytes can never disagree. Simpler and safer than the proposed build.rs.
