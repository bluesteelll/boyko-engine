...
| **KC-08** | Gang `par_phases` + `par_range` | physics §9/§10.1 rev 5 as written (`LaneBoard`, `run_lane`, `poison()`, `GangAbort`, `&mut GangLane`) | lane 0 frame; board in `PoolInner` | rev-5 protocol; NB3 cold release assert; NB4 `GangLane` is `!Clone + !Copy + !Sync` | one board probe at each of three idle-loop sites per worker iteration: after the steal, after `[J]crates/boyko_threadpool/src/worker.rs:115`, after `:126` (`PHYSICS-ECS-UNIFICATION-DESIGN.md:3055`); lowest priority | the ~~4 hand-rolled physics `pool.scope` sites~~ 5 hand-rolled physics `pool.scope` sites and 5 `try_with_active_pool` sites ⚠ *2026-09-24, doc fix-up O2: B1 PC-d; `docs/memory/ledger/ug02-pins.tsv:10` pins physics_pool_scope 5 and physics_active_pool 5, as physics `PHYSICS-ECS-UNIFICATION-DESIGN.md:90` and `:733` now read*; defect 5's serial share: `par_range` and `par_phases` run the body inline on the caller when the pool has one worker (one load of the worker count per call, no scope, no task). Red-first: a W = 1 physics step constructs 0 scopes (UG-03) |
...
| **KC-23** | Change surfaces: structural log (EK3, 2-frame horizon, exact overflow), dense tick API (EK6, refuses group/untracked ids), group edit log (EK6g, opt-in ~~`EDIT_LOG`~~ `type Edits = Logged`; ⚠ *2026-09-24, EP4 W2′ and EP5 O2, O3: a `Logged` group's columns are written only through `LoggedColumnMut`, which marks; `DenseColumnMut` and `GroupHead::view` refuse `Logged` groups with E0271; one system holds at most one log-node param per `Logged` group (B0002); the binder marks a newly anchored slot; engine P4.2-EK6g and closure C-3, C-4; physics E4-1, E4-3*), KF-25 accessors | engine ED17 + P-ED17, P-§9 | 0 when unread (opt-in per type) | hook queues (KF5 dropped); KF-25 (3) |
