...
|---|---|---|---|
| Ledger in-scope rows outside an ECS / kernel form | ~~2357 active, 691 (29.3 %) in ECS form, 1102 out of scope (`[M]docs/memory/RUNTIME-DATA-LEDGER.md:30`)~~ ⚠ *2026-09-24, B1 PC-b, PC-o (ledger rev 5, `RUNTIME-DATA-LEDGER.md:2370-2373`): 2829 active, 887 (31.4 %) in an ECS form, 1159 out of scope; the pin is `ACTIVE_STD_HEAP_ROWS` 1660. The struck 2357 was rev 4's total of all rows, not a std-heap count* | the count only decreases; end state 0 | UG-02 |
...
| `pool.scope` sites in physics src | ~~4 (`:90`)~~ ⚠ *2026-09-24, B1 PC-d: 5 `pool.scope` and 5 `try_with_active_pool` on the trunk, both pinned by UG-02's physics slice (`docs/memory/ledger/ug02-pins.tsv`)* | 0 | UG-02 physics slice |
| Host frame-loop World writes outside systems | ~~7 (`[M]docs/unification/ENGINE-RUNTIME-ECS-DESIGN.md:120`)~~ ⚠ *2026-09-24, B2 PC1: 9 steps / 18 sites, pinned by G-LOOP at B2 (`crates/boyko_app/tests/g_loop_runner_world_writes.rs`); F9 and F14 already wrote the World at `[J]`* | 0 | UG-14 |
| Exclusive UI systems | 13 ⚠ *2026-09-24, B2 (`b2/cut.md` addendum O7): a crate-wide count, the `pub fn …(world: &mut EcsMaster)` systems in `boyko_ui/src`; app (b) schedules 8 of them* | 1 with `UiPlugins`, 0 without (`:1839`) ⚠ *`UiPlugins` lands at UI2 (B2 PC3)* | UG-13 |
...
| Linked `boyko_demo` `.text` / `.rodata` per kernel rung | ~~not recorded~~ ⚠ *2026-09-24, B2: baseline `.text` 8540450, `.rdata` 3389028, Total 12463130 (`docs/measurements/ug16-linked-size.tsv`). By orchestrator ruling on B2 PC4, a second release image, `boyko-app --example playground`, is recorded before the first D rung, because `boyko_demo` links no render, physics or UI crate (03 §1 note); rung B2b records it, and the band holds per image (02 §2, Phase B)* | recorded; +1 % budget per rung, above that a written reason | UG-16 |
...
|---|---|---|---|
| U-1 | **The allocator's Heap class** (`Heap`, `HeapRef`, `HeapVec`, `HeapBox`, `HeapDyn`, `SortedMap`) **is deferred, not built.** Its clients take ledger forms: K7 spans, the erased record column, VmColumn tables, and sorted ScratchColumn pairs. | Per-frame cost equal (build-time clients). Heap: eager Miri 1.26 MiB per `EcsMaster`, VA 2 GiB per master, worst resident 204 KiB (`ALLOCATOR-DESIGN-SPACE.md:3171-3172, 2718`), plus new TB-sensitive unsafe surface. Forms: 4 KiB floor per column after D-M1, no new unsafe. | Ledger rev 5 finds a row that needs individually-freed, variable-size, non-`Copy` storage owned by `schedule`/`registry`/`master-table` and not expressible as KC-15/17/18 ⚠ *2026-09-24, B1 PC-p (`b1/fix.md:154-162`): rev 5 cannot find one by construction. Its 440 new in-scope rows inherited their form from neighbouring rev-4 rows, and the ledger's owner vocabulary has no `registry` or `master-table` value (kernel rows read `none`), so the condition cannot be screened mechanically; `b1/fix/u1_candidates.tsv` lists the 18 new `schedule`-owned in-scope rows as a first pass, not an evaluation. The detector moves to each retiring rung's cut: a rung that re-forms a row applies this condition to it, as the recount rule already makes it re-read the row (02 §6)*; or modding Stage 3 needs mod-private freed memory (then the heap lives in `boyko_mod_host`) |
...
| Document | Patch |
|---|---|
| `docs/memory/ALLOCATOR-DESIGN-SPACE.md`: closed at rev 2.4 by orchestrator ruling. AP6 ran with 0 Critical; its W1–W5 ~~are open~~ were open (`:5-10`, `:3873-3881`). ⚠ **Status (2026-09-23):** DOC-1 is done: rev 2.5 gives every AP6 remark a disposition. AP7 ran and returned CHANGES_REQUESTED (0 Critical, 2 Important, 5 Optional). Rev 2.6 (same day, branch `u/doc-1-2`) resolves W1 and W2 and adopts O1–O5; no owner ruling is needed. Open for this plan: the mechanism of the modding-crate ~~`#[used]`/ctor ban~~ ban on runtime-invoked code (03 `:192`; allocator ~~P53.3~~ P56 with P58). AP7's closure, which C1 waits for (02 §2), is the orchestrator's to declare. ⚠ *2026-09-24: AP8, a closure pass (0 Critical, 1 Important, 3 Optional), found every AP7 remark and answer resolved and C1's prerequisite met. Rev 2.7 (branch `u/doc-3-4`) resolves N-W1 by scoping the ban to its harm (runtime-invoked code, which excludes MS-15's `.boykom` statics and K-MOD-9's thunks) and adopts O1–O3. AP9, a closure pass on rev 2.7 (0 Critical, 4 Important), found that the controls could not reach their reasons (W1), that R2 was one name (W2), that leg (6)'s feature clause was met by the baseline (W3) and that a procedural SDK macro went unread (W4); the rev-2.7 closure (P58) resolves all four. The plan side lands at 03 `:16`, `:24`, `:177`, `:187` and `:192`, and 05 `:60`. The ban's mechanism holds only the first modding-crate rung; C1 is not held.* | **Step DOC-1 (02 §2) writes rev 2.5. AP7 reviews only the rev-2.5 delta and the AP6 dispositions; AP6 already reviewed rev 2.4.**<br>**Revival forms.** U-1, U-7, U-8 and U-9 move Heap, TableSet, DropColumn and HeapDyn to revival form. The ByteColumn, ZeroInit, ChunkArena/SlotChunks, injector, 1f and G-ladder content stays. P39 and P40 stay as AP6 reviewed them.<br>**§7** is re-pointed to file 05: K-MOD-3 and K-MOD-10 are removed; G6 = UG-15. The duplicate "(f)" row (AP6 W1) is relabelled and pointed to UG-15 leg (7), which sees kernel survivors that the two-arm row cannot. Leg (6) keeps only the feature-unification role.<br>**§2.0.** The thread-context placement moves from `boyko_memory` to `boyko_threadpool` (U-19). This keeps G5's `#![no_std]` for `boyko_memory` (`:337` at `b716a5dc`) intact.<br>**P29.** `HeapRef::alloc_cold` is struck from P29's pinned set (`:2619-2623` at `b716a5dc`), because U-1 never builds it (critic pass 5, W1 (a)); four symbols remain.<br>**AP6 dispositions** (02 §2, Document steps):<br>• W2 → D-S1(i)'s isolated tests.<br>• W3 → void under U-1; P41's gate is not built, and the remark is carried with the revival form.<br>• W4 → 05 §5 (row AP6-W4).<br>• W5 → B1.<br>• O1's stale passages are fixed.<br>• O2 → void under U-2.<br>• Question 4 → D-S1(i)'s cut.<br>• Question 5 → void under U-10. |
| `docs/physics/PHYSICS-ECS-UNIFICATION-DESIGN.md` (closed rev 5) ⚠ **Status (2026-09-23):** DOC-2 wrote Erratum E2. EP3 raised no Critical or Important remark against it, and Erratum E3 adopts its two Optional ones (O1: `open_chain`/`close_chain` bounded to `Chained`; O3: E2-7's heading). ⚠ **Status (2026-09-24):** EP4 found E3 resolved. Erratum E4 (DOC-3) bounds `DenseColumnMut` and `GroupHead::view` to `Unlogged` groups (K3's edit policy, from EP4 W2′); it lands in D-E7. EP5 found E4-1's `DenseColumn` sentence ungated and in conflict with `SolverBodies` (W1). E4's closure makes `DenseColumn` read-only by structure, with a census red-first at D-S3(ii); gives `SolverBodies` a `&[BodyInertia]` (E4-2, for U5); and adds the binder's mark for `Logged` groups (E4-3, D-E7) | **Step DOC-2, reviewed by EP3.** Erratum E2: `Release` is an associated type with markers `Immediate` / `Chained` / `Stamped` (U-3, 01 KC-12); U1 deletes the scratch band (U-2); NB2 as U-17; N5 span freeing inside release points; **X-14 corrected** (`:1267`): "Command order, hook order and dense slots handed out in one window depend on completion-pop order (fixed by KC-36). Entity ids do not depend on the window: `Commands::spawn` claims them on the worker during the phase (`commands.rs:169-173`), so they depend on claim interleaving whatever the apply order (U-20)." |
| `docs/unification/ENGINE-RUNTIME-ECS-DESIGN.md` (rev 3, ~~pass 3 never ran~~) ⚠ **Status (2026-09-23):** DOC-2 wrote rev 4. EP3 ran and returned CHANGES_REQUESTED (1 Critical, 4 Important, 5 Optional). Rev 4.1 (same day, branch `u/doc-1-2`) resolves C1 (`DESPAWN_AT_ZERO` gains the pin edge), W1 (the redirect's observable), W2 (the sealed asset-group form), W3 (`clear()` refused while an `AssetSentinel` lives) and W4 (G-ALLOC = UG-03 scenes E1 + E1v). It adopts O1–O5 and patches 02 and 03 on the same lines; no owner ruling is needed. By EP3's own map, none of its remarks touches D-S3(iii). D-E2, AS2, AS5 and HO2 build on the rev-4.1 resolutions, which a re-review scoped to that delta can confirm. D-E20–D-E23 are plan-sourced (KC-37) and, like D-E0, D-E18 and D-E19, do not wait for EP3 (EP3 Q1; 02 `:590`). ⚠ **Status (2026-09-24):** EP4, a closure review of rev 4.1, returned CHANGES_REQUESTED (0 Critical, 2 Important, 4 Optional). Rev 4.2 (step DOC-3, branch `u/doc-3-4`) resolves W2′ (a kernel edit policy, where `LoggedColumnMut` is a `Logged` group's only write route, and private asset-column newtypes) and N1 (the D-E2 witness uses observers), and adopts O-a–O-d; physics Erratum E4 carries the K3 half. EP4 released D-S3(iii), D-E1, D-E3–D-E6 and D-E8–D-E19. EP5, a closure review of rev 4.2 (0 Critical, 1 Important, 4 Optional), released D-E2, and AS2 on content, and held D-E7 on W1 (`DenseColumn`'s read-only property). The rev-4.2 closure resolves W1 (C-1: read-only by structure, with a census at D-S3(ii)) and adopts O1–O4; the binder's mark closes O-14. Releasing D-E7 on the closure is the orchestrator's call | **Step DOC-2, reviewed by EP3** (scope: the rev-3 patch, rev 4 and physics erratum E2). Rev 4: K6′ against physics rev 5 (U-3); registry-free EK1 (U-2); EK15b's redirect enqueues the `Pinned` removal instead of performing it (02 §4.4); prerequisites remapped to plan rung ids (02 §6). **EP3 must close before D-S3(iii), AS2 and every engine-sourced D-E rung.** D-E0 (physics P-§14), D-E18 and D-E19 (ledger KF-10, KF-09) are not engine-sourced and do not wait for EP3 (02 §2) |
| `docs/memory/RUNTIME-DATA-LEDGER.md` | rev 5 (B1): re-point to the trunk; KF-33/34 status per U-4/U-5; PC1 closed per U-1; ScopeShared per U-6; PC5 closed. **KF-45 (`:1624-1640`):** `:1628` currently reads "One VmReservation-backed column of 64-B records … Reached by slot through the platform thread-control-block pointer (an id -> slot table)". Rev 5 restates it as "one fixed `.bss` static table of 8192 64-B records and a 128-word busy bitmap, reached through a per-thread OS word that holds the record's index + 1 (U-19; 01 §6)". The precedent is KF-43's static-table form (`:1593-1605`); U-19 (d) is the owner's overturn. Per-row routes follow 01 §6 item 10: 9 record, 1 unchanged `thread_local!` (`LANE`, out of scope with its reason: diag may depend on nothing, and the cell is const-initialised and `Drop`-free), 2 OS thread id. The row's home moves from the memory library (`:1718`) to `boyko_threadpool`. ~~The four remaining key and guard `thread_local!` cells are rowed as out of scope, each with its reason.~~ ⚠ *2026-09-24, reconcile: B1 PC-f, as 02 D-M6's note reads (02 `:158`): only `LANE` of the four key/guard cells exists on the trunk, and it is ruled out of scope (U-19). `EXIT_GUARD` and threadpool's `WORD` are created by D-M6, and `TOKEN_ANCHOR` becomes `cfg(miri)`-only there, so D-M6's own UG-02 re-pin rows them: UG-02 cannot hold a row with no site.* KF-02 (`:898`): the `boyko_rhi_vulkan/src/memory.rs:728` row moves to D-E15, and the `boyko_utils/src/sparse_map/sparse_map.rs:10` row to D-R2d (02 §2) |
...
| `docs/physics/PHYSICS-ECS-UNIFICATION-DESIGN.md` | DOC-2, erratum E2, gains two items. (1) The row-identity map matches the full `Entity`, so the generation is included (A1b, H-03). (2) Until U7, the solve order follows archetype-row order (H-16). ⚠ *2026-09-23 (EP3 O3; physics Erratum E3-2): read as "rows drive the solve through U4; U5 moves S2–S6 to slots; until U7, per-body state is still carried by row". Both items are done in E2 (E2-6, E2-7).* Replays do not depend on U7 for this. Rule B's move clause (U-28) forbids Main-side moves of keyed entities, and the replay's move digest detects a breach, so UG-22 has no arm deferred to U7. |
| `docs/unification/ENGINE-RUNTIME-ECS-DESIGN.md` | DOC-2 scope gains EK7's third policy, `#[event(swap = "every_tick")]` (KC-37 (d), D-E8), and D-E20's writer lanes (EK7's lanes are keyed by writer, not worker). It also gains D-E23's move of `FixedTime`'s frame-level getters into `Time` (U-25): the engine design's interpolation readers read `Time::fixed_overstep_fraction()`. ⚠ *Done in rev 4 (2026-09-23). Rev 4.1 widens `every_tick`'s reader refusal to every non-Fixed schedule (EP3 O2) and adds a D-E23 caller (EP3 O4).* |
...
| RK-2 | The engine design's rev 3 was never re-critiqued, and it was filed against physics rev 3 | `[M]CHECKPOINT-2026-09-11.md:55`; physics rev 4/5 changed K6 (`PHYSICS-ECS-UNIFICATION-DESIGN.md:2449-2463, 3330-3343`) | DOC-2 writes engine rev 4 and physics erratum E2, and EP3 (engine critique pass 3) reviews them. **EP3 must close before D-S3(iii), AS2 and every engine-sourced D-E rung.** D-E0, D-E18 and D-E19 have non-engine sources and start when their prerequisites land (02 §2). ⚠ *2026-09-24 (EP4 O-d): so do D-E20–D-E23, which are plan-sourced (KC-37; EP3 Q1; engine P4.1-§12).* |
| RK-3 | Page-cache bit flips on a workstation without ECC | `CHECKPOINT-2026-09-11.md:236-238` | Owner memory test (Q-6); every red is re-run once before triage; receipts record the target dir. ⚠ *2026-09-24, reconcile: plus 01 §2.1's rule (01 `:383`): a single unreproduced mismatch on this host is re-run on a clean build before it is believed; a matching re-run does not close it, and it is recorded with its binary, cell and hashes and stays open under class (g); while the recorder R2 is unbuilt, the second recorded unreproduced mismatch on any engine binary triggers R2 and R3, which the physics campaign builds before the next timed window* |
...
| RK-12 | Id budget after the UI and reflect merges plus the new group columns | 512 slots (`[J]…/component_registry/mod.rs:63`) | AL:M-A13 / MD:M-K3 census (structural) at B2, after D-S3(ii) and after D-S3(iii). ⚠ *2026-09-24, B2 PC6 (`b2/cut.md:736-739`; `b2/impl.md`): M-K3's "shipped demo" points assume an `App`, and `boyko_demo` is `EcsMaster` plus `SimRunner`. B2 took the census on app (b), the playground composition and a replica of the demo world, with a syn drift guard on `crates/boyko_demo/src/app.rs`. Steady `NEXT_ID`: app (b) 65, playground 51, demo 7; the derived U is 73. On the trunk the playground pins ~~72 ids, the lowest at 407~~ 80 ids, the lowest at 406: L9-C3's new physics scratch columns took 8 more slots, and its re-read at `aef7dda4` replaced B2's playground rows, while `NEXT_ID` and U did not move (`docs/measurements/rk12-id-census.tsv`, lines 19–22 and 27, and its header line 11, which keeps B2's 72 / 407 from `4d75a4e3`)* |
...
⚠ *2026-09-24, the Phase B document step: two rows above ~~still read as written but~~ are superseded in part, because their lines abut lines that `u/doc-3-4` (`ad262579`) rewrites. §5's ledger row (`:157`, "the four remaining key and guard cells are rowed") reads as 02 D-M6's note: only `LANE` exists, and D-M6 rows the other three. RK-3's mitigation (`:172`) gains 01 §2.1's rule: a re-run on a clean build, a record that stays open under an in-process class, and the trigger that builds the recorder R2. Both are struck or reworded at that merge (02 §2's reconcile list, after the Phase B table).* ⚠ *2026-09-24, doc fix-up O1: done at the merge of `u/doc-phase-b` into `integ/unified` (`68437dfe`): `:157` is struck and `:172` gains 01 §2.1's rule (01 `:383`), each with a reconcile marker.*
...
- [Rust 1.77.1 announcement](https://blog.rust-lang.org/2024/03/28/Rust-1.77.1/) (rev 5): that
  behaviour is disabled on Windows msvc targets (03 §6 leg (7)). ⚠ *2026-09-24, B3 PC-20: measured on the
  gate host, cargo 1.98.1 passes `-C strip=debuginfo` under `release` on msvc (B3's receipts' final rustc
  lines), while the link still carries `/DEBUG`. With `seam-census` reduced to `inherits = "release"` (PC-18)
  this has no gate consequence.*
...
- [LLVM, `llvm-symbolizer`](https://llvm.org/docs/CommandGuide/llvm-symbolizer.html). It reads PDB (`--pdb`; a native reader, or DIA with `--dia`) and prints inlined frames by default. ⚠ *2026-09-24, B3 PC-24: the MSVC Build Tools copy the gate uses (LLVM 21.0.0git) has no `--pdb` option; it reads the PDB through the image's CodeView record, and B3's probe (iii) resolved the inlined frame that way.*
