...
|---|---|---|---|---|---|---|
| B1 ⚠ *merged 2026-09-23 (`6dd1f916`)* | Ledger rev 5 (three trees → trunk) + UG-02. It carries AP6 W5: KF-33 (`RUNTIME-DATA-LEDGER.md:1446-1457`), KF-34 (`:1459-1470`), `ledger/pool-utils-log.md:18` and the order-of-work row (`:1783`) are restated to allocator P2/P10/P16/P17 and to rungs D-M2/D-M3, and the ledger names the design as authoritative for mechanism and itself for row inventory. ⚠ *2026-09-24, B1 (`b1/fix.md` "Plan corrections"; ledger rev 5 `RUNTIME-DATA-LEDGER.md:2368-2410`, `:2600-2625`, `:2667-2677`): the per-rung row counts it re-derives land in the D-M2, D-M3, D-M4, D-M6, D-S6, D-E15 and R2-sweep rows, §4.2 (PC-k) and §6 (PC-l); U-1's detector is 00 §3's (PC-p); UG-02's text is 03 `:11`* | A8 | ~~pins 2357 active (after the rev-5 recount)~~ ⚠ *2026-09-24, B1 PC-b: 2357 was rev 4's total of all rows, and rev 4's `ACTIVE_STD_HEAP_ROWS` was 1245. Rev 5 pins `ACTIVE_STD_HEAP_ROWS` 1660, `TOTAL_ACTIVE_ROWS` 2829, `SEMANTIC_ROWS` 39 and `OUT_OF_SCOPE_ROWS` 1159 (`docs/memory/ledger/ug02-pins.tsv`, its one line; ledger `:2370-2373`)* | ~~P6's seven hole fixtures~~ ⚠ *B1 PC-a: P6 names five W4 holes (`ALLOCATOR-DESIGN-SPACE.md:1041-1045`), and "seven shapes" is P7's G4 red-first set (`:1063`). B1 landed P6's hole fixtures F1–F7 and the shape fixtures K1–K14, N1–N5, P1 and O6, and its controls (`tests/runtime_data_ledger_gate.rs`, 49 tests; `b1/impl.md`, `b1/fix.md`)*; an extra site → red; a missing site → red; an empty scan dir → red on the floor | UG-02 | M |
| B2 ⚠ *merged 2026-09-23 (`0e396f48`)* | G-LOOP, G-GRAPH (a)(b), G-RES, UG-03 re-pinned on the trunk (merges add systems), and UG-03's engine scene E1 lands (G-ALLOC, headless; ⚠ *2026-09-23, EP3 W4; 03 UG-03*), UG-16 baseline, AL:M-A13 / MD:M-K3 id census. ⚠ *2026-09-24, B2 (`b2/cut.md:705-742`, `:846-858`; `b2/impl.md`; `b2/fix.md`): G-LOOP pins 9 steps / 18 sites, not 7 (PC1); G-GRAPH landed **Tier A only**, the per-app Main/Fixed name inventories ((a) Main 35 / Fixed 1, (b) Main 47 / Fixed 1; `--features hwrt` adds one Main system to each and is pinned too, PC9). Tier B (the exclusive set, edges, one writer per column) needs a kernel read surface, which the orchestrator rules is D-E22's visitor extended, not a separate seam (PC2; D-E22 row). E1 is `crates/boyko_app/tests/e1_engine_ui_alloc_census.rs`, pinned exactly: scope 2, chunk 2, dispatch MAX 5, OTHER 0, EPOCH 0 per steady frame (PC5); no UG-03 pin moved, and this was the trunk's first reading of S1c release 134 / 134 / 269 (PC7). UG-16's baseline is `boyko_demo`'s `.text` 8540450, `.rdata` 3389028, Total 12463130 (`docs/measurements/ug16-linked-size.tsv`); by orchestrator ruling a second release image, the playground, is recorded before the first D rung (PC4; 03 §1 note), by B2b below. G-RES and the id census are record-only (`docs/measurements/ug20-g-res.tsv`, `rk12-id-census.tsv`; PC4a, PC6)* | A8 | — | add an exclusive system → G-GRAPH red; add a runner world write → G-LOOP red | UG-03, 13, 14, 16, 20 | M |
| B3 ⚠ *merged 2026-09-24 (`79005dfd`)* | **UG-15 pin capture on the trunk, on the msvc gate host (U-22).** It covers leg (7)'s section sizes and symbol multiset, and the sensitivity map's file, layout and containment maps (03 §6).<br>**Symbol subject: the post-LTO object**, built with `cargo rustc … -- --emit=obj`, because the msvc image has no symbol table (`[C]crates/profile_fixture/tests/profile_axis_census.rs:46-78`). B3 lands the dev-only crate `boyko_symcensus`: the object build with its pairing guard (`[C]:213-318`), and `llvm-nm`/`llvm-objdump` wrappers that are RED on an absent tool or an empty census (`[C]:90-97`, `:498-512`). The two existing copies of that instrument stay as they are.<br>**Profiles:** `bench-shipped`, `seam-census` and `sensitivity-map` are added to the root `Cargo.toml` (03 §5, §6).<br>**Probe build:** records (i) the anonymous-data forms the object shows; (ii) the object's symbol-size source (COMDAT section length, or distance to the next symbol); (iii) whether the MSVC Build Tools `llvm-symbolizer` (version printed; absent = RED) resolves the inlined `VmColumn::swap_remove` frame inside the `swap_remove/10k` body from the sensitivity-map PDB — if it does not, the file map comes from a `stable-x86_64-pc-windows-gnu` build of the same commit (03 §6); (iv) P6-1 on the object, and on the image with `-C link-arg=/Brepro`; (v) whether an uncalled `#[no_mangle]` kernel fn, and red control (viii)'s unreferenced static, survive fat LTO and `/OPT:REF` in the msvc image. A control that cannot go red on this host is redesigned at B3, never dropped.<br>**Pin list:** frozen at capture. A candidate body with no symbol in the object is recorded as absent, with its reason, and is not pinned. After capture, a pinned symbol that disappears is RED (P29; critic pass 5, W1).<br>**Leg (1)'s macro half:** a `proc_macro2` twin for every `boyko_macros` entry point, the parsers' `const` key table, and the expanded-corpus test.<br>**Other items:** the UG-08 recipe on the msvc nightly, including the joined-key trial (03 UG-08); the UG-09 recipe; the ignore-reason prefix migration (closed vocabulary).<br>⚠ *2026-09-24, B3 (`b3/cut.md` §0 PC-1–PC-17 and A.2–A.3 PC-18–PC-27; `i2.md`–`i4.md` PC-28–PC-38; `fix.md` PC-39–PC-41; `b3x/close.md` PC-39 amended, PC-42, PC-43; `b3x/r2/close2.md` PC-44). **Probe answers:** (i) v0 mangling only, and one anonymous form, `anon.<hash>.<n>`; (ii) a pin's size is its COMDAT section's `RawDataSize`, because `llvm-nm -S` prints zeros on COFF; (iii) PASS on route (a1), so no gnu build; (iv) objects are byte-identical across rebuilds, so `profile.release` is the pin profile and no deterministic profile is added, and `/Brepro` makes `boyko_demo`'s image identical but not `clear`'s; (v) an uncalled `#[no_mangle]` fn is a global in the object and is gone from the image under `/OPT:REF`, and red control (viii) as written cannot go red, so it takes the named form (b′) on `clear`. **Subjects:** `boyko_demo` and `boyko-app --example clear`, because the demo links no `boyko_scene`/`boyko_app` and never calls `EcsMaster::new` (PC-1, PC-21). **Pin list:** 42 frozen pins over five subjects and 6 recorded-absent pairs (`crates/boyko_symcensus/ug15/pins/`). **Recipes:** `crates/boyko_symcensus/ug15/RECIPES.md`; UG-08 takes the joined key (03 §1 note). **Ignore classes:** 11, `miri-unsupported` included (PC-10). `Cargo.lock` is tracked and CI builds `--locked` (PC-44). The other corrections are in 03 §1's note and §6, and in the C1, D-S1(ii) and UG-15-mode entries below* | A8 | — | UG-15 controls, each in its own branch: (i)–(viii) red, (ix)–(x) green (03 §6). (xi) and (xii) start at D-S1(ii) | UG-08, 09, 11, 15 | M |
| B4 ⚠ *merged 2026-09-23 (`3da1ea24`)* | Physics R0's census (`PHYSICS-ECS-UNIFICATION-DESIGN.md:789`). Take `tests/physics_vec_side_store_census.rs` from `ad0ebea4` (`D:/wt/ecsnative`), re-derive its pin on the trunk, and record the delta from 34 row by row. U6's "Vec 34 → 30" and S0's "census 30 → 0" are re-read as −4 and → 0 from the B4 pin. R0's timing is MQ-01; its SP-1 is MQ-02. ⚠ *2026-09-24, B4 (`b4/delta.md`, `b4/impl.md`): the pin is **34 on the trunk, delta 0**. IslandSleep's 4 and SoftBody's 30 fields have `ad0ebea4`'s names and types, and none is gone. The 4 new `Vec` fields (`broadphase_tree/tests.rs::Sim`, from `ecbfe416` and `c52ad183`) sit in a `#[cfg(test)] mod tests;` file, a module layout that `ad0ebea4`'s name rule (`*_tests.rs`) did not know. That is a layout change, not a side store: the census now skips a file by its declaring item's test cfg. So U6 is 34 → 30 (−4) and S0 is 30 → 0, as written, read from `PINNED_VEC_FIELDS = 34`* | A8 | — | an extra `Vec` side store → red; a removed one → red until the pin is lowered | UG-01, UG-18 | S |
| B2b ⚠ *2026-09-24, added by the Phase B document step's revision (critique W5), to carry the orchestrator's ruling on B2 PC4* | **UG-16's second image: the playground's baseline row.** A release build of `boyko-app --example playground` on the trunk after B3's merge, recorded with B2's script, which takes the image by path: `scripts/ug16_linked_size.py --image <target>/release/examples/playground.exe --record docs/measurements/ug16-linked-size-playground.tsv --rung B2b --baseline`, with `--note` carrying the final rustc line's `--cfg feature=…` set (B3 PC-40; 03 §1 note). Record-only, with no code change; it runs in the trunk worktree like a merge step, and the Phase B exit run may take it. No Phase D rung is cut before this row is on the trunk (§3) | B1–B3 | — | none: record-only, and the band applies from the first D rung | UG-16 | S |

⚠ *2026-09-24, the Phase B document step (branch `u/doc-phase-b`): **how its sources are cited.** Each correction carries its rung's plan-correction id (B1 PC-a…PC-r, N-a/N-b; B2 PC1…PC10; B3 PC-1…PC-44). The rungs' stage reports are not in the tree. Paths written `b1/…`, `b2/…`, `b3/…`, `b3x/…`, `b4/…` and `w1det/…` are in the orchestrator session `d9f1cf70`'s scratchpad, and `doc34/…` and `win6/…` in session `7dc37fc3`'s. The evidence that is in the tree is named beside them: the ledger, the pins and receipts under `crates/boyko_symcensus/ug15/`, `docs/measurements/`, and the gates themselves.*

⚠ *2026-09-24, the Phase B document step: **what to reconcile when `u/doc-3-4` and `u/win6` merge.** Where a correction's own line is one that `u/doc-3-4` (`ad262579`) rewrites, or sits next to one, the correction is a nearby note and the line still reads as written, because git conflicts on edits to adjacent lines. `ad262579` replaces lines one for one, so these numbers hold on the merged tree. At that merge each line below is struck or reworded to its correction:* ⚠ *2026-09-24, reconcile: applied in the merge of `u/doc-phase-b` into `integ/unified`, after `u/doc-3-4` and `u/win6`; each site carries a dated "reconcile" marker that points to the text superseding it.*
- *03 `:17` (UG-08): the joined key is adopted, and the physics Miri build fails under the recipe (03 §1 note). The "ISA baseline is dropped … takes the joined form" sentence is struck.*
- *03 `:23` (UG-14): "7 → 0" reads 9 steps / 18 sites → 0 (B2 PC1).*
- *03 `:24` (UG-15): `ad262579`'s control count stands beside B3's instrument controls, 42 pins and 6 recorded-absent pairs (03 §1 note).*
- *03 `:25` (UG-16) and `:53` (§2's UG-16 row): two images from B2b on, each banded against its own previous row (03 §1 note).*
- *03 `:177`, `:178`, `:188` and `:192` (§6 legs (1), (2), (7) and Controls): leg (7)'s `strip = "none"`, "The explicit `strip` matters …" and "legacy-mangling hash" are struck, and the four rows take the §6 note's text.*
- *00 `:157` (00 §5, the ledger row): "the four remaining key and guard cells are rowed" reads as the D-M6 row's note below.*
- *00 `:172` (RK-3): the mitigation gains 01 §2.1's rule: a re-run on a clean build, a record that stays open, and the trigger for R2.*
- *The D-S3(ii) row's parenthesis (`ad262579`'s text), engine C-1 (r1) (`ENGINE-RUNTIME-ECS-DESIGN.md:4954` on `u/doc-3-4`) and the EP5 table's "no impl of it names …" (`:4787` there) take the EP6 O1 note's wording, below the Lane STORE table.*
- *The D-E7 row (`ad262579`'s text): (g4)–(g6) and the binder's route take the EP6 note below the Lane ENG table.*
- *`u/doc-3-4`'s design files: engine `:120` (G-LOOP "7") takes B2 PC1's 9 steps / 18 sites, and physics `PHYSICS-ECS-UNIFICATION-DESIGN.md:90` and `:733` (`pool.scope` "4") take B1 PC-d's 5 `pool.scope` + 5 `try_with_active_pool`.*
- *`docs/physics/perf-campaign/levers/00-RULINGS.md`: `u/win6`'s "G9's remainder on C3" says "G9 is not formally closed … whether K=12 is required is an open ruling". This branch's "G9 on C3 CLOSED by ruling" answers it, and that sentence is struck to a pointer.*
- *Not a merge item: B1 N-b in the ledger's "What the scanner does not see" (`RUNTIME-DATA-LEDGER.md:2506`) goes to the next ledger revision (03 UG-02).*

⚠ *2026-09-24, the same step: **B3's line moves and the citations outside UG-10.** B3's merge counted 198 explicit `path.rs:N` citations in 26 ungated documents that point at a trunk line B3 moved, and handed them to this step (`b3x/anchors.md:59-62`; `b3x/mg/ungated-moved.txt`). Disposition:*
- *The 10 in this plan (00 ×4, 01 ×2, 02 ×2, 05 ×2) all cite `boyko_macros` at `d552be05`: each carries `[J]`/`[Jw]`, or sits in a verification block that names `[J]` at `d552be05` (00 `:397` and `:1077`, in the rev-6 and rev-5.1 re-verification blocks). B3 cannot move a line of a fixed commit, so they stand. The rows that carry two of them, D-S1(i) and D-E21, re-derive their citations at their cut, as every rung does.*
- *151 are in the ledger files (`macros-aether` 122, `RUNTIME-DATA-LEDGER.md` 10, `render` 8, `reflect-lane` 7, `ecs-services` 4). Each file's header pins its rows to `c1e9f1db`, so they are commit-tagged. The next ledger revision that re-pins UG-02 re-points them.*
- *The other 37 are in 17 other documents: modding 5, physics 4, engine 4, profiling 3, and 1–2 in each of the rest. Each goes to its document's next revision. The allocator, engine and physics designs are under revision on `u/doc-3-4`.*
...
| C1 | KC-01: move `vm.rs`, `vm_column.rs`, `utils.rs` and the granule constants to `boyko_memory`; re-exports; `raw::commit_at<O>`; UG-04 counter | D-M0; AP7 has closed rev 2.5 | u: KF-32 (9) | a commit from a system body → `commit_delta` red; setup window must be non-zero | UG-01, 04, 10, 15 ⚠ *2026-09-24, B3 PC-12, PC-34: C1 widens leg (1)'s scope to `boyko_memory`, which is a gate edit, so the controls re-run, and red control (iv) moves from `boyko_utils`, where B3 ran it because `boyko_memory` does not exist before C1, to `boyko_memory`* | M | `ecs/memory/{vm,vm_column,utils}.rs`, `constants.rs` |
...
| D-M2 | KC-05 + KC-06a (allocator 1a + 1b as one commit, then 1f) | D-M1, A2 | ~~KF-33 (4) + "ScopeShared placement" (10) = 14~~ 10 (R5) ⚠ *2026-09-24, B1 PC-m: KF-33's 4 rows are among the 10 ScopeShared-placement rows, so D-M2 retires 10 (ledger rev 5, `RUNTIME-DATA-LEDGER.md:2617`)* | C2 Miri `miri_scope_slot_identity.rs` (`ALLOCATOR-DESIGN-SPACE.md:899-903`); P3 poison-write protector gate re-blessed (zero `KE16-PROTECTOR-GATE-ARMED` lines); mem-shake proven to fire; exhaustion degrades (`D = 1`) | UG-01; UG-03 (App scenes scope/chunk/OTHER → 0; AL:M-A10 `chunk_bytes_resident` pinned); UG-04; UG-08; UG-09 | M |
| D-M3 | KC-06b injector ring | D-M2 | ~~KF-34 (4)~~ 2 ⚠ *2026-09-24, B1 PC-e, PC-q (ledger `:2618`, `:2624`, `:2672-2674`): under U-5, D-M3 retires the injector's 2 rows, the field and, new in rev 5, its constructor (`thread_pool.rs:769`). The `Stealer` array is KF-31's (D-M4). The lane `Worker`, at its construction and as `worker_main`'s parameter, and the epoch `Local` are 3 rows ruled out of scope under U-5* | loom: push/steal, steal/steal, overflow-spin with park (`LANE_CAP = 2`) | UG-03 (injector → 0, spin counter 0); **UG-05 lands** (first scene at 0); UG-09 | M |
| D-M4 | KC-07 pool ownership | D-M3 | KF-31 ~~(19)~~ (25) ⚠ *2026-09-24, B1 PC-n (ledger `:2609-2611`, `:2619`, `:2675`): 18 rev-4 rows (`boyko_demo/src/app.rs:266` is an `Arc::clone`, a non_row at rev 5) + 7 new rows of the same data. Two rows hold the same `Arc<ThreadPool>` datum under other destinations, `App::with_pool` and `Schedule::run`'s `pool_arc`; D-M4's cut decides them (`b1/fix.md`, known limitations)* | `Schedule` holds no `Arc` (compile); teardown-order test | UG-01, 02, 15 (`size_of` re-bless recorded) | M |
...
| D-S1(ii) | KC-19b (rev 5.1), the modding Stage-1 seam: `ModSeam` and MS-03's four readers (01 §2; 05 MS-03) | D-S1(i) | 0 | S-1's shape check (leg (5)); red control (xi); green control (xii) (05 §6). The sized-name rows move to MS-02b's Stage-3 rung (RM-3) | UG-01, UG-10 (`tags.rs` again), UG-19; **UG-15 strict, parent = its cut commit**, which contains D-S1(i) and may also contain D-S3(i) or D-E1 (first cut wins on the registry files, §4.3). Legs (1)–(5), (7) and (7b) must be identical; leg (6) is N/A until a modding crate exists. Red controls (i)–(viii) and (xi) must each be red, and green controls (ix), (x) and (xii) green, each in its own branch (03 §6). Leg (7) is the check that no KC-19b item survives in the linked `boyko_demo`: `id_space_census()` is reachable from engine code from D-S1(i) on, so a KC-19b item that it comes to call would show up there. If a leg moves, the moving item leaves the kernel for `boyko_mod_host` (modding R1, `MODDING-DESIGN-SPACE.md:1818-1823`) and D-S1(ii) re-lands without it. ⚠ *2026-09-24, B3 PC-25 (`b3/cut.md` A.3, probe (v)): adding non-generic code to `boyko_ecs` moves `boyko_demo`'s leg (7)(a) even when nothing added reaches the image (`.text` −16 B, `.reloc` +4 B). The generic-only readers must leave (7)(a) identical, and this rung's own run shows it. A control that relies on `EcsMaster::new` runs on `clear`, because `boyko_demo` never calls it (PC-21, PC-34)* | S |
...
| D-S6 | KC-16, plus the ~~7~~ 17 KF-02 rows (⚠ *2026-09-24, B1 PC-q; see Rows*) in `boyko_ecs` and `boyko_render`. **Touch set:** `component/scratch/**` (the typed wrapper), `component/component_pool_bundle.rs`, `component/dense/dense_registry.rs`, `component/enable/enable_store.rs`, `ecs_master/ecs_master.rs`, `asset/staging.rs`, `boyko_render/src/retired_gpu_buffers.rs` | D-S2 (the `drop_fn` constructor) | KF-02 ~~(7)~~ (17); the rhi row goes to D-E15, the `sparse_map` row to D-R2d ⚠ *2026-09-24, B1 PC-q (ledger `:2607-2611`, `:2621-2623`): 7 rev-4 rows + 10 new rows of the same data, all in files this touch set already names (`staging.rs:63`; `component_pool_bundle.rs:27`, `:35`; `dense_registry.rs:100`, `:101`, `:103`; `enable_store.rs:600`, `:615`, `:806`, `:867`). D-E15's KF-02 rows are 2 and D-R2d's 3* | drop counts on truncate / `take_at` / clear / drop; `NEXT_ID` unchanged across 1000 `OwnedColumn` constructions (0 ids) | UG-01, 02, 04, 08, 12 (the render row), 15 (attributed: mode table) | M |
...
⚠ *2026-09-24, EP6 O1 (`doc34/ep6.md:55-61`): **D-S3(ii)'s `DenseColumn` read-only census, stated as its rules read.** The D-S3(ii) row's parenthesis that `u/doc-3-4` adds (`ad262579`) summarises the census as "no impl of it names `*mut`, `&mut` or `TypedDenseView`". No `SystemParam` impl can pass that, because the census reads the `SystemParam` impl too, and that impl must spell `&mut` in `init_state`, `init_access` and `get_param`. The census this rung builds is the engine's rev-4.2 closure C-1 with one change, to (r1): (r0) `DenseColumn`'s only non-zero-sized field is a `&'w [T]`; (r1) no fn of it, inherent `pub` or in any trait impl, returns a type that contains `*mut`, `&mut`, `TypedDenseView`, `NonNull`, `Cell` or `UnsafeCell` (C-1 says "`pub` fn", which a trait impl's fns never spell); (r2) it implements none of `DerefMut`, `AsMut`, `BorrowMut` and `IndexMut`; (r3) none of its impl blocks names `from_raw_parts_mut`, `as_mut_ptr`, `cast_mut` or `*mut`. A `&mut` parameter is not refused. ~~The row's parenthesis reads as this; it is not edited here because `ad262579` rewrites that line, and the engine text it mirrors (engine C-1, and the EP5 table's "no impl of it names …") is on `u/doc-3-4` only.~~* ⚠ *2026-09-24, doc fix-up O1: applied at the merge of `u/doc-phase-b` into `integ/unified` (`68437dfe`). The row's parenthesis (`:168`), engine C-1's (r1) (`ENGINE-RUNTIME-ECS-DESIGN.md:4954`) and the EP5 table (`:4787`) carry reconcile markers, and this fix-up marks physics E4-2's restatement (`PHYSICS-ECS-UNIFICATION-DESIGN.md:4285`).*
...
| D-E7 | KC-23 EK6 + EK6g (K-EK6g gate). ⚠ *2026-09-24 (EP4 W2′ and EP5; engine P4.2-EK6g and the rev-4.2 closure C-1–C-4; physics E4-1, E4-3): EK6g's opt-in is `type Edits: EditPolicy` (`Unlogged`/`Logged`) on `DenseGroup`, replacing `const EDIT_LOG`. `DenseColumnMut` and `GroupHead::view` are bounded to `Unlogged`, and `GroupEdits`/`GroupEditsMut` to `Logged` (E0271). `LoggedColumnMut<T>`, which marks on access and checks the entity's generation, is a `Logged` group's only write route; one system holds at most one log-node param per `Logged` group (B0002). The binder marks a `Logged` group's newly anchored slot (a `logged` byte on `DenseGroupStore`, read only in `anchor_transition`) ⚠ *2026-09-24, reconcile: EP6 O3, below the Lane ENG table (`:216`): the binder's route to group g's log is named at D-E7's cut, an array indexed by group beside `DenseRegistry::groups` or a handle in `DenseGroupStore`; an `Option` handle duplicates the `logged` byte, and the cut keeps one of the two*, which closes O-14. Red-first: UG-17 K1–K3 → E0271, and the green arm (g1)–(g6): `GroupHead<'_, LG>` scheduled with `live_count` and `release_dying_before` (g3), the recycled index (g5), and slot reuse (g6, whose mutation deletes the binder's mark) ⚠ *2026-09-24, reconcile: (g4)–(g6) as EP6 O2 recomposes them, below the Lane ENG table (`:215`): (g6) states and asserts its precondition, a window read after the release and before `e2` is anchored, at a tick above the install's tick, does not contain the slot; the "marks nothing" clauses of (g4) and (g5) are recomposed the same way*; the `DenseColumn` census re-runs. The touch set adds the file that defines `DenseGroup`, `system/params/**`, the binder and `boyko_macros` (`#[dense_group]`'s `edit_log` option).* | D-E6, D-S3(iii) | — | M |
...
| D-E15 | KC-34 KF-36 edge; then KF-02's `boyko_rhi_vulkan/src/memory.rs:728` row onto KC-16 | D-E14, D-S6 | u: KF-36 (44); KF-02 ~~(1)~~ (2) ⚠ *2026-09-24, B1 PC-q: + `memory.rs:737`, a new rev-5 row of the same datum (ledger `:2610`, `:2622`)* | M |
...
| D-E22 | **KC-37 (i): the generic access visitor.**<br>• `Schedule::for_each_system<V: FnMut(SystemView<'_>)>` and `App::for_each_system_access<V: FnMut(CoreSchedule, SystemView<'_>)>`.<br>• `SystemView { index, name, exclusive, access: &Access }`.<br>• Visit order is post-topological.<br>• Calling the `App` method before `finish` is a coded panic.<br>• **Why the rung exists:** today the systems are `pub(crate)` (`[Jw]…/schedule/schedule.rs:122`), and only `SystemMeta::access()` (`[Jw]…/system/system_meta.rs:255`) is public.<br>• **Lock set:** `schedule/schedule.rs`, `app/app.rs`, `boyko_log/src/codes.rs`.<br>⚠ *2026-09-24, orchestrator ruling on B2 PC2 (`b2/cut.md:709-723`): **UG-13's Tier B reads this visitor, extended; it is not a separate seam.** B2 landed G-GRAPH's name inventories only, because no public route reaches a built schedule's exclusivity, edges or sets. `SystemView` gains the system's ordering predecessors (post-topological indices) and its set membership. The red-first list gains UG-13 Tier B on apps (a) and (b): the exclusive set, the edges and sets, and one writer per resource column (03 UG-13). The lock set gains `schedule/schedule_builder.rs`. The set-membership table is the one datum `Schedule` does not keep today (PC2 item 5). D-E22 stays a strict seam commit, so its cut either reads membership from what the builder already keeps, or shows that the added table leaves leg (7) identical, and otherwise returns to the orchestrator. It must land before the first rung whose cell reads "G-GRAPH: … not exclusive": AS1, HO2, UI2 and HO4 (engine `:1059`, `:1061`, `:1073`, `:1075`), which gain D-E22 as a prerequisite (Phase E table; §3)* | B1–B3 | 0 | S |
...
⚠ *2026-09-24, EP6 (`doc34/ep6.md`), notes to the D-E7 row, which `u/doc-3-4` rewrites (`ad262579`), so they stand here. EP6 reviewed the engine rev-4.2 closure C-1–C-4 and physics E4-2/E4-3 and returned APPROVED, 0 Critical and 0 Important, with D-E7 released (`ep6.md:1-2`).*
- *O2 (`ep6.md:63-73`): **(g6)'s precondition.** The edit-log window is `[last_run, this_run]` inclusive, and marks are deduplicated per tick. So when the installing writer and the reader run in one schedule run, or under `run_system_once`, which never bumps the tick, the post-anchor window holds the slot whether or not the binder marks it, and (g6)'s mutation survives. (g6) states and asserts its precondition: a window read after the release and before `e2` is anchored, at a tick above the install's tick, does not contain the slot. The "marks nothing" clauses of (g4) and (g5) are recomposed the same way, because after C-4 the binder's own mark on `e2`'s slot makes them unobservable.*
- *O3 (`ep6.md:75-78`, `:85-86`): **the binder's route to group g's log** is stated at D-E7's cut. EK6g's log lives beside `DenseGroupStore`, not in it, and the erased `anchor_transition` walks `DenseRegistry::groups`. The cut names the structure: an array indexed by group beside `groups`, or a handle in `DenseGroupStore`. If the handle is an `Option`, the `logged` byte duplicates it, and the cut keeps one of the two.*
...
⚠ *2026-09-24, B3 PC-31 (`b3/i2.md:157`): the pinned "`Schedule::run` dispatch" is candidate H-3, located by the rule `Contains("bench_schedule_two_disjoint")`. In `phase9_scheduler`'s object that rule also pins 13 small `FunctionSystem<closure>` trait bodies (1–204 B) beside `Schedule::run`, and they move with any `FunctionSystem` codegen change. A rung above that names the `Schedule::run` dispatch names those 13 with it, and so does any rung that edits `FunctionSystem`. A narrower rule would edit the candidate list, which is a gate edit that re-runs the controls (03 §6).*
...
| Rung | Group | Rows |
|---|---|---|
| D-R2a | ecs-storage | ~~103~~ 170 |
| D-R2b | ecs-schedule | ~~146~~ 158 |
| D-R2c | ecs-services | ~~65~~ 89 |
| D-R2d | pool-utils-log ~~6~~ 11 + macros-aether ~~5~~ 8 + render 1 + ui-lane 5 + reflect-lane 2, plus KF-02's `sparse_map.rs:10` row onto KC-16 (needs D-S6) | ~~19 + 1~~ 27; KF-02's `sparse_map` rows are 3 at rev 5 |

⚠ *2026-09-24, B1 PC-i, PC-l: the counts are ledger rev 5's R2 column per group (`RUNTIME-DATA-LEDGER.md:2374-2390`, R2 total 444 = 170 + 158 + 89 + 27). KF-02's `sparse_map.rs` rows are counted by destination (`:2623`: the `SparseMap::dense` field, plus the new constructor rows in `SparseMap::new` and `SparseMap::with_capacity`, `runtime-data-ledger.tsv:2311`, `:2820`, `:2823`); D-R2d's cut settles whether they sit inside its 27. The recount rule below still applies.*
...
- **Retires R3** (~~80~~ 86 rows at rev 5, ⚠ *2026-09-24, B1 PC-l; `RUNTIME-DATA-LEDGER.md:2402`*), the interim `row_identity.rs` (7 rows,
  `RUNTIME-DATA-LEDGER.md:1880`), and defects A2 and warm-start.
- **Size.** U5 and P2 are L; the rest are M.
- **Census numbers.** U6's and S0's census numbers are read from B4's pin. ⚠ *2026-09-24, B4 (`b4/delta.md`): the pin is 34, delta 0, so U6 is 34 → 30 (−4, IslandSleep's four fields) and S0 is 30 → 0 (SoftBody's thirty). U6 deletes its row and lowers `PINNED_VEC_FIELDS` in one commit, because the census's const assertion pairs them; S0 also retires the census's non-empty anti-vacuity guard, deliberately (`b4/impl.md`, notes for U6/S0).*
...
| IN4 | D-E8 |
| HO1 | D-E9 |
| HO2 | D-E9, AS1, D-E22 ⚠ *2026-09-24, B2 PC2 ruling: G-GRAPH Tier B* |
| HO3 | IN1, D-E9 ⚠ *2026-09-24, B2 PC10: no production code writes `UiViewport` today (engine `:547`), so B2's app (b) tests insert it and name HO3 as its writer; HO3's deriver replaces that insert* |
| HO4 | D-E9, D-E22 ⚠ *2026-09-24, B2 PC2 ruling* |
| HO5 | HO4 |
| HO6 | D-E9 |
| RE1 | — (Q4 decided) |
| AS1 | ~~—~~ D-E22 ⚠ *2026-09-24, B2 PC2 ruling: its cell reads "G-GRAPH: the six are not exclusive" (engine `:1073`), which needs Tier B* |
| RE2 | AS1 |
| RE4 | D-E4, HO4 |
| RE5 | D-E5, D-E6 |
| RE6 | AS3, D-E7 |
...
| AS4 | D-E2, D-E1 |
| AS5 | AS4 |
| UI2 | A7, HO3, D-E22 ⚠ *2026-09-24, B2 PC2 ruling. UI2 lands `UiPlugins`, and E1 and G-GRAPH (b) re-derive then; until UI2 they run on today's UI plugin composition (B2 PC3)* |
| UI3 | D-E2, D-S2 |
| UI4 | D-E8 |
| UI5 | D-E3, D-E7 |
| UI6 | D-E5 |
...
These rungs retire R4 (~~156~~ 235 rows at rev 5, ⚠ *2026-09-24, B1 PC-l; `RUNTIME-DATA-LEDGER.md:2402`*).
...
- **Order.** After the physics lane's U7 and after the tree broadphase's own C4/C5 (which are lever commits on
  the physics campaign, not plan rungs); it does not block Phase F. ⚠ *2026-09-24: this plan does not track the
  physics levers (L5–L12, the tree broadphase's C-steps). Their rulings, window 6's included (L11's G9 closed under
  the window protocol, its three J-A cells at W 2/4/16, and the runner's `--bp-kernel` before the tree lane's F3
  window), are in `docs/physics/perf-campaign/levers/00-RULINGS.md`.* The refactor campaign F4 treats the feature's
...
| Rung | Scope |
|---|---|
| F1 | R6 (~~648~~ 860 rows at rev 5, ⚠ *2026-09-24, B1 PC-l; `RUNTIME-DATA-LEDGER.md:2402`*): diagnostics form, KF-09 / KF-10, B and X rows, by group |
...
B:      A8 → {B1, B2, B3, B4}                (pool order at A8: B3, B1, B2; B4 next; §4.1)
        {B1–B3} → B2b → every Phase D rung, D-M0 included   (⚠ 2026-09-24: UG-16's playground baseline, B2 PC4 ruling)
...
ENG:    B1–B3 → {D-E0, D-E20, D-E21, D-E22} ; {B1–B3, A5} → D-E23
        ...
        D-E22 → {AS1, HO2, HO4, UI2}   (⚠ 2026-09-24, B2 PC2 ruling: G-GRAPH Tier B reads D-E22's visitor; Phase E)
...
M:      Stage 1 = D-S1(ii) (+ MS-08, planned with A6; ⚠ 2026-09-24, B3 PC-5: its four MOD-SEAM doc lines landed with B3); Stage 3 after Phase-D exit (05 §7)
...
- **Anchor lines are not locked; re-deriving them is a merge step** (critic pass 4, W5).
  - The ~~four~~ UG-10 documents are `docs/FEATURE_MAP.md`, `docs/SYSTEMS.md`, `docs/ARCHITECTURE.md`
    ...
    enter no rung's touch set on account of anchors. ⚠ *2026-09-24, B1 PC-g: on the trunk `GATED_DOCS`
    holds 15 documents (`tests/internal_docs_anchors.rs:349-365`): these four, `docs/OPEN-QUESTIONS.md`,
    `docs/AETHER-GAIA-REVISION-2026-08-29.md`, four `docs/gaia/` files and five `docs/REFLECTION-*.md` files.
    The rule is the same for all 15: `GATED_DOCS` is the list, not this sentence.*
...
- **UG-02's pin history is append-only by the merge, not by the gate** (⚠ *2026-09-24, B1 PC-k;
  `docs/memory/ledger/ug02-pins.tsv:1-8`; `b1/fix.md` W1*).
  - Only the first pins line is anchored, in gate code (`GENESIS`). The FNV-1a chain catches a later
    line edited in place and not re-chained, but it is no anchor: the hash is public, and the failure
    message prints the repairing value.
  - So every merge that brings `ug02-pins.tsv` checks that `git diff --numstat <base> --
    docs/memory/ledger/ug02-pins.tsv` deletes 0 lines. That check is what protects lines 2..N.
...
| `ecs/core/schedule/schedule_builder.rs` ⚠ *2026-09-24, B2 PC2 ruling: it joined D-E22's lock set* | D-E22 (set membership for UG-13 Tier B) · D-R2b (the ecs-schedule sweep: the file holds `kernel-internal` ecs-schedule rows, `docs/memory/runtime-data-ledger.tsv`) · F1 (its `diagnostics` rows); no order among them changes behaviour, so the first cut wins. RF-K3 splits it in F4 |
...
| root `Cargo.toml` | A8 → B3 (the three profiles; `boyko_symcensus`) → C1 (the `boyko_memory` member) · RP-2 (`boyko_replay`, `boyko_build_id`) · any later rung that adds a crate or a profile. ⚠ *2026-09-24: B1 edited it too (its `[dev-dependencies]`), in a region disjoint from B3's (B1 PC-j)* |
| `Cargo.lock` ⚠ *2026-09-24, B1 PC-h (withdrawn while the lock was untracked) revived by B3 PC-44: tracked since `44d9ce22` (B3); CI builds with `--locked` (`25df4649`)* | Every rung that adds a crate or a dependency, or changes a resolved version, commits the regenerated lock (CI's `--locked` refuses a stale one). UG-15 leg (2)'s receipt records the lock's sha256 over LF text; a pin move whose body differs only in content-named constants stays RED and prints a hint line toward the lock (PC-44; 03 §6, leg (2) Profile). A textual conflict at a merge is resolved by re-resolving (`cargo check`), never by hand (PC-h). ⚠ *2026-09-24, the Phase B document step's revision: the re-resolution starts from the trunk's side of the lock, so that cargo adds only the branch's new entries and keeps every version the trunk resolved unless a new requirement forces one. A re-resolved lock is a new resolution, and by PC-44's own measurement another resolution moved 4 leg-(2) pins on an identical tree. So the merge commit re-runs UG-15 leg (2), whose receipt records the new lock digest, and names every pre-existing package whose resolved version changed. A pin move under such a change takes PC-44's route: RED with its hint, attributed in the merge commit, never masked* A worktree created before `44d9ce22` holds an untracked lock: move it aside before merging a commit that tracks it (CLAUDE.md, Build commands) |
...
| `.github/workflows/ci.yml` | AH → RP-3 ⚠ *2026-09-24: B3 edited it between them: the loom job names all six models (B3 PC-14, PC-43), and every cargo build runs `--locked` (PC-44). `docs.yml` gained `--locked` too* |
...
| Ledger rung | Count (`RUNTIME-DATA-LEDGER.md:1809-1840`; ⚠ *2026-09-24, B1 PC-l: rev 5, `:2374-2404`*) | Plan rungs |
|---|---|---|
| R1 (kernel features physics needs, 0 own rows) | — | D-M5, D-S2..S5, D-E8, D-M6 |
| R2 | ~~333~~ 444 | retired by D-M1, D-M2 (part), D-S6, D-S7, D-E*, D-R2a–d |
| R3 | ~~80~~ 86 | physics U4..S0 |
| R4 | ~~156~~ 235 | engine subsystem rungs + 5 boyko_ecs eliminations (in D-R2a) |
| R5 | ~~38~~ 45 | D-M2, D-M3, D-M4, D-M6 |
| R6 | ~~648~~ 860 | F1 |
| — (replay, KC-37) | 0 | RP-0..RP-3 add code on kernel forms only; UG-02 is unchanged |
| out of scope | ~~1102~~ 1159 | none |

⚠ *2026-09-24, B1 PC-l: rev 5's total is 2829 active rows (the sum of the six counts); rev 4's was 2357 (`RUNTIME-DATA-LEDGER.md:2402-2404`). The per-rung rows each plan rung retires, counted by destination, are in the ledger's table at `:2617-2625` and in the MEM, STORE and ENG rows of §2.*
