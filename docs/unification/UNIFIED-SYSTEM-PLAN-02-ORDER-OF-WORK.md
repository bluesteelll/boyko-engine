...
- **D-E2, `counted_target_despawn_keeps_group_slot`.**
  ...
  - **A `Pinned` target.**
    ...
    - Inside `delete_entity_core` the row is untouched. ~~The `Pinned` removal applies at depth 0 in~~
      ~~the outermost drain, and its `on_remove` test hook records depth 0.~~ ⚠ *2026-09-23 (EP3 W1): the drain applies every command inside its own bracket, so no hook reads depth 0. The observables instead: in one queue, `despawn(E)` and then a closure that logs "returned", with `Pinned`'s test `on_remove` logging "unpinned", must log `["returned", "unpinned"]` (red if the removal runs inside the redirect); and no despawn hook or observer of E ran (engine P4.1-§17).*
    - Afterwards E has no `Pinned`, and its `TG` slot is live and unchanged. ⚠ *2026-09-23 (EP3 C1): the set gains `DESPAWN_AT_ZERO`'s pin edge, with I as E's only user: (1) `[despawn(I), despawn(E)]` in one system's `Commands`, E pinned → E is dead after that drain, and its `TG` slot is in `dying`; (2) the same from one hook's deferred queue; (3) the reverse order → E is dead, and its despawn hooks ran once; (4) `remove::<Pinned>()` at count 0 → dead in that drain; (5) `[unlink to 0, insert Pinned]` in one queue → alive; (6) a never-linked, unpinned E → alive. Mutations: deleting the pin edge turns (1), (2) and (4) red; dropping `DespawnAtZeroCommand`'s `Pinned` re-check turns (5) red (engine P4.1-§9, §17).*
...
3. **D-E2's redirect.** Inside the existing `if !flags.is_empty()` branch (`:1050`), before
   ...
   despawn (critic pass 2, O1). ⚠ **2026-09-23 (EP3 C1):** `Pinned`'s removal is also `DESPAWN_AT_ZERO`'s second edge. Its `on_remove` enqueues `DespawnAtZeroCommand`, which despawns the entity if, at its apply, the entity is live, unpinned and at count 0 (engine P4.1-§9). Without that edge, `[despawn(last user), despawn(pinned asset)]` in one queue leaves the asset alive at count 0 forever.
