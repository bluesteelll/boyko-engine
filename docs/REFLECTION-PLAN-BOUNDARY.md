...
* `Component::stable_name()` defaults to `std::any::type_name::<Self>()` and is
  ...
  `crates/boyko_ecs/src/ecs/core/component/component.rs:195`. It is a **method, not a
...
* `load.rs::resolve_type_table` materializes a **dense `Vec<ResolvedType>` indexed by the
  ...
  (`load.rs:365~-433`). This is the shape rung B4 copies.
* **A `format_version` mismatch is a hard `LoadError::VersionMismatch`** — *"never a
  ...
  same-shape semantic reinterpretation)"* (`load.rs:216~-218`). Precedent for **D7**.
...
* The only public route to an `EnableTagId` from outside is
  ...
  (`enable_tag_api.rs:61`, `:73`) — a **name-keyed dynamic mint** whose name space is
...
* Therefore: **`REFLECTION-ANALYSIS.md` B.4's "public round-trip" is half-true.** The
  round-trip test it cites (`component_registry/mod.rs:1678`) runs *inside* the crate.
...
* `storage_kind(component_id) -> StorageKind` (`mod.rs:390`) and
  `get_layout(component_id) -> Option<&'static ComponentLayout>` (`mod.rs:1125`, with
...
| `format_version` | **hard `LoadError::VersionMismatch`, even when the fingerprint matches** (`load.rs:216~-218`) | **hard refusal**, counted (v1 is `Strict`-only — D19(b); a counts-and-continues mode is the v2 arm) | a `format_version` bump means a **semantic** reinterpretation of the same bytes. Neither a blit nor a field-wise apply can detect it; the loader's own doc says so. A name-keyed apply is not smarter here, only quieter, and quieter is worse |
...
has an internal round-trip test (`component_registry/mod.rs:1678`) asserting a property the
...
| resolution shape | `resolve_type_table` → dense `Vec<ResolvedType>` by file-local index (`load.rs:365~-433`) | the same shape | **adopted deliberately**; the ledger row is the statement that it was adopted, so a later divergence is a decision |
| version guard | `LoadError::VersionMismatch`, hard (`load.rs:216~-218`) | hard by default (D7) | **adopted** |
...
(`crates/boyko_ecs/src/ecs/core/component/component.rs:195`), which embeds the **test binary's own
...
(`crates/boyko_ecs/src/ecs/core/ecs_master/enable_tag_api.rs:367`) reached only from
...
`std::any::type_name::<Self>()` (`crates/boyko_ecs/src/ecs/core/component/component.rs:195`,
...
`("REFLECTION-PLAN-BOUNDARY.md", 5)` (`tests/internal_docs_anchors.rs:2951`) becomes `1` **in
...
remain), and `MAX_COMPONENTS` is `pub` (`component_registry/mod.rs:63`) so the snippet is
...
> superset of the `Refusal` EG1 lands"* (`crates/boyko_reflect/src/ecs.rs:64`). `SinkEvent` is
...
`boyko_serialize::resolve_type_table` shape, `load.rs:365~-433`); `format_version` refusal
...
6. **`has_component` is never the presence probe** — a source census asserting the string
   ...
   `false` for every bitset tag (`ecs_master/component_api.rs:785-814` has no `Bitset`
...
| 5 | `REFLECTION-PLAN-ECS.md` | three-source enumeration **with each id kind-tagged** (B.3) — `dump_entity` needs `(ComponentId, StorageKind, ResidencyKind)`, not a bare slice. **PART-DISCHARGED: sources 1+2 LANDED at EG1** (`IdKind` at `crates/boyko_reflect/src/ecs.rs:94`, `components_of_into` at `:170`); **source 3 (bitset presence) is outstanding at EG3**, which EG2 gates | B4 |
...
| B-4 | `has_component` has no `Bitset` branch and reports `false` for a tag the entity demonstrably has (`component_api.rs:785-814`). Reflection is how it was found; whether the kernel fn grows the third branch is a `boyko_ecs` question | engineering | — |
