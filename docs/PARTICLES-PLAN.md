...
* **DEVIATION from D9's one-line pseudocode, and it is the conservative direction.** D9 writes the skip as `cached_d - speed*timestep/L > radius`. The shipped form is `cached_d - travel*L > radius*L`, i.e. the Lipschitz constant MULTIPLIES the travel instead of dividing it. From `|∇f| ≤ L`, a move of `s` can drop the reported distance by up to `L·s`, so `d − L·s` is the conservative bound; equivalently, in the euclidean units `sdf_field.hlsli` states its own rule in ("a cone-trace consumer divides the reported distance by L"), the test is `cached_d/L − travel > radius`. Both sides are scaled by L so every per-substep op stays a multiply. **The two forms agree exactly at `L == 1`** — hard-CSG scenes, which is every fixture today — and diverge only where a smooth (`k > 0`) edit makes the field super-Lipschitz, where D9's literal form over-estimates the clearance and can skip a substep in which contact happened. Recorded in `docs/OPEN-QUESTIONS.md` (2026-08-20).
* **The response is D9's, term for term**, as an `EvalCf`-oracled leaf: `p += n·(radius − d)`, `v' = (v − v_n)(1 − friction) − v_n·restitution`. Its oracle pins are exact (every case is a dyadic rational, so binary32 carries no tolerance): head-on `−2 → +1` at restitution 0.5, oblique `(4,−2) → (3,1)` at friction 0.25, a non-axis normal to exercise the `dot` as a real three-lane sum, and the two coefficient extremes as properties (restitution 0 ⇒ the normal term is annihilated; friction 1 ⇒ the tangent is). **Noted for the architect, not changed:** the formula is unconditional in `dot(v, n)`, so a particle already moving OUTWARD while still inside the shell has its outward normal component re-flipped inward at scale `restitution`. Reachable only in the marginal re-contact frame (and never at `restitution == 0`); the plan's text is what shipped.
