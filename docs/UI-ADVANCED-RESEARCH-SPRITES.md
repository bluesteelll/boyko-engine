...
- ~~the pack scratch (`UiRenderScratch.pack`, a `Resource`)~~ **the in-schedule staging box (`UiUploadSystem::staging`)** is already the frame-transient buffer, so the 9 sub-quads need **no new storage**; *(buffer corrected 2026-08-21 — [`UI-PLAN-SPRITES-DECISIONS.md` **S-D13 (2)**](UI-PLAN-SPRITES-DECISIONS.md#2-s4-does-not-expand-the-legacy-loop--it-has-no-caller-in-this-workspace): `UiRenderScratch` is production-filled only through `pack_sort_upload`, which has no caller anywhere in the workspace; S4's expansion lands in `gather_into_staging`, whose target is a fixed `Box` sized at `initialize`. The argument survives intact — the box is preallocated and never grown in the frame loop — but a fixed box CLAMPS where a `Vec` grows, so the sizing obligation is real and is gated by G4-6.)*
...
> *Superseded 2026-08-21 as a statement of THIS engine's design (the survey findings above are unaffected).* The shipped shape is ruled in [`UI-PLAN-SPRITES-DECISIONS.md` **S-D11**](UI-PLAN-SPRITES-DECISIONS.md#s-d11--tiling-is-frac-inside-the-sub-rect-it-belongs-to-s5-and-the-nine-sub-quads-are-added-to-the-background-rect-not-substituted-for-it) (3) and [**S-D12 (1)**](UI-PLAN-SPRITES-DECISIONS.md#1-uinineslice-suppresses-the-sub-10-image-record--the-slices-are-the-image-sliced): `UiNineSlice` + `UiImage` ⇒ **10** records — the node's own background rect (kept, because a nine-slice source is a frame and the background is the surface it sits on) plus 9 sub-quads that **replace** the node's image record rather than joining it. `UiNineSlice` **alone** ⇒ 1 record, a structural no-op (S-D12 (3)). The component also carries **`border_uv: [f32;4]`**, the source inset in fractions of the current sub-rect, because the texel-sized border Unity and Godot use needs a texture size this engine never records (S-D12 (2)) — 36 B, not 20.
...
3. **No non-test caller.** ~~`host_upload_frame` / `host_upload_frame_from_world` / `ui_pass` / `ui_setup` are referenced only from `boyko_render`'s own tests.~~ **Re-measured 2026-08-21 ([`UI-PLAN-SPRITES-DECISIONS.md` S-D13 (2)](UI-PLAN-SPRITES-DECISIONS.md#2-s4-does-not-expand-the-legacy-loop--it-has-no-caller-in-this-workspace)) and it has got STRICTER, not looser: `host_upload_frame_from_world` was DELETED at S0, and `host_upload_frame` now has NO caller at all — its only non-doc occurrence in the whole `crates/` tree is its own definition (`upload.rs:509`), so the tests do not reach it either. Its one callee `pack_sort_upload` (`:526`) is therefore unreachable too. `ui_setup` / `ui_pass` are still test-driven.** `boyko_app` does not drive the UI pass. **⚠️ `docs/APP-HOST-PLAN.md` op 5 still prescribes `if has_ui { pack_sort_upload(&token) }` as the host's per-frame UI upload (`:316`, `:136`, `:230`, `:343`) — an UNIMPLEMENTED prescription naming the dead function, which is the counter-argument to deleting it and is filed as such in `docs/OPEN-QUESTIONS.md`.** Sprites therefore have unusual freedom to change `UiInstance`'s layout — and unusual exposure, because only goldens will notice a regression.
...
- `UiNineSlice { border_px: [f32;4], mode: u8 (Stretch|Tile), .. }` — presence ⇒ the pack emits 9 records instead of 1. *(As shipped: `{ border_px, border_uv, mode, fill_center, _pad }`, 36 B; with `UiImage` ⇒ 10 records — background kept, image record replaced; without ⇒ 1. [`UI-PLAN-SPRITES-DECISIONS.md` S-D12](UI-PLAN-SPRITES-DECISIONS.md#s-d12--a-nine-sliced-nodes-slices-are-its-image-sub-10-is-suppressed-the-source-split-is-an-authored-border_uv-and-a-slice-with-no-texture-is-a-structural-skip), 2026-08-21.)*
...
- `UiSpriteCursor { elapsed: f32, frame: u16, dir: i8 }` — the **hot cursor**, the only column the animation system writes each frame; the churn split is the one `components.rs:5` already prescribes, and it keeps `Changed<UiSpriteAnim>` meaningful as "the author retargeted the animation". *(As ruled: `{ elapsed: f32, dir: i8, loops_done: u8, _pad: [u8;2] }`, 8 B — `frame` was written and read by nobody once the pack reads `UiSpriteSheet.index`, and `UiSpriteAnim.repeats` had no reader without a completed-cycle count. **And "the only column written each frame" does not survive:** the system also writes `UiSpriteSheet.index`, and it MUST — a dense `Changed<C>` inside the discovery `Or<..>` was measured never to fire, so a per-frame write landing only on the dense cursor would leave the picture frozen. `UI-PLAN-SPRITES.md` **S-D16**, 2026-08-21.)*

**Sheet table.** One `Resource`-owned dense column keyed by `u16 sheet_id`: `{ slot: u32, cols: u16, rows: u16, frame_count: u16, pad_uv: [f32;2] }` for uniform grids, plus an optional ragged sub-rect column for trimmed/packed sheets. Never a `HashMap<name, sheet>` on the hot path; the name→id mint is a load-time concern with the existing `TypeIntern`/`FontId` dense-handle precedent. *(As ruled: the field is `inset_uv`, and the precedent is copied down to its verb — `UiSheetTable::register(UiSheet) -> SheetId`, the shape of `FontTable::load`. `UI-PLAN-SPRITES.md` **S-D16 (3)**, 2026-08-21.)*
