...
- ~~the pack scratch (`UiRenderScratch.pack`, a `Resource`)~~ **the in-schedule staging box (`UiUploadSystem::staging`)** is already the frame-transient buffer, so the 9 sub-quads need **no new storage**; *(buffer corrected 2026-08-21 — `UI-PLAN-SPRITES.md` **S-D13 (2)**: `UiRenderScratch` is production-filled only through `pack_sort_upload`, which has no caller anywhere in the workspace; S4's expansion lands in `gather_into_staging`, whose target is a fixed `Box` sized at `initialize`. The argument survives intact — the box is preallocated and never grown in the frame loop — but a fixed box CLAMPS where a `Vec` grows, so the sizing obligation is real and is gated by G4-6.)*
...
- `UiNineSlice { border_px: [f32;4], mode: u8 (Stretch|Tile), .. }` — presence ⇒ the pack emits 9 records instead of 1. *(As shipped: `{ border_px, border_uv, mode, fill_center, _pad }`, 36 B; with `UiImage` ⇒ 10 records — background kept, image record replaced; without ⇒ 1. `UI-PLAN-SPRITES.md` S-D12, 2026-08-21.)*
