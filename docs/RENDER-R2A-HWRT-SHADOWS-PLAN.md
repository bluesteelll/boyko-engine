...
| **R2a-4** ✅ SHIPPED (4a @29c2fe7 · 4b @8eb770f) | detail → [RENDER-R2A4-HWSHADOW-PLAN.md](RENDER-R2A4-HWSHADOW-PLAN.md). **R2a-4a** = AS-descriptor RHI support (binding 19 needed MAX 19→20 + a NEW `DescriptorKind::AccelerationStructure` + `VkWriteDescriptorSetAccelerationStructureKHR` pNext write — byte-identical, own GPU smoke). **R2a-4b** = the `#if HWRT` rayQuery variant (`deferred_pbr_hwrt.comp.spv`, software `.spv` frozen byte-identical) + HWRT resolve pipeline/set triple (gated on `scene.tlas.is_some()` — a zero-mesh frame falls back to software, no unbuilt-TLAS trace) + routing (`resolve_ray_backend` Weak/Strong→HardwareTri, tier-conditional dormancy assert) + the build→trace barrier. **SOFT cone-sampled shadows** (16 Vogel-disk rays in the sun's ~2° cone + IGN rotation → penumbra + contact-hardening, min-combined with SDF). | golden `58f6c6c3` byte-identical hwrt-OFF **and** hwrt-ON; GPU smokes clean; **OWNER-APPROVED** on RTX 3060 (soft RT sun shadows). Follow-up: TAA/more-ray denoise of the single-frame sampling noise. | YES — owner-approved |
