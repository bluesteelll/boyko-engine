...
| GPU-side producer #2 — MISSING from round 1 | `gpu_upload.rs:289-298` (the two bounds args at `:296-297`) → `MeshGeometryTable::register` → `MeshLocalBounds::new` at `mesh_geometry_table.rs:680` | the streaming backfill path into the choke point. |
