...
>
> **Status: APPROVED. Ready to implement, P4-1 first.** Every anchor below was re-verified against branch `feat/multi-paradigm-render`, HEAD `c502c9f`. Line numbers drift; the implementer re-verifies at authoring time and treats a mismatch as a finding, not a transcription error.
...
⚠️ **After P4-4, `ForceKeep` is no longer "the disarm route"** — `OcclusionMode::Off` is. `PINS.toml:481` says otherwise; P4-7 updates that comment (comments are not hashed).
...
**Bands.** For every published quantity `Q`, the band comes from running the **identical reduction on the zero control**: substitute `A1` for the armed leg and `A0` for the baseline wherever `Q`'s definition names them. `band(Q) = max( |median_r Q⁰_r| , p90_r |Q⁰_r| )`. A sum's band is that sum's own band; no per-pass band is applied to an aggregate.
...
- the zero control is not exactly `0.00 %` on any quantity;
...
### P4-7 — the doc repairs and the dispositions

**Moves:** `vb_occ_split_gate.rs:52-53`; `PINS.toml:377-378` (the same claim in `[vb_occ_split]`'s block); `PINS.toml:481`'s "named disarm route until piece 4"; `occlusion_marker.rs`'s ladder paragraph gains the config axis; `docs/OPEN-QUESTIONS.md` closes the two 2026-08-07 items and records the dispositions; `docs/FEATURE_MAP.md` + `docs/SYSTEMS.md` gain `OcclusionConfig`/`OcclusionForce`/`occ_fixture`. The mdBook page is `doc-writer`'s.
**Gates:** `cargo doc`, clippy, the full 30-pin sweep once.
**Control:** N/A (documentation). The discipline instead: **every repaired sentence cites an assertion that runs, and names its profile** — 3 of this campaign's 5 doc-rot repairs introduced a new lie, and round-4's condition 2 was a fourth caught in review.
**⚠️ `PINS.toml:410` says "TWO of the 26 pins"; this file now holds 30 top-level tables.** Flagged, not transcribed: count it from the file at repair time. Copying "30" from this plan is the exact failure mode the discipline exists to prevent.

---

## Dispositions for every item in (c)

**(c1) The unconditional FIFO present mode — OUT.** `present/swapchain.rs:199`. **Channel W is superseded, not repaired**: the timestamp channel measures the split directly at CV 0.24–0.68 % where W's floor is 4.7–14.3 % and not constant; removing the present limit would improve a channel that no longer decides anything. Separately, present mode is a **product-surface** decision (vsync, tearing, power) belonging to an owner-facing `PresentConfig`, not to an occlusion piece. Recorded in `docs/OPEN-QUESTIONS.md` as a VALUES item with that framing. No forward reference to any number P4-6 produces.

**(c2) D8 — `vb_indirect_late`'s provenance covered by nothing — OUT, unchanged.** Piece 3 retired the P2-8 first-touch guard's reach over that buffer by adding `vb_batch_cull`'s write and gained `vb_late_count` in exchange (net coverage unchanged, OQ 7; `graph_bridge.rs:3769-3775` states the mechanism). The fix is P2-7's framegraph-core change (`is_write || res_written || res_seeded` + a 14-site audit), whose only gate is a replica the campaign has measured blind to the class it would catch. **Piece 4 declares no new access and therefore neither improves nor worsens it** — stated explicitly so a reader does not infer coverage from the fact that piece 4 touched the recorder. Stays open; owner: framegraph core.

**(c3) PROBE-ON barrier-stream rows — IN, as P4-5**, as a derived **delta** rather than a second field-by-field matrix. G-P3-B/C execute a barrier stream nothing pins, and a defect there makes a *gate* lie (a readback of stale bytes), which is worse than a defect that only shows in a diagnostic. The cheap form buys most of that; the expensive form is explicitly not taken.

**(c4) The intra-pass `TRANSFER → COMPUTE` control on `VbCullUniform` — IN, as P4-3, and only half of it.** The record-order half becomes an executable dev-profile red on every VB golden — previously no red existed anywhere. The declaration half remains uncovered; closing it needs the per-pass accessor C29 says does not exist, and P4-3's doc says so at the site.

**(c5) The stale future-tense header in `vb_occ_split_gate.rs` — IN, as P4-7.** Stale text at `:52-53`: *"`vb_occ_multi` has no golden, so **no golden covers a multi-BATCH late scope.** That is piece 3's first gate."* The same claim sits at `PINS.toml:377-378`. Replacement:

> The `vb_occ_mixed` family (two registered meshes, `draw_batches == 2`) is pinned in four regimes, so a multi-BATCH split now renders under a golden — and all four hashes are one literal, including `[vb_occ_mixed_off]`, which never splits. That is the pins' claim and its limit: they prove a two-batch split reproduces the unsplit pixels; **no hash can show that the late scope drew.** The executed evidence is `late_draws == draw_batches` (`vb_occ_split_gate.rs:632-641`), `draw_batches == BATCH_COUNT` (`vb_occ_mixed.rs:485`), `Σ n_defer == 6` under FORCE-LATE (G-P3-C), and — since piece 4 — `vb_mesh_occ_pins_actually_split`, which adjudicates the pinned configurations inside the pinned binary.
...
| **(c1)** unconditional FIFO | **OUT** | superseded channel + product-surface decision; `docs/OPEN-QUESTIONS.md` VALUES item |
| **(c2)** D8 `vb_indirect_late` provenance | **OUT** | framegraph-core owner; piece 4 declares no new access — neither improves nor worsens it |
| **(c3)** PROBE-ON barrier rows | **IN — P4-5** | derived delta, not a second matrix |
| **(c4)** `VbCullUniform` `TRANSFER → COMPUTE` | **IN — P4-3, half** | record-order half executable; declaration half stays OQ 9 |
| **(c5)** stale `vb_occ_split_gate.rs` header | **IN — P4-7** | replacement text given verbatim; `PINS.toml:377-378` repaired with it |
| **Ladder** | **P4-1 → P4-7**, each landing green and committing alone | P4-1 seam+disarm+epilogue · P4-2 brackets · P4-3 order witness · P4-4 config · P4-5 probe delta · P4-6 measurement · P4-7 docs |
| **Blessing** | **No rung requires re-blessing any pin.** All 30 hashes, all `[*.env]` values and all `[*.sha256*]` literals are unchanged at every rung; `PINS.toml` edits are comments only | §Integration, last row |