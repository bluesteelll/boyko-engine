...
- **Channel G becomes real.** Per leg, per round, one worker with `BOYKO_VB_BENCH=1`; the per-pass **median over that worker's timed frames** (warm-up discarded by `VB_BENCH_WARMUP`), parsed from the `VB-P4 pass=…` lines. ⚠️ **That parenthesis stopped being true when the channel moved.** Profiling rung 7 step 6c deleted the `BOYKO_VB_BENCH` leg this bullet describes and `vg_occ_split_timing.rs` now drives channel G through `BOYKO_VB_ZONE` + the artifact — and the zone leg budgeted for the warm-up without discarding it until 2026-09-18 (`runner.rs`'s `VB_BENCH_WARMUP` doc). Any channel-G figure taken between rung 7b and that date has its 20 warm-up frames inside the median and needs a re-take, not a correction. Any pass flagged `FALLBACK`/`TORN` is excluded from every aggregate and reported by name. A worker whose `VB-P4 regime` line reports `n_distinct > 1` is **rejected**, not averaged (condition 6).
