...
pub struct ActionState<A: Actionlike> {
    ...
    fixed_just_pressed: BitSet256,    // edge, sticky-until-consumed; OR-accumulated each Main, cleared after a fixed batch (C3)
...
2. **Main writes a frozen fixed snapshot once per frame — levels overwrite, edges OR-accumulate.** After computing the per-frame action edges/levels/values, `update_action_state` calls `freeze_fixed_snapshot`, which **overwrites** the frozen *levels* (`fixed_pressed` / `fixed_value` / `fixed_axis2` — a level is sampled, not latched) and **OR-accumulates** the frozen *edges* (`fixed_just_pressed |= just_pressed`, `fixed_just_released |= just_released` — NOT overwritten). Within one frame the snapshot is **frame-stable**: every substep of the next frame's fixed loop reads identical bytes, with no per-substep mutation, so no substep index is needed. Across frames the edges are **sticky-until-consumed** rather than discarded each frame — this is the BUG-I4-C3 no-miss fix (see step 3).

3. **Single-consume is owned by ingest + a post-fixed-loop clear, not by substeps.** A frozen edge bit is **sticky**: once set by some frame's `freeze_fixed_snapshot`, it remains set across subsequent frames until a fixed batch consumes it. "Consumed" is defined as: the frame's fixed loop ran **≥ 1 substep** (`FixedTime::steps_this_frame > 0`). The clear is performed by a dedicated system, `clear_consumed_fixed_edges`, ordered at the **start of Main, `.before` `update_action_state`**. Because the frame order is fixed-loop-first then Main (`app/app.rs:664-676`), this system sees the *current* frame's `steps_this_frame` (the fixed loop already ran), and:
   - if `steps_this_frame > 0` → a batch just read the frozen edges, so it clears them (`fixed_just_pressed`/`fixed_just_released` ← empty) **before** `update_action_state` OR-accumulates the next batch;
   - if `steps_this_frame == 0` → no substep consumed them, so it skips the clear and the edges persist to the next frame.

   For **edge** semantics the convention is: a Fixed system reads `fixed_just_pressed` and the engine guarantees it is true on every substep of exactly the **one fixed batch that first runs after the press**, then false. This is deterministic and miss-free:
   - **0-substep frame:** the edge persists in the snapshot (the post-fixed-loop clear is gated on `steps_this_frame > 0`, which is false) and is consumed by the *first* substep of whatever later frame first runs the fixed loop — the press is never lost (the original "press straddling a 0-substep frame is invisible" bug is fixed: the snapshot is sticky until a batch consumes it, NOT until the next Main overwrites it).
   - **N-substep frame:** all N substeps of the consuming batch see the same `fixed_just_pressed`; the post-loop clear then removes it, so the *next* frame's batch does not re-observe it. A system wanting "fire once per press" reads `fixed_just_pressed` and is expected to act idempotently per batch (the standard fixed-step input contract); a system wanting "fire once per substep while held" reads `fixed_pressed`. Both are well-defined and documented.

   The footgun the plan must prevent — *the same OS press being counted by a variable number of substeps, or lost across a 0-substep frame* — is prevented on both halves: the snapshot is frame-stable (not re-derived per substep), the edge is delivered to exactly the first batch that runs after the press (no-miss), and the post-loop clear (gated on `steps_this_frame > 0`) removes it after that batch (no-double-count), independent of how many substeps that batch ran.

4. **Why this needs no `FixedTime` change:** the only state required is "the sticky frozen edges + a clear gated on whether a batch ran." `FixedTime::steps_this_frame` (a read-only field already written by `fixed_advance` after the loop, `:83`) is exactly that gate, and the two per-frame boundaries that matter — the start of Main (clear, `clear_consumed_fixed_edges`) and the ingest within Main (OR-accumulate, `freeze_fixed_snapshot`) — both run on the once-per-frame Main schedule. The `step` closure never touches edge bookkeeping. The verified clock API (`FixedTime::overstep`, `delta`, `steps_this_frame`) is read-only here and sufficient.

5. **Deterministic 1-frame (or more) latency** (V10): because Fixed (④) precedes Main (⑤), the edge frozen by frame F's Main is first eligible for frame F+1's fixed loop; if F+1 runs 0 substeps the edge carries forward to the first later frame that runs ≥ 1 substep. Standard, imperceptible ≥ 60 Hz, and the price of a race-free, miss-free, double-count-free contract.

**`consume()` interaction (resolves O3):** `ActionState::consume(a)` clears `just_pressed`/`just_released` bits for action `a` in the **Main-facing** sets and in the **frozen fixed sets**, and sets the `consumed` bit. Clearing the *live* edge stops the action being re-accumulated into the frozen set by the next `freeze_fixed_snapshot`; clearing the *frozen* edge bit removes it from the current/future fixed batch. Because the fixed view is otherwise frame-stable, a `consume` call from a Main system is observed identically by every substep of the next frame's fixed loop — no desync. `consume` from a Fixed system mutates only the frozen snapshot for the remaining substeps of the current batch (documented; rarely used).
...
**`clear_consumed_fixed_edges` (per frame, Main, FIRST — `.before` `update_action_state`):**
0. **Clear the accumulated frozen edges** (`fixed_just_pressed`/`fixed_just_released` ← empty) **iff** the current frame's fixed loop ran ≥ 1 substep (`FixedTime::steps_this_frame > 0`). On a 0-substep frame the clear is skipped, so the sticky edge carries forward (C3 no-miss). Runs before the ingest so the freeze below OR-accumulates onto a correctly-pruned edge set.

**`update_action_state` (per frame, Main, early, after the clear):**
1. **Clear** the previous frame's *Main-facing* edge sets (`keys_just_pressed/released`, action `just_*`, `consumed`). The `fixed_*` edges are NOT cleared here (that is `clear_consumed_fixed_edges`'s job, gated on `steps_this_frame`).
...
5. **Freeze the fixed snapshot** — overwrite the `fixed_*` *levels*, OR-accumulate the `fixed_*` *edges* (sticky-until-consumed) — frame-stable for the next frame's fixed loop (C3).
...
**I4 — ECS integration + `InputPlugin` + C1/C3.** Generic-resource `ResourceId` minting via the `TypeId` registry; `update_action_state`; register on `CoreSchedule::Main` `.before_set(GameplaySet)`; the sticky-until-consumed fixed snapshot (C3): edges OR-accumulated by `freeze_fixed_snapshot` each Main, cleared by `clear_consumed_fixed_edges` (`.before update_action_state`, gated on `FixedTime::steps_this_frame > 0`).
...
| **C3** (fixed latch relied on a non-existent mid-loop substep index) | **Fixed.** Mechanism is a **sticky-until-consumed frozen snapshot**: levels overwritten + edges OR-accumulated by `freeze_fixed_snapshot` each Main, cleared by `clear_consumed_fixed_edges` (`.before update_action_state`) only when the fixed loop ran ≥ 1 substep (`FixedTime::steps_this_frame > 0`). A press is delivered to exactly the first fixed batch after it (no-miss across 0-substep frames) and cleared after that batch (no-double-count); **no `FixedTime`/`fixed_advance` change required** (read-only `steps_this_frame`, verified `:51-89`, `:83`). §7.3. (The original BUG-I4-C3 — `freeze_fixed_snapshot` overwriting the frozen edge every Main, destroying a press before any 0-substep-followed batch saw it — is fixed by OR-accumulate + gated clear.) |
...
| **O3** (`consume()` vs fixed view) | **Resolved.** `consume` clears the Main-facing edge (stops re-accumulation into the frozen set) AND the already-frozen fixed edge bit; the otherwise frame-stable frozen view keeps substeps consistent. §7.3. |
