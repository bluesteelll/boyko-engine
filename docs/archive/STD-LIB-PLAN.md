...
> **IMPLEMENTATION NOTE (shipped design SUPERSEDES the column-blit framing below).** The "column-blit" approach this section describes was BUILT and REVERTED: it captured only `SerPod` (plain-old-bytes) components, so it SILENTLY DROPPED `Transform`/`GlobalTransform` (not `SerPod`) — every spatial gate failed — and it duplicated the already-audited `boyko_ecs::clone` machinery (the re-scope below checked `boyko_serialize`, not `boyko_ecs::clone`). The SHIPPED S7 is **clone-based**: `Prefab` is a source-independent frozen image captured via each component's registered `clone_fn` (so it round-trips EVERY `Clone` component, including non-`SerPod` `Transform`), reusing `deep::link_child` (Children rebuild) + `deep::remap_clone_child_of` (ChildOf remap) verbatim. `EcsMaster::capture_prefab(root)` / `EcsMaster::instantiate(&prefab)` (detached root); survives source despawn; instances are "Added" at instantiate. See `crates/boyko_ecs/src/ecs/core/clone/prefab.rs`. The blit-specific details below (POB byte engine, recorded-row-range remap, loader-fuzz) DO NOT describe the shipped code.
