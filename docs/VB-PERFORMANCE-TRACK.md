...
|---|---|---|---|---|
| **VB-P1** Clustered light culling in `vb_resolve` | Replace the all-lights flat scan with a froxel/cluster lookup | Kills G1 — the biggest win; `O(pixels × lights)` → ~`O(pixels × lights_per_cluster)` | **Reuses `cluster_cull.hlsl` + the ForwardPlus `#ifdef FROXEL` light-cull infra verbatim** (already in tree, built for ForwardPlus). Add a `#ifdef FROXEL` seam to `vb_resolve` mirroring `forward_opaque.fs`. App-side must arm the cluster buffers (the known unwired L1-cluster gap — `scene()` hardcodes `cluster_* None`). Landed as a two-stage rung: **P1a** ("dark infra" — the seam + the app-side build + the `light_cull` graph pass, all gated behind one boot-frozen arm bit hardcoded OFF, byte-identical to every golden) then **P1b** (flips the arm from a real `LightingConfig` toggle). The honest size for the FULL machinery (shader seam + `vb_layout0_froxel` + 3 `_froxel` pipelines + the app-side cluster build + the `light_cull` graph declare/record wiring across `graph_bridge.rs`/`passes/vb.rs`/`targets.rs`) is larger than a single-file seam. | **L** |
