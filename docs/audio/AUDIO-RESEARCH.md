...
the repository root. Code is cited as a path plus a symbol; no line anchors are used. ⚠ *At the
trunk merge (onto `da4f4099`) rung C1 had moved `vm.rs` and `vm_column.rs` into `boyko_memory` and
made both types public there; the §11 storage entries that named them are re-derived by content.*
...
- **Kernel storage today.**
  ...
  - `VmColumn<T>`, which backs `LogRing`'s byte arena, is `pub` in `boyko_memory` since rung C1
    and `pub(crate)` through `boyko_ecs`'s re-export
    (`crates/boyko_memory/src/vm_column.rs`, `crates/boyko_ecs/src/ecs/core/log/ring.rs`).
  - Growth is a VM commit on the pushing thread: `ComponentPool` grows in place by committing fresh
    ...
    `VmReservation::commit` (`crates/boyko_memory/src/vm.rs`), since rung C1 a `#[doc(hidden)]`
    `pub unsafe fn` that routes to `commit_at`, the one choke point every kernel commit goes through.
  - Under Miri, `VmReservation` falls back to `alloc_zeroed`; the module doc says Miri can validate
    only that fallback arm (`crates/boyko_memory/src/vm.rs`, module doc).
