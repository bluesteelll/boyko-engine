...
symbol. ⚠ *At the trunk merge (onto `da4f4099`) rung C1 had moved `vm.rs` into `boyko_memory`; the
two paths that named it are re-derived by content (F7.1 Coverage, K-A7).*
...
**Coverage.** `crates/boyko_memory/src/vm.rs` has a Miri arm: under `cfg(miri)` the
...
| **K-A7** | A thread-scoped commit deny: a bit in the thread's record (KC-04) that the cold commit path (`commit_at` in `crates/boyko_memory/src/vm.rs`, since rung C1 the one choke point every kernel commit goes through; `VmReservation::commit` is its `ColumnOwner` route) checks, aborting in gate builds. It is the VM-side sibling of `DenyAfterSteady` | **New.** One load and one branch on a cold syscall path; 0 on hot paths | M0 |
