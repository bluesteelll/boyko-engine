...
W6 = os.path.dirname(HERE)  # the window-7 directory (name kept from window 6's driver)
...
AP.add_argument('--cutoff', default='04:15')
...
ROWS_JSON = json.load(open(os.path.join(W6, 'rows7.json'), encoding='utf-8'))
...
KNOWN = {'J500': '0x32d5e235342b4143'}  # every other hash comes from gate/fixtures/fixtures.json (recorded and checked by gate7.py)
...
def print_schedule():
    ...
    print(f'WINDOW 7 DRY RUN: rows7.json, {len(BLOCKS)} blocks, K = {PROTO["k_per_cell"]} '
...
def main():
    ...
    for k in ('RUSTFLAGS', 'CARGO_ENCODED_RUSTFLAGS'):
        env.pop(k, None)
    manifest = {'generated': D.now(), 'rows_json': os.path.join(W6, 'rows7.json'), 'binaries': BINS,
