"""WINDOW 4 (tree broadphase C3, G5) under the window-3 protocol block. window3_run.py with FOUR edits
(diffable: tools/window4_run.diff): the tree driver path, the canary_of field threaded into D.B (the recipe's
one field), the warm-up row (T-D-tree W=8, this rows.json), the initial idle budget (90 min, the task's rule),
and the rehearsal cell set. Everything else is window 3's, verbatim.
...
TREE_DRIVER = r'D:/wt/mq-de06b6c9/tools/physics_parity/driver.py'
WAIT_PS1 = os.path.join(HERE, 'wait_idle3.ps1')
TEST = os.environ.get('WIN4_TEST') == '1'
...
TEST_CELLS = {('T-A-tree', 1), ('T-A-allpairs', 1), ('T-A-tree', 8), ('T-A-allpairs', 8), ('T-A-tree-armed', 8),
              ('T-A-allpairs-armed', 8), ('T-C-tree', 8), ('T-D-tree', 8), ('T-D-allpairs', 8), ('S16-tree', 1),
              ('S16-allpairs', 1), ('R-tree', 1), ('R-allpairs', 1)}
...
def main():
    ...
    for p in range(PASSES):
        ...
        wrec, receipt, st = run_one(m, 'T-D-tree', 8, p, -1, 0, 'warmup', pdir, {}, receipt, env, sampler, deadline)
        append(runs_path, wrec)
        log(f'  p{p} warm-up T-D-tree W=8 (untimed): {tag_of(wrec) if st == "ok" else wrec.get("aborted")}')
        if st == 'abort':
