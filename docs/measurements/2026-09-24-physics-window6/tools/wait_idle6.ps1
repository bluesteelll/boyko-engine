# Idle wait for window 6 (copy of window 5's wait_idle5.ps1; MaxPolls default 30 = 30 min, the window-6 task's limit; P0 idle rule: 0 cargo/rustc/link/miri/test-exe processes on 3 polls 60 s apart, 10-s CPU < 5 %). Adapted from window 3's wait_idle3.ps1 (+miri, cargo-miri, runner_c3, the bench exe name).
...
$build = @('cargo', 'rustc', 'link', 'lld-link', 'dxc', 'clippy-driver', 'cl', 'msbuild', 'miri', 'cargo-miri', 'jolt_parity_pyramid-49b4ab1b3c0229aa', 'narrowphase_classes-48558213d8414e7f', 'broadphase-feaf22adb9abc4a9')
"# wait_idle6.ps1 start $(Get-Date -Format o); poll every $PollSeconds s, max $MaxPolls polls; quiet = build procs 0 AND lane-target procs 0 AND cpu10 < $CpuLimit % AND 100 <= proc count <= 999; need $Needed consecutive" | Out-File -FilePath $Log -Append -Encoding utf8
$streak = 0
