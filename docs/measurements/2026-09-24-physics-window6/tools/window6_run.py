...
AP.add_argument('--cutoff', default='12:45')
...
WAIT_PS1 = os.path.join(HERE, 'wait_idle6.ps1')
...
ROWS_JSON = json.load(open(os.path.join(W6, 'rows6.json'), encoding='utf-8'))
...
for key, e in ROWS_JSON['binaries'].items():
    BINS[key] = dict(e)
    BINS[key]['path'] = os.path.join(W6, e['exe'].replace('/', os.sep))
SUMS = {}
...
for key, e in BINS.items():
    e['sha256'] = SUMS.get(os.path.basename(e['path']))
EST = {}
...
def est_s(rid, key, w):
    ...
    if v is None:
        row = ROWS[rid]
        v = 10.0 if row['kind'] != 'runner' else 0.01 * row.get('steps', 500)
...
def runner_args(row, key, w, cwd):
    ...
    if not TEST:
        args += ['--expect-pose', os.path.join(FIX, f'{row["pose_ref"]}.pose')]
    return args
...
def col_stats(cols, lo, hi):
    ...
    for name, col in cols.items():
        if not (name == 'wall_ns' or name.endswith('_ns') or name in ('manifolds', 'pairs', 'awake', 'waves')):
            continue
...
def criterion_stats(so, se):
    ...
    t = res['est_ms'].get('bp_g4_scene/tree/j100')
    res['mean_ms'] = t[1] if t else None
    return res
...
def run_one(rid, key, w, block, pass_no, attempt_no, rnd, seq, tag, pdir, receipt, env, sampler):
    ...
    cwd = os.path.join(pdir, f'{seq:03d}_r{rnd}_{rid}_{key}_W{w}{suffix}')
    ...
    rec = {'run_tag': RUN_TAG, 'block': block['name'], 'item': block['item'], 'pass': pass_no, 'pass_attempt': attempt_no, 'round': rnd,
           ...
           'policy': 'P-none', 'timed': tag != 'warmup'}
    if row['kind'] == 'runner':
    ...
    if row['kind'] == 'runner':
        ...
        rec.update({'criterion': cs, 'mean_ms': cs['mean_ms']})
        why = []
        if rec['exit'] != 0:
            why.append(f'exit {rec["exit"]}')
        for need in ('bp_g4_scene/tree/j100', 'bp_g4_scene/tree_rowwalk/j100'):
            if need not in cs['est_ms'] and not TEST:
                why.append(f'no estimate for {need}')
        rec['invalid'] = why
...
def cell_order(bname):
    out = []
    ids = [rid for rid, r in ROWS.items() if r['block'] == bname]
    for w in (1, 2, 4, 8, 16):
        for rid in ids:
...
def run_pass(block, p, attempt, runs_path, env, sampler, counters, state):
    ...
    rounds = 1 if TEST else PROTO['rounds_per_pass']
...
def block_estimate(block):
    ...
    wr, wk, ww = block['warmup']
    per_pass = IDLE_MIN_S + PASS_RECEIPT_S + est_s(wr, wk, ww) + RECEIPT_S + PROTO['rounds_per_pass'] * per_round
    return {'cells': len(cells), 'processes': len(cells) * PROTO['k_per_cell'], 'per_round_s': per_round,
            'per_pass_s': per_pass, 'block_s': PROTO['passes'] * per_pass}
...
def print_schedule():
    ...
    print(f'WINDOW 6 DRY RUN: rows6.json, {len(BLOCKS)} blocks, K = {PROTO["k_per_cell"]} '
          f'({PROTO["passes"]} passes x {PROTO["rounds_per_pass"]} rounds), cutoff {CUTOFF.strftime("%H:%M")}, '
          f'assumed start {t.strftime("%H:%M")}')
    print(f'per pass: idle rule >= {IDLE_MIN_S:.0f} s (3 polls 60 s apart), opening receipt {PASS_RECEIPT_S:.0f} s, '
          f'one warm-up; per process: the gate\'s untimed wall + {LAUNCH_OVERHEAD_S} s launch + {RECEIPT_S:.0f} s receipt')
    bad = verify_binaries()
    print(f'binaries: {"all match bin/SHA256SUMS" if not bad else bad}')
    missing = sorted({r['pose_ref'] for r in ROWS.values() if r['kind'] == 'runner'
                      and not os.path.exists(os.path.join(FIX, f'{r["pose_ref"]}.pose'))})
    print(f'fixtures: {"all present" if not missing else "MISSING " + ", ".join(missing)}')
    done = set()
    if os.path.exists(PASSES_DONE):
        done = {l.strip() for l in open(PASSES_DONE, encoding='utf-8') if l.strip()}
    total = 0.0
    for block in BLOCKS:
        e = block_estimate(block)
        start = t
        t = t + dt.timedelta(seconds=e['block_s'])
        total += e['block_s']
        fits = 'fits' if t <= CUTOFF else ('PARTIAL (cut inside)' if start < CUTOFF else 'LEFT OUT (after the cutoff)')
        print(f'\n[{block["item"]}] {block["name"]}: {e["cells"]} cells x K 6 = {e["processes"]} timed processes '
              f'+ 2 warm-ups; per round {e["per_round_s"]:.0f} s; per pass {e["per_pass_s"] / 60:.1f} min; '
              f'block {e["block_s"] / 60:.1f} min; {start.strftime("%H:%M")} -> {t.strftime("%H:%M")} {fits}')
        order0 = cell_order(block['name'])
        for p in range(PROTO['passes']):
            order = order0 if p % 2 == 0 else order0[::-1]
            key = f'{block["name"]}-p{p}'
            skip = ' (DONE, skipped by --resume)' if key in done and ARGS.resume else ''
            wr, wk, ww = block['warmup']
            print(f'  pass {p}{skip}: idle rule; warm-up {wr}#{wk}@W{ww}; each of 3 rounds: '
                  + ', '.join(f'{rid}#{k}@W{w}({est_s(rid, k, w):.1f}s)' for rid, k, w in order))
    print(f'\nTOTAL {total / 60:.1f} min if every block runs; the cutoff stops starting processes at '
...
def main():
    ...
    for k in ('RUSTFLAGS', 'CARGO_ENCODED_RUSTFLAGS'):
        env.pop(k, None)
    manifest = {'generated': D.now(), 'rows_json': os.path.join(W6, 'rows6.json'), 'binaries': BINS,
    ...
    passes = 1 if TEST else PROTO['passes']
    for block in BLOCKS:
        bname = block['name']
        rows_here = [rid for rid, r in ROWS.items() if r['block'] == bname]
            ...
            while True:
                ...
                try:
                    ...
                    return finish(2, f'cut at {CUTOFF.strftime("%H:%M:%S")} after {state["last"] or "nothing"} (next: {c})', state,
                                  sampler, counters)
                log(f'{ptag}: idle rule (wait_idle6.ps1, up to {IDLE_MAX_POLLS} polls)')
                rc = wait_idle()
                if rc != 0:
                ...
                try:
                    ...
                    for rid in rows_here:
                        progress(f'{block["item"]} {rid} p{p} cut ({rd} of {PROTO["rounds_per_pass"]} rounds complete)')
