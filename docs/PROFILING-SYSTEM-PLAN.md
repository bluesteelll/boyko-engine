...
**All of D9 is behind `feature = "profiling-analysis"` — OPT-IN since rung 14, not "default ON in dev, OFF at retail" as this line read until then.** A cargo feature cannot be switched by an environment variable, and while it was default-on **no command line could clear it** (nine dependent manifests, none with `default-features = false`, so unification restored it). It is enabled with `--features boyko-ecs/profiling-analysis`; the axis emits `ANALYSIS_ADMITTED` and `boyko_ecs` refuses at compile time to be built with the feature under a profile that does not admit it. `compat` + `intervals` = 384 KiB, which a shipping title has no use for.
...
| **13** *(**SHIPPED**)* | The framed binary telemetry stream — but **split**: the wire format is `boyko_diag::telemetry` (hosted, on a measured graph argument — a `prof_decode` rooted there builds 2 crates, at `boyko_app` **45**) and only the writer is `boyko_app::profiling::stream`. **Plus what this row did not anticipate:** the `WindowRec`'s ten fields do not fit its own 40 B and two of them are per-WINDOW, so a 32 B `WindowHead` carries them once and the record is 32 B; `WindowRec::drops` has no per-zone source; `build_hash` and `calib_cv` do not exist and `build_profile` has no name constant; `NO_QUANTILE` cannot be `u32::MAX`, which the store's clamp makes reachable; a `ZoneDesc` carries **no kind**, so `ZoneRow.kind` is an observation and the store grew a 1 B/zone map for it; the file handle cannot be `.bss` because `Option<File>` is not `ZeroInit`; and **A10's zone-major reduce misses its own 150 µs budget** (168.1 µs p95, release, this box) — row-major gives 128.0 µs. The full record is `docs/diagnostics/profiling/05-LADDER-GATES.md`'s rung-13 row | **G15 (a/b/c), G9 (d), G26** — all run, four measured REDs | additive; the decoder is a separate binary, and the only kernel edit is one section |
| **14 (= joint rung J1, merged with logging L17 — S9)** *(**SHIPPED**, axis half)* | The **single `BOYKO_PROFILE` axis**: `crates/boyko_diag/build.rs` — the workspace's first build script — the five-row table, the sizing consts moved onto it, 4 net-new CI legs, the `compile_error!` on a stray per-knob override. **Plus what this row did not anticipate:** a symbol census over a **non-LTO** artifact is INERT on this target (all four cells read 1, and `-Wl,--gc-sections` changes nothing — the whole `boyko_diag` rlib rides into the image), the axis **cannot set** the `profiling-analysis` `#[cfg]` split and could not even have it turned off while it was default-on, `BOYKO_PROFILE=off` is a **logging** off switch only (`ZoneTier` has no `Off` and the `profiling` feature the row names does not exist), and `BOYKO_BUILD_HASH` is not emitted because nothing reads it. **The logging half of J1 — `LogRuntimePreset`, the three header facts, `G16(d)` — is NOT delivered:** it needs L10–L16, which have not landed. The full record is `docs/diagnostics/profiling/05-LADDER-GATES.md`'s rung-14 row | **G14 (a/b/c) + G16 (a/b/c)** — six clauses, five in one runnable harness, each shown red; (a) is a 2×2 rather than the specified 1×2, and (c) is a build refusal rather than a census | a build-configuration rung; the workspace is green in **all five** profiles (measured). One axis cannot be split across two rungs, so this rung is shared |
| **15** *(**SHIPPED**)* | `boyko_ui/profiling_overlay.rs` + `boyko_demo` wiring (`profiling_partition!(User)` **and a real `User` zone**, so the partition line is load-bearing). **Plus what this row did not anticipate:** `G19` went green while unable to fail **three times** — a process-global allocation counter measured a sibling test's allocations (6 in 4 800 row writes) and `--test-threads=1` is not the fix under a parallel sweep; the fixture's literal zone ids made every row render `"zone N not registered"`, so the gate drove ONE early return 4 800 times; and **a thread that has not called `set_lane` produces samples the fold never sees while the zone's own `calls()` accumulator increments**, so the zone looks alive with an empty store. Also: two `Profiler`s in one process split the same lane rings, so the non-vacuity check had to become phase 1 of `G19` rather than a sibling test; and the console-command clause was **already discharged at rung 11** by `G12` clause 1. The full record is `docs/diagnostics/profiling/05-LADDER-GATES.md`'s rung-15 row | **G19**, two-sided, RED shown on the real path (**3 600** acquisitions = 600 frames × the 6 registered rows of 8) | additive; the acceptance path for the whole game-facing half |
| **16 (= joint rung J2 — S10)** *(**BLOCKED — refused on the measurement**)* | **The joint baseline sitting.** Re-take `zone_cost`, `fold_cost`, P1 and P2 in the **both-present** configuration, in one sitting; stamp every baseline file with `config_tag = {profiler, logger}`. **Attempted after rung 15 and refused: that configuration does not exist here.** MEASURED — `boyko_log` has **zero** production emission sites (a workspace grep returns a comment and rung 15's own fixture), **nothing calls `enable`/`boot`**, `boyko-log` is depended on by **two** manifests and neither runs a frame, and `Profiler::arm` **still has no non-test caller**. So `GJ1`'s leg A is unbuildable and a file stamped `both-present` taken with neither present would make every downstream regression gate confidently wrong. Preconditions: logging **L3**, **L6–L8**, and a non-test arm path. Separately, **`config_tag` is already taken** in `boyko_app::profiling::artifact` — it is the *render path*'s hash — so S10's field needs a different name before it lands. Full record in `docs/diagnostics/profiling/05-LADDER-GATES.md`'s rung-16 row and `docs/OPEN-QUESTIONS.md` | the `config_tag` clause on every regression gate | whichever subsystem landed second must not be measured against a baseline taken without it. **The `UNPROVEN` state REMAINS IN FORCE**: the +25 % gate and the revert clauses still record `UNPROVEN` and may not fail a rung |
...
3. **What is the non-blocking source of `fence_seen`, expressed as an RHI verb?** There is none, and none is added. It is **derived** from `RenderEpoch >= slot.submit_epoch + FRAMES_IN_FLIGHT`, the asset-retire rule stated by `frame_driver.rs:255-262` and already an ECS `Resource` at `asset_refcount.rs:56` (F13/D4a).
...
| **X9** | Runtime toggling without restart or a hot-path lock | **FOLDED, THE PROPOSED MECHANISM REFUTED, SHIPPED at rung 11** — `ProfilingScope` + kernel `IsEnabled` stays; the **observer does not** (`enable_tag_api.rs:77-88`: "no hook / observer fire"). Projection is step 0 of the fold, and it is a **query under `Enabled<ProfilingScopeEnabled>`** rather than a loop over a `scope_entity[]` table. See `profiling/06-DISPOSITIONS.md` for the shipped row and `profiling/05-LADDER-GATES.md`'s rung-11 record for the four measured REDs | D20, A8, G12, rung 11 | one cached query + one archetype walk per frame, inside `instrument_measured`; toggle latency one frame. *(The "≤ 5 ns × `scope_count`" figure costed the refuted mechanism.)* |
...
| **X25** | A second sink thread for telemetry | **REFUSED ON THE NUMBER** — 20-60 µs per 2 s = 0.36 % of one frame in 120; the engine's only threads stay the pool's. ⚠️ That number omitted the window reduction (M7); the refusal is restated on the corrected total in `profiling/06-DISPOSITIONS.md`. **MEASURED at rung 13**, this box, release, p95 over 32 runs, 64 quantile zones: reduce **128.0 µs**, write **11.2 µs**, total **139.2 µs** — inside the 350 µs the restated refusal rests on, and the reduce is 92 % of it | D23, open q5 | named escalation trigger: `__telemetry_write` p95 > 200 µs on a real title ⇒ hand off to `boyko_log`'s sink, one thread for both. **Restated against the TOTAL** (`__telemetry_total` p95 > 500 µs), because the write turned out to cost 11 µs and a trigger on it alone would never fire |
