...
### Post-loop restitution (W5): ONCE, velocity-only, bias-free, vs the relative normal approach velocity captured BEFORE substep 0. `v_target = -e·min(vn_initial,0)`; apply `Δλ=(v_target−vn_current)/k_n`, total `λn≥0`. NO position write. Skip zero-normal contacts.
