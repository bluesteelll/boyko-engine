...
| **Added 2026-09-03** `systems::o9_manifold_tests::systems_has_no_fma_or_approx_callsites`, `solver::colored::tests::colored_has_no_fma_or_approx_callsites` | run | run |
...
| The FOUR no-FMA source censuses: `solver::simd::tests::solver_simd_has_no_fma_or_approx_callsites`, `sdf_simd::o9_kernel_tests::sdf_simd_has_no_fma_or_approx_callsites`, `systems::o9_manifold_tests::systems_has_no_fma_or_approx_callsites`, `solver::colored::tests::colored_has_no_fma_or_approx_callsites` | Source census over a file's non-comment lines: no `_mm256_`/`_mm_` × `{fmadd, fmsub, fnmadd, fnmsub, fmaddsub, fmsubadd, rsqrt, rcp}` × `_ps(` call site, no `mul_add(`, and no `algebraic_` stem; needles assembled at runtime; a non-vacuity witness, also assembled — `_mm256_mul_ps(` for three of them, `_mm256_loadu_ps(` for `systems.rs`, whose kernel packs and stores lanes and delegates every arithmetic op to `sdf_edit_list_x8` | **yes** — by design | These are what replaced the `compile_error!` guards on 2026-09-02 (commit `cced895a`). They gate the right proposition (a source property), and they now cover every file in the crate that holds an `_mm256_*` call site. The core-wide census in the decision file is still their strict superset, because none of them scans the SCALAR oracles the SIMD files must match. **Two claims that stood in this cell until 2026-09-03 are gone, and both were true when written:** (a) "over two files only" — there were two censuses on 2026-09-02, and the two added since cover `systems.rs` and `solver/colored.rs`, whose AVX2 kernels were `cfg`-compiled out and therefore uncensused until the `x86-64-v3` baseline landed; (b) "their `mul_add(` needle does not match … any `algebraic_*(` spelling" — commit `98ede3e0` added `algebraic_` as a bare stem to all of them, so every `algebraic_{add,sub,mul,div,rem}` spelling is now caught, UFCS included. The `mul_add_relaxed(` half of (b) is still literally true and still moot: that method exists on neither toolchain (measurements file, "API spelling probes"). The cell also said "they stay unchanged"; they did not. |
...
| `.cargo/config.toml` · the ISA-baseline comment | Named the two per-file censuses as "what keeps the determinism contract load-bearing now", and said the build fails "if **either file**" gains a banned op | ✅ **DONE 2026-09-03**: the comment now names all four censuses with their module paths, states that they cover every file in the crate holding an `_mm256_*` call site, and records why the two later ones did not exist (their kernels were `cfg`-compiled out until this baseline landed). What is still open is what this row always said was open — no census scans the scalar oracles the SIMD files must match — so the core-wide census should be named there as the gate over the whole core once it lands. (Revision 2's claim that one of them never ran in CI was false on this tree and is withdrawn.) **2026-09-21**: the "all four … every file in the crate" claim went stale exactly as this row's neighbour predicted — `broadphase_tree/kernel.rs` (the tree broadphase's 8-wide kernel) is a fifth file with `_mm256_*` call sites and carries the fifth census (`broadphase_tree::kernel::census::kernel_has_no_fma_or_approx_callsites`); the comment now lists five and tells the reader to count the files with `grep -rl _mm256_` before trusting the list. The gap between "a new AVX2 file" and "this list" is still closed by prose only; a census over the file list itself is the mechanical fix and is not written. |
| `tests/isa_baseline_census.rs` · the `fma` entry's reason string | "the physics kernels … are written `mul_add`-free and two source censuses enforce that" (`tests/isa_baseline_census.rs:47`) | ⚠ **Still open, and now wrong by two rather than one — this row predicted exactly that.** Point at `docs/physics/` rather than a count of censuses, so the entry does not go stale the next time a gate is added; it has now gone stale once for that reason. |
