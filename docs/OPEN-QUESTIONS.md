...
* **Two of the five were in nobody's list.** `UI-PLAN-SPRITES.md` said `:663` is *"today a sentence
  ...
  currency claim in the cluster. And part 10 wrote that `:1656` *"is today the F2 ordering-axis <!-- doc-anchor-ignore -->
...
The owner asked to stop at a checkpoint and record everything. ~~Pass 8 was launched and stopped before it wrote anything, so the tree is exactly the pass-7 state.~~ *[done: pass 8 is applied - see "Pass 8 (2026-09-11): the last dating-rule application" below]* What pass 8 is to do, from the pass-7 verifier (full report: `docs/unification/checkpoint-2026-09-11/msvc-citations-pass8-work-order.md` on `feat/multi-paradigm-render`): <!-- doc-anchor-ignore -->
