...
> ⚠ *At the trunk merge (onto `da4f4099`, after rungs C1 and L10) two citations were re-derived
> by content and are marked where they stand: A11's `vm.rs`, which C1 moved into `boyko_memory`,
> and I5's `resources.rs` line, which L10 moved. Every other citation is unchanged.*
>
...
| A11 | Memory: VM reservations, one allocator | Gregory [S1] | **PARTIAL** | kernel `VmReservation` (`crates/boyko_memory/src/vm.rs`, re-exported from `boyko_ecs`). ⚠ *Re-derived at the trunk merge onto `da4f4099`: at `6394bc5e` the file was `boyko_ecs::ecs::memory::vm` and the `boyko_memory` crate was in flight. Phase C (D-M0, then C1) has since landed, and rung C1 moved `vm`, `vm_column` and `utils` into the crate. The one-allocator half is Phase D's lane MEM (D-M1 onward), still Wave 0 work.* The crate is Phase C of the unified plan ([UNIFIED-SYSTEM-PLAN-00-OVERVIEW.md](unification/UNIFIED-SYSTEM-PLAN-00-OVERVIEW.md) §4) | P0 | — | in flight (Wave 0) |
