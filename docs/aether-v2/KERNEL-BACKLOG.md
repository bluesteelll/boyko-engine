...
| KE13 | ✅ **LANDED (2026-09-10)** — `QueryView::get` / `get_mut` now apply `F`'s per-row predicate, so they agree with `iter()` on the same view for every `F`. **The repair is a SHARED function, not a second copy**: `Query::point_filter_passes`'s body moved into `crates/boyko_ecs/src/ecs/core/iters/query/point_filter.rs` (generic over `F` alone, `#[inline]`, first statement `if const { F::IS_ARCHETYPAL } { return true }`) and BOTH types call it — a mirrored predicate is exactly how the two drifted apart. `QueryView` holds no `SystemMeta`, so each call site carries `const { assert!(!F::NEEDS_CHANGE_DETECTION) }`, which turns `EcsMaster::query`'s existing const-refusal into a CHECKED licence for the `SystemMeta::dummy()` argument rather than a comment. **The false doc paragraph was struck in the same commit**, on both `get` and `get_mut`. ⚠ `#[inline(never)]` was considered and REFUSED: the whole 0%-gate is the caller seeing the `IS_ARCHETYPAL` fold, and an out-of-line call whose body is `return true` would put a real `call` on every `Query::get` in the engine. Gate: the oracle file now reads **`running 8 tests`** with ZERO `#[ignore]`s — `-- --ignored` on it now prints `running 0 tests`, which is a vacuous pass and is named as such in its header. The row below is the pre-fix statement, kept for the mechanism. 🆕 **UNOWNED — raised by R0's census, 2026-08-29.** `QueryView::get` / `get_mut` apply the matched-archetype bitset, the dynamic tag terms and the enable terms, but **never call `F::filter_fetch`, and resolve `resolve_dense` only for `D`**. A **dense** `With<C>` / `Without<C>` is therefore silently ignored on the point-lookup path: a dense `With` has `IS_ARCHETYPAL = false` and a `matches_component_set` that admits every archetype, so its ONLY gate is the `filter_fetch` `get` does not run. `get` returns `Some` for a non-member and **disagrees with `iter()` on the same view** | Same shape as KE1 one path over — *a caller that does not make the dense call* — but strictly worse: KE1 made an arm never true, this applies **no gate at all**, in both polarities. ⚠ **`get`'s own doc comment asserts the opposite in writing** ("So `get` only ever sees archetypal and enable terms — there is no silent-ignore path"), on the premise that `With`/`Without` are archetypal, which the Dense plan falsified. A fix owes that paragraph an edit **in the same commit**. ⚠ In-tree precedent that this path was known under-served: `state.rs`'s `dense_get_iter_agree` carries a "PRE-EXISTING BUG … flagged for the reviewer" note that `get` never calls `resolve_dense`, calling the repair "a one-line `resolve_dense` mirror" — **the `D` half was since fixed, the `F` half was not**, and it is not a one-liner (there is no `filter_fetch` call to feed) | n/a (bug fix) | ✅ already written and **observed red**: `crates/boyko_ecs/tests/ke13_query_view_get_ignores_dense_filter.rs` — two `#[ignore = "deferred: KE13 …"]` tests (both polarities) plus a non-ignored control pinning that `get` DOES apply a table filter and that `iter()` answers the dense one correctly. Un-`#[ignore]` them when a rung takes this |
| KE14 | ✅ **LANDED (2026-09-10)** — all five defects closed, red-first per defect. **D1** (four faces, all four observed panicking at the predicted call sites BEFORE the fix: `migration_helpers.rs:596` insert, `:1234` remove, `:1658` tag-attach, `:1937` tag-detach) is fixed by making the declaration/table split EXPLICIT in the type: `Archetype::component_ids` is renamed `all_component_ids` (declaration record — race-dependent, retained for KE10's poolless attach-flag walk, never a membership oracle) and a stored `table_component_ids` is added (the signature-storage subsequence; every member owns a `ComponentPool`, debug-asserted at all three mint funnels by `Archetype::debug_assert_id_lists_agree`). Every retained-copy / pool-resolving loop walks the table list; the rename made the audit COMPILER-ENFORCED — ~30 call sites had to be re-classified, so a silent miss was not expressible. It also retired six per-consumer `is_signature_*` screens (the four defective loops were precisely the ones that forgot one). ⚠ Cost, MEASURED not estimated: `size_of::<Archetype>()` 8704 → **8736** (one `Vec` header; the tripwire constant carries the arithmetic for both `VmReservation` arms). ⚠ *Normalisation at mint was refused with evidence*: `ke10_flags_direct_on_attach.rs` pins the retention verbatim, and dropping poolless ids from the list would have turned it red. **D2** is fixed by `migration_helpers::reseed_dense_presence`, called at the end of all FOUR paths that repoint an entity into a different archetype — it re-seeds `arch_presence` from the authoritative `e2s` membership. The write-site `mark_arch_present` calls stay as the free fast path (idempotent). 0%-gated by `DenseRegistry::is_empty()`. **D3** gave `apply_replace_in_place` the required pass it never had, sharing `construct_required_dense_one` with the migrating insert — the PER-ID arm, not a walk, because both callers already own a `for_each_required_id_excluding` walk and that walk allocates a `Vec` per call. The replace path's dense-fire buffer was widened `MAX_BUNDLE_ARITY` → `MAX_DENSE_FIRE` with the fail-loud overflow site, the same widening KE11 made on the insert path for the same reason. **D4** needed TWO gate edits, not one: the dense attach-flag walk moved OUT of `migrate_entity_insert`'s `FLAGS_ON_ATTACH`-on-the-target gate, AND `EcsMaster::apply_attach_flags_for` now gates per id on `declares_flags(cid)` instead of the archetype flag word — that word is a sound over-approximation for SIGNATURE ids only. A **D4b** was found during the work and fixed with it: `apply_replace_in_place` called no attach-flag pass at all. **D5** moved the CALLEE's text: `ComponentPool::construct_at_uninitialized`'s `idx >= self.len` bullet was written for its one append-only caller and is widened to the real requirement (no LIVE value at `idx`), which `DenseStore`'s LIFO reuse arm satisfies; the two callers that had recorded ABSENCE as a safety condition were corrected — it is a `# Panics` item, and `insert_with_ctor`'s `# Panics` section now records what a release build actually does (double enumeration + a leaked slot, no UB). Oracles: `crates/boyko_ecs/tests/ke14_retained_dense_migration.rs` (13 tests: a phantom-exists control that must be green before AND after, four D1 faces, five D2 faces, D4, D4b, and a proptest asserting the D2 invariant — dense enumeration == dense store membership — after every step of a random migration sequence) plus `ke11_require_poolless_storage_kind.rs::site3_replace_in_place_constructs_the_required_dense_component`. ⚠ The spawn-path half of D4 is NOT closed and was split out as **KE19** (KE17 on the lane; renumbered at the A3 merge) rather than done blind. The row below is the pre-fix statement, kept for the repro and the mechanism. 🆕 **UNOWNED — opened 2026-08-30 by AB-6's landing.** The retained-dense path in `migrate_entity_insert`: five confirmed defects, four of them pre-existing, none of them memory-safety (Miri clean). **D1** is a reachable panic — `invariant: retained component must exist in source` at `migration_helpers.rs:596`, with a sibling face on the remove path at `:1234` — and **D2** makes a correctly constructed required dense component **vanish from every dense query on the next insert**. Full statement, repro and oracle: **§KE14** below the tables | `#[require]` over dense works on the **spawn path only**. KE11 made the declaration accepted and advertised, so it is now a documented route into defects that were previously unreachable | n/a (bug fix) | red-first per defect, written from the three-step repro **before** any fix; the `ke11_*` table-storage controls stay green throughout, since they are what distinguishes a real fix from a broken fixture |
...
| KE16 | 🚧 **LAUNCHED 2026-09-01 on the owner's word — research-and-build, one pass, worktree `D:/wt/threadpool`, branch `feat/threadpool-ke16`; phase 1 = code probe + measurement harness + five-lens survey + synthesis (`docs/threadpool/KE16-DESIGN-SPACE.md`). Was: UNOWNED, deliberately not launched (owner, 2026-08-30).** Pool occupancy: work spawned by a worker is unreachable by siblings (defect A, `worker.rs:370-371`), and the joining thread parks half a wave in a stealer-less private scratch (defect B, `scope.rs:440-531`). The owner adds a third mechanism neither defect covers — an **idle-worker queue**, since the first-level system scheduler distributes unevenly and lanes drain at different times. Full decomposition, the three mechanisms with their separate prices, the cache-locality axis, and the research scope: **§KE16** below the tables | measured: `par_iter` in a system body is **1.01×** where the same driver called from outside is **7.69×**; all four parallel physics sites are on that path | n/a (bug fix + design) | ⚠ **the acceptance criterion is THROUGHPUT, not occupancy** — owner: *"if it needs synchronisation heavier than the gain from maximum core loading, it should not be done."* A red-first occupancy gate proves the mechanism; the decision between variants is wall-clock on a real consumer, and "keep the current behaviour" is a legitimate outcome for B |
| KE17 | 🆕 **OPENED 2026-09-02 by KE16's design, not started.** The scheduler-level lever behind "lanes drain at different times": the apply-window barrier (`schedule.rs:602/:722` — a finished system's `running` bit clears only when the whole window drains, so its successors stay blocked even when a worker is free) — candidate S6 (release per completion, carrying the deferred-command visibility question SCH7), S10 (run the released successor inline on the completing worker; presupposes S6), and the instrument that sizes the barrier's share of lane idling against the pool's; plus a lifetime-independent wake target for EXTERNAL joiners (KE16's count-gated completion covers worker joiners only). Design: `docs/threadpool/KE16-DESIGN-APP.md` §3, `docs/threadpool/KE16-DESIGN-W.md` §3.5 | unmeasured: the barrier's share of the idling the owner observed; the instrument is the first rung | n/a (scheduler design) | ⚠ presupposes KE16 landed; system assignment is already dynamic, so there is no hand-out mechanism to build — only the gate |
| KE18 | 🆕 **UNOWNED — opened 2026-09-10 by KE14's review.** `remap_relink_generic_relations` (`crates/boyko_ecs/src/ecs/core/clone/deep.rs`) resolves the deep-clone FK remap set out of `Archetype::all_component_ids()` — the DECLARATION record — and then through `EcsMaster::get_component_raw_mut`. That callee is NOT `None` for a dense id: `component_api.rs`'s dense branch routes to `dense_registry.store(id)` + `slot_of(entity)` and returns `Some` for a member. So an entity-bearing DENSE relationship source is remapped iff the clone's archetype record happens to name the dense id, which is precisely the mint race KE14 D1 documented `all_component_ids` as never being a membership oracle for. `materialize_dense_memberships` (`materialize.rs`) already hands the clone the copy using the authoritative oracle (`dense_ids()` + `slot_of`), so on the losing side of the race the clone carries a live dense FK still pointing at the SOURCE-side target. ⚠ Reachability CONFIRMED, not assumed: `boyko_macros/src/component.rs` suppresses `install_relationship_clone_remap` for `storage_bitset` ONLY — a `storage = "dense"` relationship source installs its `map_entities_fn` like any other | silent wrong answer on deep clone / prefab instantiate; pre-existing under the list's old name, surfaced by KE14 D1 making the two lists distinguishable | n/a (bug fix) | red-first: a dense relationship source whose signature-twin archetype was minted FIRST (the mint ordering IS the defect, same fixture discipline as KE19), deep-cloned with the target in-subtree, asserting the clone's FK points at the CLONE-side target. The walk must move to `dense_ids()` + `store.contains(clone)`, the oracle `reseed_dense_presence` and `materialize_dense_memberships` already use |
| KE19 | 🆕 **UNOWNED — opened 2026-09-10 by KE14's landing, and deliberately split out of it.** *(Numbered KE17 on the KE13/KE14 lane; renumbered at the A3 merge because KE17 was already the apply-window barrier on the integration line — `benches/ke17_apply_window.rs`, `schedule.rs`, `docs/scheduler/KE17-*.md`.)* A dense / bitset declarer's `flags (…)` on the **SPAWN** path still depends on winning the archetype-mint race: `apply_attach_flags_all` walks the archetype's DECLARATION record, and archetype identity keys on the FILTERED signature, so `[anchor]` minted before `[anchor, dense_declarer]` collapses the two and the surviving list is the one WITHOUT the declarer. `spawn_at_command.rs` states the same race in its own words. The eight `mark_arch_present` sites are the same eight that owe an explicit `apply_attach_flags_for` — `spawn_at_command.rs` ×2, `spawn_batch_command.rs` ×2, `migration_helpers.rs` ×2 (closed by KE14 D4), `insert_command.rs` (closed by D4b) and `materialize.rs` / `component_api.rs` | KE14 D4 closed the INSERT and REPLACE faces; the spawn face is mechanical but touches the BATCH-SPAWN hot path, which is why it was not done blind in the same rung | n/a (bug fix) | red-first: a dense declarer spawned into an archetype whose signature-twin was minted FIRST, asserting the declared bit lands. ⚠ the fixture MUST mint the twin first or the test is vacuous — that ordering IS the defect |
...
## KE16 — pool occupancy: the design space, RECORDED 2026-08-30, LAUNCHED 2026-09-01

**Status: LAUNCHED 2026-09-01** on the owner's word («Начинай работу»), in the system worktree `D:/wt/threadpool` on branch `feat/threadpool-ke16`. Model routing per the owner's instruction of the same day: Fable only where a decision is made (synthesis, architect, critic, verdict); probing, surveying, implementing and measuring on Opus. Phase 1 produces `docs/threadpool/KE16-DESIGN-SPACE.md` and the red-first harness; the decomposition below remains the starting point it was recorded as. Original record, kept verbatim — owner, 2026-08-30: *"we need to consider exactly all possible
