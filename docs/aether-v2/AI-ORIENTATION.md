...
| AIR-06 | Introspection, TWO artifacts never mixed: (a) grammar schema GENERATED from the parser dispatch tables into a committed versioned JSON manifest + regenerate-and-compare test; (b) project schema (what THIS workspace declares) as CLI/build artifact only — Aether as an expansion by-product, hand-written derives via `boyko_reflect`, Gaia free from the baker. Closes the failure the BMW case measured as heaviest (cross-file semantics, not syntax). **PRECONDITION, with its own rung before R3:** restore the single dispatch table — AETHER-LANG-PLAN §6.1's `const CONSTRUCTS: &[(&str, fn(...))]`. Today the construct-keyword set has **THREE carriers** (the dispatch table, `diag::CONSTRUCT_KEYWORDS`, `Stub::for_keyword`) and "generated from the dispatch tables" is therefore not yet a true statement; the census must span all three and **supersede the existing spot-check test, whose name overpromises what it covers**. (b) additionally depends on `boyko_reflect`, which is **not a workspace member** — sequencing was ballot **AB-9**, ✅ **RULED 2026-08-30: the merge is pulled forward as its own rung before R8, and (b) is NOT descoped.** ⚠ **Reflection is opt-in per component** (`boyko_reflect::registry::type_info_of` returns `None` without `#[component(reflect)]`), and on `feat/reflection` **not one of the 50 opt-in sites is in an engine crate** — so merging alone leaves this dump covering nothing. Engine-crate opt-in (a sweep, or a derive that opts in by default — **R8's own design pass, deliberately not decided by AB-9**) is a precondition on this row, not a nicety | byte regenerate test; red-first: **a hand-written `#[derive(Component)]` component FROM AN ENGINE CRATE must appear in the dump** — ⚠ this clause read "a probe-crate component" until 2026-08-30 and was corrected by AB-9's ruling, because that form is satisfiable by declaring the probe in an `aether!` block, which greens the gate over a dump covering **0 of the 138 `#[derive(Component)]` sites in the engine crates** (measured 2026-08-30: **zero** production `aether!` declaration blocks exist — all **58** live under `crates/aether_tests/`. ⚠ A bare `grep -c 'aether! *{'` over `crates/` returns **60**; the two extras are a `///` doc example in `crates/aether/src/lib.rs` and a `//` comment in `crates/aether_lang/src/expand.rs`, neither a block. This row previously read *50*); three-carrier census green before the manifest is trusted | R8 / Gaia |
...
|---|---|---|---|
| ~~**AB-9**~~ ✅ **RULED 2026-08-30 [delegated]** → [`DECISIONS.md`](DECISIONS.md) §Sequencing rulings, entry **AB-9** | `boyko_reflect` sequencing for AIR-06(b) | **Ruled: pull the merge forward** (own rung before R8; measured `git merge-tree` cost — 7 conflicted paths, of which one source file at 2 hunks / 23 lines). **AIR-06(b) is NOT descoped** — the ballot's claim that the reflection-free halves "still satisfy its oracle" was measured false: a *grammar* manifest cannot contain a workspace component, so descoping removes clause 2's subject rather than satisfying it. **Fourth item, named by no option and now a precondition on R8's Lands**: engine components must opt into reflection (sweep vs derive-default is R8's own design pass, not decided here) — `type_info_of` returns `None` without `#[component(reflect)]`, and on `feat/reflection` **not one of the 50 opt-in sites is in an engine crate**, so the dump covers the empty set under all three original options. Full body + rejected alternatives: [`../OPEN-QUESTIONS.md`](../OPEN-QUESTIONS.md) §2026-08-29 | R8 |
| ~~**AB-10**~~ | AIR-10 residue | ✅ **BOTH HALVES RULED BY THE OWNER, 2026-08-30.** **(a) the measurement script IS required** when the audit branch is taken — a DECISIONS audit line alone does not discharge AIR-10. **(b) the widening to the joint Aether+Gaia vocabulary is RATIFIED** ([`docs/gaia/PENDING-SYNTAX-PLAN.md`](../gaia/PENDING-SYNTAX-PLAN.md) §Tier 1, `key`). Ground, in the owner's own framing from the same session: *Aether is the language for logic, Gaia for data, and it is ONE body of work* — if the two are one body, the vocabulary is one vocabulary, and a word meaning one thing in Aether and another in Gaia is a false friend **between the author's own languages**, the worst kind, because both look like "our language" and no outside experience warns of the difference. **Consequences that must be carried, not implied:** the risk list is no longer `set` / `flag`-vs-`tag` / `each` / `attributes` / `relation` alone — it is those **plus every Gaia keyword as that vocabulary is written**; and a collision discovered on the Gaia side may force a **rename in Aether**, which is the price the ⚠ was marking. Accepted deliberately, on the ground that Gaia's vocabulary is still unwritten and a cross-language collision is cheapest to fix now — after both ship, renaming is the expensive case | ~~R3's keyword surface~~ — **settled.** The audit lines for `set`, `attributes` and `relation` were already being written, so R3 was never stalled on this; what the ruling fixes is **which vocabulary they are audited against** |
