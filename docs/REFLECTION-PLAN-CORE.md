...
| # | Fact | Anchor |
|---|---|---|
| F1 | `MAX_COMPONENTS = 512`, `pub`, importable | `component_registry/mod.rs:63` |
| F2 | `LAYOUTS` and `HOOKS` are `[OnceLock<T>; MAX_COMPONENTS]`, *"a single acquire-load + branch — no `Mutex`, no `static mut`, no data race"* | `component_registry/mod.rs:208`, `:226` |
...
`csm_config.rs:393`'s `view_proj: [[f32;4];4]`.
...
* **The bound is asserted with a named diagnostic, and the diagnostic is house style.** The derive
  ...
  `query/filter.rs:2557~`, the second with a `compile_fail` fixture
...
load-bearing at `boyko_ecs/src/ecs/core/iters/query/chunked_data.rs:67~` and `query/filter.rs:2557~`,
...
relationship_target))]` (`boyko_macros/src/lib.rs:169~-171`) names five, and `reflect` is not one.
...
* **At C9, the message.** The spanned `compile_error!` is C9's table row and stays there. **ECS D5's
  ...
  obligation no rung carries:** `REFLECTION-PLAN-ECS.md:352-377` requires a *"release `assert!`
  ...
  (`boyko_ecs/src/ecs/core/component/component_registry/mod.rs:390`). **C8 does not need it:** after the suppression
...
[`REFLECTION-PLAN-GATES.md`](REFLECTION-PLAN-GATES.md):1196-1419 is a fully specified rung — *"G5 —
...
> governs the ship-visible table; the workspace's dev-only instrument, [`Cargo.toml`](../Cargo.toml):70~); the
...
`csm_config.rs:393`'s `view_proj: [[f32;4];4]`**, and it is named here so it is a recorded
...
* the **registration of `reflect` as a derive helper attribute.**
  `#[proc_macro_derive(Component, attributes(component, require, entities, relationship,
  relationship_target))]` (`boyko_macros/src/lib.rs:169~-171`) lists five names and `reflect` is not
...
> called from exactly one site, ~~`boyko_macros/src/component.rs:165~`~~ → `:184~` → **`:186~`** (re-derived again after C9 landed;
...
`hooks.reflect && !hooks.storage_bitset`, joining the six neighbours ~~at `boyko_macros/src/component.rs:127~`,
`:148~`, `:257~`, `:281~`, `:333~`, `:352~`. Today `:168~` gates on `hooks.reflect` alone~~ and
...
> reading the file, not by trusting the previous text.** *"Today `:168~` gates on `hooks.reflect`
> alone"* was true of HEAD `bf7803d6` and is false of the tree this rung leaves: the condition is
> **`:182~`**, `let reflect_enabled = hooks.reflect && !hooks.storage_bitset;`, and it carries the
> term. The six neighbours are now **`:127~`, `:148~`, `:272~`, `:296~`, `:348~`, `:404~`** — only
...
| **a type with no `Default` and no `#[reflect(no_default)]`** — **D20** | `default_in_place`'s `Some` arm is baked from `Default`, and an inspector's "Add Component" needs it. **This row is not a `compile_error!` and cannot be**: a proc macro cannot see trait impls, so the refusal is a trait bound carrying `#[diagnostic::on_unimplemented]`, which is this tree's existing answer for that class (`query/chunked_data.rs:67~`, `query/filter.rs:2557~`, with a blessed fixture at `tests/compile_fail_chunk/changed_filter_rejected.rs:11~`) | the **type name**, via the `const _: fn() = …` witness the derive emits | yes — as a **message-only** row (D36) |
...
(`crates/boyko_ecs/src/ecs/core/component/component_registry/mod.rs:390` is the public getter;
[`REFLECTION-PLAN-ECS.md`](REFLECTION-PLAN-ECS.md):352-361 states the requirement as *"refusal is
...
in, at `tests/internal_docs_anchors.rs:349`, one commit before the comment was read. The by-name
...
1. A `trybuild` corpus, one case per `REFUSALS` row, with blessed `.stderr`. ~~the established
   ...
   **compile** and the harness would red with *"expected compile failure"* on all of them. MEASURED
   ...
   [`REFLECTION-PLAN-GATES.md`](REFLECTION-PLAN-GATES.md):1196-1451 already reserves (D32), and it
...
| 2-D arrays `[[T; N]; M]` | **v2** | **D19** (stated at C5); the named victim is `csm_config.rs:393`'s `view_proj: [[f32;4];4]`, which is refused (D15) rather than silently flattened. A recursive `ArrayInfo` was rejected because it moves the unbounded descend into the *descriptor*, where §3.1's acyclicity argument does not reach |
