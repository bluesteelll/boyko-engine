...
|---|---|---|---|
| **B1** | `LogRing`/`LogCensus` cannot be `Resource`s — `VmColumn` is `!Send + !Sync` | **FOLDED (correctness bug)** | §Data structures, "B1" block — verified against `vm_column.rs:70` and `resource.rs:42`, both re-read. A SEND10-shaped `unsafe impl` with a **named holder set**: `ResMut` only in `log_drain_system`, `Res` readers via the scheduler's exclusivity, and — the load-bearing clause — **the sink thread never touches either type**, because B2 gives it `ECS_HANDOFF` instead. Quotes `vm_column.rs:73-77`'s own invariant list for the columns, and — under S13 — leaves materialization LAZY, clauses 1-2 (one `ResMut` holder, no `Res` reader run concurrently) carrying the argument that a pre-materializing `LogPlugin::build` used to buy. `const _ { assert_send_sync::<LogRing>() … }` is retained but is **not** F7's treatment: the manual impls are unconditional, so they forge exactly the property it asserts and it cannot red on a future `!Send` field — it reds only if the impls are deleted, as would `#[derive(Resource)]` itself. The mechanical half is an **exhaustive-destructuring witness** beside it: a struct pattern that omits a field is `error[E0027]`, so adding a field to either type is a compile error at the five SAFETY clauses it obliges a human to re-read |
