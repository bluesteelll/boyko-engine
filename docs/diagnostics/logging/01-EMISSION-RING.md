...
> **SHIPPED at profiling rung 14 (= J1's axis half), and the paragraph above is accurate as built** — with two notes a reader will want. The build script emits **`cargo::rerun-if-changed=build.rs` as well**, because emitting *any* `rerun-if` directive opts out of cargo's default package rescan and without it an edit to the table would not re-run the script. And the `compile_error!` is generated **beside a full valid table**, so a stray knob fails the build with exactly one diagnostic — the named one — instead of that plus an `E0425` for every constant the workspace reads. The refusal is gated two-sidedly (`G16(c)`): `BOYKO_PROFILE=custom` with the same knob must **build**, or the refusal is indistinguishable from the knob being broken, and a profile name that matches nothing is refused **by name** rather than falling back to `dev` — a typo must not ship as a full-fat development build.
...
**`load`-then-CAS survives, in `boyko_diag`** (M10): the claim path is `if slot.load(Relaxed) == FREE { try CAS }` over the 14 spares. An unconditional `compare_exchange` over the array takes every occupied slot's line exclusive — the exact defect this repo already fixed at `crates/boyko_rhi_vulkan/src/present/passes/gbuffer.rs:344-359` ("load first, store once"; verified this session — `WARNED.load(Relaxed)` at `:349`, `WARNED.store(true, Relaxed)` at `:352`, inside a `#[cold] #[inline(never)]` helper). The `hash(thread_id)` spread is gone with the scan; its cost is bounded and stated in Decision 3.
