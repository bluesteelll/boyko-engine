...
| **L8b** *(**UNBLOCKED** — profiling rungs 7 and 7b are both SHIPPED as of 2026-08-12; see `profiling/05-LADDER-GATES.md`)* | Migrate `boyko_app`; **zero measurement rows** (S1 — profiling rung 7 removed the producers already, verified: the retired channel has zero producers and `G24`'s census leg is now armed over it). Delete `boyko_demo`'s third-party `log = "0.4"` and migrate `main.rs:113`; add the tidy check banning third-party `log`/`tracing` in any workspace manifest. ⚠️ **The dependency is on rung 7, not on 7b.** This row said "after profiling rung 7 **and 7b**", which is stricter than the reason the profiling corpus gives for it: `:218-219` there names only rung 7 (because rung 7 removes the producers, which is what makes these rows cease to exist), while 7b *"blocks nothing but **licenses** rung 8's verdicts"*. Both are shipped now, so nothing turns on it — recorded because the over-strict form is what made this rung read as blocked | ledger, `crates/boyko_demo/` | — |
