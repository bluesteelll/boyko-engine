...
| `sink_sustained_rate` *(**BUILT AND MEASURED**)* | finds the drop knee; reports records·s⁻¹ | zero-record idle sink. **Three valid sittings 2026-08-19: 53.48 / 53.19 / 52.73 ns/rec ≈ 18.7 M rec·s⁻¹.** A fourth was refused by the A-vs-A' twin (3.9 % drift on a 56 ns leg against a 2 % limit) — the instrument doing its job on a box still busy from a sweep |
...
| `sched_cpu_logger_on_off` *(**BUILT AND MEASURED** in profiling rung 16's sitting — `crates/boyko_ecs/benches/gj1_flag_cost.rs` leg (A) minus leg (D): **10.16 ns/site with the profiler armed**. Taken there rather than in its own bench because J2's instruction is to re-take the baselines in the BOTH-PRESENT configuration, and a second sitting is a second drift.)* (gate **P1**, re-specified) | not resolvable above the floor, **at each of the two profiler states** | interleaved zero control, ABBA, **2×2 with {profiler absent, armed}** (S10) |
...
| `log_enabled_0args_sampling` *(**UNBUILDABLE**, and the reason is recorded rather than estimated)* | NOT RESOLVED vs the pre-L12 baseline | pre-L12 baseline, same sitting. **The baseline does not exist in the tree**: it is the same site with no sampling code in it, and reconstructing it needs a build that removes the code — so the comparison is cross-build, and this box's cross-build absolutes drift further than the effect (the same unchanged leg read 10.16 / 10.94 / 11.72 / 12.11 ns across four sittings). Identical in kind to `G10(d)`, which this row IS in bench form, and to `GJ1`'s leg C. What CAN be said in-sitting: at shift 0 `sample::admits` returns before touching the counter, so the perturbation is one compare and one predicted branch — the class `log_gate_cost` measures at the instrument floor |
...
| `sink_sustained_rate_binary` *(**BUILT AND MEASURED**; the `5×` clause FIRED and the owner kept the rung)* | ≥ 3 M rec·s⁻¹ **and ≥ 5× the text sink** | the text sink, same sitting. **11.58 / 11.88 / 11.31 ns/rec ≈ 86 M rec·s⁻¹, ratio 4.62 / 4.47 / 4.66×.** Twenty-nine times the 3 M floor; the ratio misses `5×` and the owner ruled (2026-08-17) that the `5×` was an estimate written before anything was measured. The bench's bound is now a REGRESSION GUARD at `≥ 4.0×`, below the observed minimum with margin. **Note the absolutes moved and the ratio did not**: an earlier session read 41 / 9.5 ns, this one 53 / 11.6, while the ratio stayed inside 4.29–4.76 across both. That is the reason the clause is a ratio measured in ONE sitting rather than two absolutes |
...
| `sched_cpu_flag_on_off` *(**BUILT AND MEASURED** — `gj1_flag_cost`: flags ON 26.95, OFF 3.52, control 0.39 ns/site ⇒ **+23.44 ns/site saved, RESOLVED**, twin 0.391.)* (gate **GJ1**, S13) | (A) flag-on vs (B) flag-off vs **(C) ceiling removed, flags off** — three pairwise verdicts | interleaved zero control, ABBA, one sitting, **leg C is the control that decides whether the instrument measured anything** |
...
> **(c) IS BUILT** — `crates/boyko_log/tests/l12_sampling_independence.rs`: 8 threads × 8 targets, 4096 occurrences per pair at shift 3, and every one of the 64 pairs must deliver **exactly** `N >> k`. Per PAIR and not in total, because a counter shared across lanes still delivers the right total — which is the shape a wrong implementation passes if you only sum. Real threads and not a loop over indices, because the `LANE_COUNT × MAX_TARGETS` array exists to make the increment SINGLE-WRITER, and a single-threaded loop would prove the indexing rather than the property the indexing is for. **The corpus's own RED fires: `idx = target` alone ⇒ 60 of 64 pairs wrong (620 / 412 / 469 / 493 / 564 against 512).** A second control — `admits` refusing at shift 0 — catches the failure where every pair reads zero, equal to each other and equally wrong.
>
> **(d) CANNOT BE TAKEN, and the reason is the same class as GJ1's leg C.** It compares `log_enabled_0args` against a **pre-L12 baseline** — the same site with no sampling code in it. That code is not in the tree and cannot be reconstructed without a build that removes it, so the comparison is cross-build, and this box's cross-build absolutes drift further than the effect: the same unchanged leg read 10.16 / 10.94 / 11.72 / 12.11 ns across four sittings. What CAN be said in-sitting is the bound: at shift 0 `admits` returns before touching the counter, so the perturbation is one compare and one predicted branch — the class `log_gate_cost` measures at the instrument floor on this box. Recorded as UNBUILDABLE with its reason rather than estimated, and the `log-sampling` default therefore stays as it is: the row's own instruction to flip it is conditional on (d) RESOLVING, which it cannot.
