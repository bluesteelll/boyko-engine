...
| **L13b** *(**READER SHIPPED**: `logdec`, `binary::frames`, `docs/LOG-BINARY-FORMAT.md`. **The anchor cadence is still OWED** — the anchor is written at `open` and never re-emitted, so a file running past ~1.4 s at 3 GHz wraps its `tsc_delta`; recorded in the spec rather than left to be discovered from a stamp.)* | **Volume, part 2.** `BinarySink` with the widths pinned in Decision 21 (M2), the **anchor cadence** (1 s or `u32` overflow), `SITE_DICT` + full-table `W0116` + inline site records, `SINK_OUT`, dictionary records, `logdec`, `docs/LOG-BINARY-FORMAT.md` | `src/sink/binary.rs`, `src/bin/logdec.rs` | text-sink output byte-for-byte; **the audited widths** |
