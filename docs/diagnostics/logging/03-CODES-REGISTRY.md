...
| `11xx`–`14xx` | input, scene, physics, math/sdf | `W1301`, `W1302`, `W1303` (physics, L8a) — all three `soft/self_collision.rs`'s; `W1302` is the one that KEPT its `#[cfg(debug_assertions)]`, so it has no release emitter at all |
| `15xx` | schedule sets & ordering | `W1501` |
| `18xx` | app / plugins | `B1801`, `B1802` — **`boyko_ecs`'s, not `boyko_app`'s** (measured at L8b: `ecs/core/app/app.rs:887` and `:899`). The block is about `App`, the TYPE, which lives in the kernel; reading "app" as the host crate is what made this row look like L8b's own work rather than a flip it happens to carry |
| `20xx`–`27xx` | RHI, RHI-Vulkan, render, shaderdsl, UI, fontbake, image, GPU columns | `E2001` (**`boyko_rhi`**, L8c) — the first code in the `20xx` band proper, and the one this ledger did NOT ask for: five migration rungs each enumerated the crates they were converting and `boyko_rhi` appeared in none of them. Its one production print was a `Drop` leak tripwire whose own comment required it to *"survive in RELEASE"*, and `print_census.rs` found it the first time it ran. **The ledger's crate coverage was incomplete and no reading of the ledger could have shown that** — only a walk of the tree could · `E2101` (L7a) · `W2102`, `E2103`, `W2104`, `W2105`, `W2106` (L7b) — all six `boyko_rhi_vulkan`'s; `2106` is **not** in the ledger and was minted at L7b when the seven-site `E2103` split in two · `W2201`, `W2202`, `E2203`, `W2204`, `W2205`, `W2206` (render, L8a) — of which `W2204`, `W2205` and `W2206` are **not** in the ledger: `2204` was minted when `W2201`'s two sites split, `2205` and `2206` because the ledger named their sites without naming codes · `W2601`, `W2602` (image, L8a) |
| `30xx` | host / runner | `E3001` (`boyko_demo`, L8b — pinned by the ledger before the block was written, which is why the HOST's own codes start at `3002`) · `E3002`, `E3003`, `E3004` (terminal exits) · `W3005`, `W3006`, `W3007`, `W3008`, `W3009` (degrades) · `E3010` (dump writes). Ten rows over **fifteen** of `boyko_app`'s 45 sites; the other thirty are `info!` with no code |
...
- `boyko-E0115` fires **once**, naming the prefix and the code that could not be minted, and `LogStats.codes_unindexed` counts every subsequent unindexed emission. Both are printed by the census. *(**LANDED** with the rate wiring: `census::print` now emits `LOG-CENSUS limiter suppressed=N unindexed=M` unconditionally, after the per-target rows. Unconditionally, because a line that appeared only when a counter was non-zero would make "the limiter refused nothing" and "this census does not report the limiter" the same output. Until then `rate::suppressed` and `rate::unindexed` were `pub`, written by `admit`, and read by NOTHING — so the first policy that actually suppressed would have made the log quieter with nothing accounting for it.)*
...
| `boyko_render/src/light_system.rs:397,456` *(L8a, **SHIPPED**)* | **two codes, not one** — `W2201` for the `MAX_LIGHTS` overflow, `W2204` for the non-finite drops. Same class and same level, so L7b's argument does not apply; what splits them is that check 2 makes a code a page with ONE `## How to fix`, and "your scene has too many lights" is not "one of your lights has a NaN". **The ledger's `dropped_count` is unobtainable at the overflow site** — the fold takes `impl Iterator`s it contractually walks once and RETURNS on overflow, so nothing has seen the remainder. `W2204` carries a real count, accumulated in the fold and reported once at the end instead of once per dropped light |
| `boyko_render/src/render_path_config.rs:311-337` *(L8a, **SHIPPED**)* | both latches → `W2205` + `OnceSite`, kept as per-World FIELDS rather than `static`s. The per-frame `swap` was real: on the divergent path the `&&`'s left side stays true forever, so every reader stored `true` over `true` on a shared line, per frame, from an `#[inline]` hot reader |
| `boyko_render/src/texture.rs` ×3 *(L8a, **SHIPPED**)* | the DECODE failure → `warn!(W2206)`; the **absent-file** arm and the load-success line → `info!` with no code (Decision 7). Warning on absence would put a `Warn` in the log of every material that ships four of five optional maps. **Named limit**: that arm also covers an UNREADABLE file, because `resolve_candidate` drops the `io::Error` with `.ok()` — in `docs/OPEN-QUESTIONS.md` with its cost |
| `boyko_serialize/src/load.rs:750` *(L8a, **SHIPPED**)* | → `warn!(W0901)`, `Every` (the subject is a component TYPE), and **un-gated from `#[cfg(debug_assertions)]`**: `report.dense_stores_skipped += 1` already runs on the line above the call in every profile |
| `boyko_physics/src/soft/self_collision.rs` ×3 *(L8a, **SHIPPED**)* | `W1301`/`W1303` un-gated, `W1302` **stays** `#[cfg(debug_assertions)]` — the fork is measured, not stylistic: the first two conditions are already computed in release, the third needs `min(body.c_rest)` per call per body. The three "no `log` crate dependency (architect SCOPE call)" comments go with them: that call refused a third-party facade |
...
| `boyko_app/src/runner.rs` (measurement sites) | ~~**Not this plan's rows** (S1). Migrated to the profiler's artifact at profiling rung 7, before L8b. By L8b there is nothing measurement-shaped left in `runner.rs` for this ledger to disposition~~ **FALSE AT HEAD, measured at L8b.** Rung 7 retired the channel that had **parse contracts** — `VB-P1d`, `VB-P4`, `VB-SV0-S1.5` — and moved those figures to the artifact. It did not touch `VB-CENSUS`/`VB-ZONE`, which never had a contract, so nothing carried them along and nothing noticed: **seven measurement `println!`s survived in `runner.rs`**, plus one in `vb_cull_probe.rs`, and a measured grep across `crates/*/tests` and `scripts/` finds **zero readers** for any of them. They are dispositioned by L8b as `info!(Profiling, …)` with no code, **not deleted** — the artifact's `LabelCensus` carries `measured`/`not_bracketed`/`lost`/`torn` and nothing else, so `stream_pos`, `profiling_cmds`, the reset/stamp/repair tallies, the stamp positions and stages and the per-zone command counts exist nowhere else. Deleting a producer whose numbers have no other home is not a migration |
| **`boyko_app`'s print census, measured** *(L8b)* | The four entries above name **22** sites; the walker's own rule (comments stripped, string literals kept) measures **45 code sites across 10 files, plus 3 prose mentions**. `runner.rs`'s **25** — over half the crate — appear in no entry at all, and `gpu_scene/mod.rs (3)` is **1**. Fifteen of the 45 take the ten `30xx` codes; the other thirty are `info!` with no code (Decision 7) |
...
**Secondary: `clippy.toml`'s `disallowed-macros` — CANARY RUN, ENTRY REJECTED** *(L8c)*. The rung specified it as *"added only after a shown-red canary"* and anticipated one failure mode: `clippy.toml:21-25` records, empirically, that clippy *silently ignores a config path it cannot resolve*, so the key might be inert. **It is not.** Measured on clippy 0.1.97 / rustc 1.97.1 with `disallowed-macros = [{ path = "std::println" }, …]` plus a deliberate `println!` in `boyko_utils/src/lib.rs`: *"warning: use of a disallowed macro `std::println`"*. It was rejected on a **second** measurement this document did not anticipate — the lint has no notion of test code, and `--all-targets` compiles all of it: **697** print sites in `crates/*/tests`, **66** in `benches`, **60** in `src/bin`, **31** in `tools/`, ~**197** in in-`src` `#[cfg(test)]` regions — **~1051 sites** that would each need an `#[allow(clippy::disallowed_macros)]`, against **six** production exceptions. The entire value of the `#[allow(..)]`-plus-rationale discipline is that *one grep enumerates every exception*; a thousand allows in test code destroys that property and buries the six the ban is about. So the fallback this document already names — *"the entry is dropped and the tidy test stands alone"* — is the right outcome by the **wrong route**, and the route is what a future reader needs: not *"does clippy resolve the path"* (it does), but *"can the lint be scoped to production code"* (it cannot — which is precisely the distinction the walker's cross-file `#[cfg(test)]` rule exists to make, and why the census is the primary and not the backup). Independently noted and unchanged: the lint cannot see `stdout().write_all`, `io::Write` on a raw handle, or `libc::write`, so it could never have carried the migration claim by itself.

---

## Compatibility

`Arena` / `ComponentPool` / `UnitId` untouched. `LogRing` and `LogCensus` use `VmReservation`-backed columns whose element sizes divide `COMMIT_GRANULE`, pinned by const asserts (M13, F7), **and whose `Send`/`Sync` is a manual impl with a named holder set** (B1) rather than a derivation `VmColumn` does not permit. `golden.ps1:229`'s `[vk-validation]` grep: preserved, still synchronous, **and its producer is not edited at all** (F12); `write_oracle_line` shares `stderr()`'s handle with it, so neither can splice a line into the other (S7).
