...
keeps `scripts/golden.ps1:253`'s line-start match on `[vk-validation] ` working. *Ordering* between
...
| The `LogRing` / `LogCensus` `VmColumn::grow_to` — reserve + commit | `LogPlugin::build` | **first drain that carries a record**, under `log_drain_system`'s `ResMut` | **This row was missing, and its absence left the ONE boot-time reservation the corpus still had.** The destination is deliberately *not* `enable()`: `boyko_log` has no dependency edge to `boyko_ecs`, so it cannot reach an ECS resource — see `substrate/crate-graph`. It needs no move mechanism at all, because `VmColumn` is lazy **by construction** (`crates/boyko_ecs/src/ecs/memory/vm_column.rs:462-474`: `self.vm` is `None` until the first growth event and the reservation syscall is deferred to it — the anchor moved by +25 at L5, when `as_mut_slice` landed above it). What had to go was an EXPLICIT pre-grow in `LogPlugin::build`, added to buy a soundness argument that `LogRing`'s clauses 1-2 already supply. **With the flag off, `log_drain_system` returns before touching anything**, and that early return is load-bearing rather than an optimisation: the system has THREE duties and only one of them consumes `ECS_HANDOFF`. The `TARGET_STATS` snapshot copies a `.bss` array and the per-frame `frame_epoch` record is written **by the drain itself, not by the emission path** — so without the early return, `LogRing`'s column grows on frame 1 with every `CONTROL` byte still `Off`, and S13's property is false one layer below where it looks true. Cost when off: one `Relaxed` load per frame in `LogSet` (this engine has no `Last` schedule — `logging/ladder-and-gates` L5 decision 1). **Shipped at L5, and NOT verifiable there**: with only the ring-feed duty present, deleting the early return leaves the gate green, because an empty handoff is a no-op either way. The check is written now and labelled; the L16 obligation is to delete it and confirm the flag-off assertion reds once `frame_epoch` gives the system a duty of its own. |
