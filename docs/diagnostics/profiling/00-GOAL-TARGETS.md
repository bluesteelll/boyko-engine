...
| resident, armed (`Z = 1024`) | ≤ 7 MiB (computed **6.67**) | ≤ 7 MiB after first arm (D15) | ⚠️ **BUDGET BREACHED — computed 1.18, target was ≤ 1 MiB.** Q1 deleted `LANE_COUNT`'s profile axis (32 → 80 everywhere), which moved `LANES` 8 → 20 KiB and the sample slab 192 → 480 KiB. See the **three** corrections below | boot-residency gate over **three** domains — the counting allocator, `Profiler::reserved_bytes()` (`VmReservation::os_len()`; `reserved_bytes()` **does not exist in the tree**), and `boyko_diag::section_report` for `.bss` — two-sided, plus "a second `arm` allocates 0" (G23a/G23b) |
...
### Three corrections the "≤ 1 MiB" headline must carry — the third RETRACTS it

The retail headline is quoted often enough that all of its qualifications travel with it, or it becomes a false claim by omission. **The third correction is not a qualification: the headline no longer holds and must not be quoted.**

0. **RETRACTED by Q1.** The 908.2 KiB that satisfied this target was assembled with `LANE_COUNT = 32` in the shipping profile. `substrate/lane-registry`'s **Q1** deleted that profile axis outright — the constant indexes `MAX_WORKERS = 64`, which is unconditional, so any per-profile value was unsound and 32 was below the topology's own floor of 66. At 80 lanes the row is **1 208.2 KiB = 1.18 MiB**, over by 184.2 KiB. **288 of the extra 300 KiB is the sample slab**, which D15 commits for all `LANE_COUNT` lanes at first arm; committing per lane on first use instead brings a ~10-lane shipping title back to roughly 60 KiB of slab and back under budget with the constant unchanged. That is a D15 edit, it belongs to the rung that owns the arm path, and it is **not made here** — so until it is, this target is **breached, not met**. Detail and arithmetic in `profiling/01-EMISSION-STORAGE.md` §Sizing.
