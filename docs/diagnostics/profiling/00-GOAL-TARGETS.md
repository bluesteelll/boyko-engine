...
| zone at a tier above `GLOBAL_TIER` | **0 instructions** | 0 | 0 | **per-site-by-construction symbol census over a single-site fixture bin, across two profiles** + a behavioural liveness clause (G14). The token-level expansion test belongs to the FEATURE axis alone (G1(a)), where `#[cfg]` runs before expansion; it **cannot** decide the tier axis, because a `macro_rules!` expansion is a function of its token stream alone and is therefore uniform across every site in one configuration — *not* a per-binary recorder-symbol census, which cannot attribute a reference to a site (B5) |
...
| resident, armed (`Z = 1024`) | ≤ 7 MiB (computed **6.67**) | ≤ 7 MiB after first arm (D15) | **≤ 1 280 KiB** (computed **1 208.2**, 71.8 KiB headroom). Raised from 1 024 KiB by an owner decision on 2026-08-08 after Q1 deleted `LANE_COUNT`'s profile axis (32 → 80 everywhere), moving `LANES` 8 → 20 KiB and the sample slab 192 → 480 KiB. See correction 0 below | boot-residency gate over **three** domains — the counting allocator, `Profiler::reserved_bytes()` (`VmReservation::os_len()`; `reserved_bytes()` **does not exist in the tree**), and `boyko_diag::section_report` for `.bss` — two-sided, plus "a second `arm` allocates 0" (G23a/G23b) |
...
| **joint resident, profiler + logger both present** | **owned by `seam/joint-cost` — not restated here** | — | **owned by `seam/joint-cost` — not restated here** | `SEAM.md`'s joint cost table is the single owner and this file now carries **no** joint figure at all. Rev 4 of this row did carry one — **≈ 9.35 MiB dev / ≈ 1.99 MiB retail**, built by quoting the logger's halves as **2.68 / 1.10 MiB** — and both halves are contradicted by the logger's own files and by `SEAM.md`, which state **2.90 / 1.15**. The row said in the same breath that it "quotes and does not re-derive"; it re-derived. What this file owns is the profiler's **own** column, and that is stated as arithmetic a reader can check rather than a number a reader must trust: shipping `66 + 480 + 636 + 6.8 + 11.4 + 8 KiB = 1 208.2 KiB = 1.18 MiB`; dev `284 KiB + 3.75 MiB + 2.48 MiB + 52 + 108 + 8 KiB = 6.67 MiB` (both rows computed field by field in `profiling/01-EMISSION-STORAGE.md` §Sizing). The shipping sum read `54 + 192 + … = 908.2 KiB` until Q1's resolution reached these two operands; **dev never moved, because dev was already at 80 lanes** — which is exactly why the staleness was invisible to anyone checking one row. **The row above is profiler-attributable only**; a shipping title that also boots the logger pays the joint figure, which is the larger number and the one the owner is asked about. Owner call open (S10, `seam/open-owner-calls`) |
...
### Three corrections the retail budget must carry — the first RE-CUT it

The retail figure is quoted often enough that all of its qualifications travel with it, or it becomes a false claim by omission.

0. **The target is 1 280 KiB, not "≤ 1 MiB" — raised by an owner decision on 2026-08-08.** The 908.2 KiB that satisfied the old number was assembled with `LANE_COUNT = 32` in the shipping profile. `substrate/lane-registry`'s **Q1** deleted that profile axis outright — the constant indexes `MAX_WORKERS = 64`, which is unconditional, so any per-profile value was unsound and 32 was below the topology's own floor of 66. At 80 lanes the row is **1 208.2 KiB**.

   **The decision was to change the number, not the design**, and the reasoning is worth carrying because it is the reasoning that should be applied to the next such figure: *the alarming quantity measured a reservation.* Printed in the three columns it always needed —

   | Column | Shipping | Meaning |
   |---|---|---|
   | declared / reserved | **1 208.2 KiB** | address space; free in any practical sense on 64-bit |
   | committed at `arm` | **≈ 1 142 KiB** | the reservation, taken only when diagnostics are turned **on** |
   | resident, flag off | **≈ 0** | nothing armed, no lane claimed, no `.bss` page touched |

   — and the shipped default is the third column. Two cheaper-looking levers were considered and **retained rather than pulled**: `REGION_CAPACITY = 64` in shipping (halves the slab to 240 KiB, costs sample depth under a burst) and per-lane lazy commit of the slab (≈ 60 KiB for a ~10-lane title, costs a D15 edit, a branch on the hot path, and a warm-up-dependent `G23a`). Pull one only if a measurement says the **committed** column is too high; neither touches the flag-off one, which is where a player lives.
...
|---|---|---|
| resident, armed (`Z = 1024`) — 6.67 MiB dev / **1.18 MiB** retail | the committed reservation plus the `.bss` statics, **after** a first `arm` | ~0 resident; `.bss` extents are reserved address space, never touched |
...
3. **The backing memory is the kernel's, not `std`'s.** Rev 2 put 6.7 MiB on `Box<[T]>`. The standing owner correction is *"VM-native storage, NOT `std::Vec` — even inside a `Resource`."* So: **`boyko_diag::profiling_abi` allocates nothing.** `LANES` holds `AtomicPtr<Sample>` control blocks in `.bss`; `arm()` — which runs in `boyko_ecs` and has a world — reserves one `VmReservation` (`vm.rs:109`), commits it (`vm.rs:199`), publishes each lane's base pointer and the `VM_BASE` / `VM_LEN` statics `Release`, and then **`mem::forget`s the reservation, so it has no owner at all** (D8, `profiling/01-EMISSION-STORAGE.md`). That is not a stylistic choice. `impl Drop for VmReservation` (`vm.rs:263`) unmaps, and the lane `buf` pointers derived from the base are published to every thread and never nulled — so any surviving owner, the `Profiler`'s or a world's or a multi-world test's, would dangle every one of them when it dropped. Rev 3 kept the reservation in the `Profiler` `Resource` and argued the use-after-free away; B4 struck that down, because an argument cannot fix it and only a location can. What the `Profiler` `Resource` holds is a `base: NonNull<u8>` copied from `VM_BASE` plus **byte offsets**: the store columns are **offsets into** that reservation, reached through accessors over that base — never `&'static mut` slices (B4, D8, D15). The `Box<[T]>` in rev 2's `Profiler` is deleted.
4. **Where the boundary between `.bss` and `VmReservation` falls, stated as a rule rather than a plea (S12).** *Extent known at compile time ⇒ `.bss` static. Extent chosen at run time from config ⇒ `VmReservation`, and the owner must therefore sit at or above `boyko_ecs`.* `.bss` is not what the owner's standing correction targets: it is demand-zero, address-stable and allocation-free — exactly like a reservation — and the *only* property separating the two is whether the extent is a run-time quantity. Applied here: lane control blocks, `REGISTRY`, `DYN_DESCS`/`DYN_NAMES`, the folded `ZoneHandle` statics and the telemetry double buffer are `.bss`; the sample slab and every store column, whose extent comes from `ProfilerConfig` at `arm`, are the reservation. The boundary is forced anyway by the rejected alternative above. Both halves are measured by the same gate in three domains (G23a/G23b) — including the `.bss` domain, which rev 3's two-domain gate **structurally could not see** (M10). The rule itself is owned by `substrate/never-freed-storage`; this is its application.
...
- **`VmReservation`'s real surface** (M10 — rev 3 cited a method that does not exist). `crates/boyko_ecs/src/ecs/memory/vm.rs:85` — `pub(crate) struct VmReservation { base: NonNull<u8>, os_len: usize, … }`, accessors `base() -> NonNull<u8>` (`:184`) and **`os_len() -> usize`** (`:190`); `reserve` at `:109`; `commit` at `:199`; **`impl Drop`** at `:263` (it unmaps). There is **no `reserved_bytes()`**. It is `pub(crate)`, so no gate outside `boyko_ecs` can call it — G23a/G23b reads `Profiler::reserved_bytes()`, a public accessor over `vm.os_len()`, instead of widening the kernel type's visibility. It is documented `!Send`/`!Sync` (`vm.rs:82-84`: *"owners that are shared across threads opt in with their own `unsafe impl` and their own exclusivity argument"*), while `pub trait Resource: 'static + Send + Sync + Sized` (`crates/boyko_ecs/src/ecs/core/resources/resource.rs:42`) — so a `Profiler` owes an explicit `unsafe impl Send + Sync` with an argument. **D8's ownerless reservation does not discharge that obligation, it only changes which field carries it:** the `Profiler` keeps no `VmReservation` (it is `mem::forget`ed and owned by nobody), but the `base: NonNull<u8>` it does keep is `!Send`/`!Sync` for the same reason the reservation is, so the impl and its three clauses are owed either way. It is in the unsafe inventory and on the Miri list — rev 3 had it in neither (B4/D8/D15).
