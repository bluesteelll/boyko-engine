...
4. **Shipping builds** — D21/S9. Three tiers, one `const` ceiling, one **shared** `BOYKO_PROFILE`
   ...
   (computed, `.bss` included — M10), which is **184.2 KiB OVER the ≤ 1 MiB target and retracts it**:
   Q1 deleted `LANE_COUNT`'s profile axis and the 0.89 figure was assembled at 32 lanes. See
   `profiling/00-GOAL-TARGETS.md` correction 0. Jointly with the logger, the figure `seam/joint-cost` owns —
