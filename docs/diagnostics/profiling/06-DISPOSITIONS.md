...
5. **If `MAX_SYSTEMS` rises to 1024, is 128 KiB + 256 KiB acceptable, and what is `MAX_ZONES` sized
   ...
   profile-dependent** so the shipping `.bss` is **66 KiB** rather than 284 (M10; it read 54 KiB
   while `LANE_COUNT` was 32 in shipping — Q1 deleted that axis); per-system minting is
...
4. **Shipping builds** — D21/S9. Three tiers, one `const` ceiling, one **shared** `BOYKO_PROFILE`
   axis; `shipping` = `Always` + `profiling-analysis` off ⇒ **1.18 MiB profiler-attributable**
   (1 208.2 KiB computed, `.bss` included — M10) **against a budget of 1 280 KiB**, raised from
   1 024 by an owner decision on 2026-08-08 after Q1 deleted `LANE_COUNT`'s profile axis; the
   0.89 figure was assembled at 32 lanes. See `profiling/00-GOAL-TARGETS.md` correction 0. Jointly with the logger, the figure `seam/joint-cost` owns —
   **not restated here**, because the restatement this row carried (≈ 1.99 MiB) rested on a logger
   ...
   analysis and the TOML writer do not. **Amended at the split (S13):** the retail figure and the
...
| `resident ≤ 7 MiB`, proved by `debug_assert` | **four configurations**, ≈ 0.9 / 6.7 / 7.1 / 21 MiB, proved by a two-sided allocator + `VmReservation` gate | F26 / X5 | A `debug_assert` is gone in release and an artifact field is self-reported. G23a/G23b observes the allocator and asserts a **second** `arm` allocates 0 |
...
| **F8** | BLOCKER | "Zero cost when off" asserted, not proved | **FOLDED** — G1 becomes two-sided and token-level: feature-off `zone!(UNDECLARED)` must compile, feature-on the same source must not, plus an object-symbol census. The cost is stated, and it is narrower than this row claimed until the PD3 repair: a typo'd zone name is invisible only where the FEATURE is off, because `#[cfg]` deletes the tokens. On the TIER axis it is **not** invisible — the fold deletes codegen while the expansion still names the identifier, so a typo'd `Deep` zone is a hard `E0425` in every profile including retail. **Extended at the split:** G1 and G14 bound the COMPILE ceiling only; the RUNTIME flag's off-cost is `GJ1`'s, and the two are kept apart because only one of them reaches zero (S13) | G1, D1, GJ1 |
...
| **F26** | MINOR | Resident-memory proof is a `debug_assert` | **FOLDED** — G23a/G23b: a two-sided boot-total gate over the counting allocator **and** the reservation, plus "a second `arm` allocates 0". **Extended in rev 4 to a THIRD domain** (`.bss`, via `section_report`), because two domains could not see a static array at all (M10) | G23a/G23b |
...
| **B5** | BLOCKER | **G14's two clauses contradicted each other, so no RED was constructible.** Rev 3 asked one per-binary object-symbol census to report the recorder symbol **absent** (clause 1) and **present** (clause 2) at once. A census answers "is symbol S referenced in this object", per binary, and **cannot attribute a reference to a site** | **FOLDED by DELETION, not by patching** — rev 3's recorder-symbol census is in the "deleted rather than patched" list. G14 is replaced by **three clauses, none contradicting another**: (a) *per-site BY CONSTRUCTION, across two profiles* — a symbol census over a **single-site fixture bin** whose only dependency is `boyko_diag`, so a whole-binary census answers a per-site question because the binary holds exactly one site. **NOT a token test, and this row said it was until the PD3 repair:** a `macro_rules!` expansion is a function of its invocation token stream alone, so *"does the expansion name its argument"* is uniform over every site in one configuration and cannot separate two tiers. `const { false } && …` deletes CODEGEN, not tokens; `if false { UNDECLARED; }` is still `E0425`. The token form is sound on the FEATURE axis only, where `#[cfg]` runs before expansion, and it stays there as G1(a); (b) *behavioural liveness* — in the shipping binary, `__frame`'s span count > 0 over 10 frames, which is the clause rev 3 wanted, obtained from behaviour instead of from a symbol; (c) *a census only where a census can answer* — no reference to `ConcurrencyReport`, `resolve` or the TOML writer, all `#[cfg]`-removed with `profiling-analysis` | D21, G14, "Deleted rather than patched" |
...
| **M10** | MAJOR | **The sizing rows omitted the `.bss` statics entirely, and named a method that does not exist.** Rev 3 carried the *dev* `.bss` figures into *both* configurations, leaving **234 KiB uncounted in the retail row** — which alone breaks the ≤ 1 MiB claim at 873 + 234 = **1107 KiB**. And it cited `VmReservation::reserved_bytes()`; the tree has `os_len()` at `vm.rs:190` and the type is `pub(crate)` (both verified), so no gate outside `boyko_ecs` could have called it either way | **FOLDED — and the fix is not to stop counting them.** The sizing table gains a `.bss` column with the breakdown written out: **shipping** = `LANES` **20** + `REGISTRY` 6 + `DYN_DESCS` 24 + `DYN_NAMES` 16 = **66 KiB**; **dev** = 20 + 56 + 144 + 64 = **284 KiB**. `MAX_USER_BUDGET`, `DYN_NAME_BYTES` and `ENGINE_ZONE_SLOTS` become **per-profile consts**, which held the shipping row at **908 KiB = 0.89 MiB** *with* the statics counted. **`LANE_COUNT` did NOT stay per-profile — Q1 deleted its axis**, so `LANES` is 20 KiB in both columns and the shipping row is **1 208.2 KiB = 1.18 MiB**, over budget; M10's fold is unaffected, the number it produced is not. `Profiler::reserved_bytes()` — a public accessor over `vm.os_len()` — replaces the non-existent kernel method rather than widening the kernel type's visibility. **G23a/G23b gains a THIRD domain** (`section_report` over the four statics), because its two existing domains, the std allocator and the reservation, **could not observe a static array at all** | §Sizing, D21's profile table, G23a/G23b, `Profiler` API |
...
| **X9** | Runtime toggling without restart or a hot-path lock | **FOLDED, THE PROPOSED MECHANISM REFUTED, SHIPPED rung 11** — `ProfilingScope` + kernel `IsEnabled` stays; the **observer does not** (`enable_tag_api.rs:77-88`: "no hook / observer fire"). Projection is step 0 of the fold, and it is a **query under `Enabled<ProfilingScopeEnabled>`**, not a loop over a `scope_entity[]` table — the table would be the mirror D20 forbids | D20, A8, G12, rung 11 | one cached query + one archetype walk per frame, inside `instrument_measured`; toggle latency one frame. *(The corpus's "≤ 5 ns × `scope_count`" costed the mechanism that was refuted: without a bit → entity table there is nothing to iterate `scope_count` times.)* |
| **X10** | Per-subsystem granularity | **FOLDED, SHIPPED rung 11** — 64 scope bits: 0..7 channels, 8..31 engine, 32..63 game. **The split is load-bearing, not cosmetic:** the projection owns 8..63 and the channels stay `arm`'s, because the fold's entry gate is `any_armed()` and the projection is a step of the fold — a projection able to clear the last bit would stop the fold and therefore itself | D20 | 32 game bits; a hierarchy is refused to keep the gate one `bt` |
| **X11** | Shipping builds — what survives | **FOLDED, and SHIPPED at rung 14** — `ZoneTier {Always,Dev,Deep}` + `const GLOBAL_TIER`; `profiling-analysis` split; retail **1 208.2 KiB against a 1 280 KiB budget** (raised from 1 024 on 2026-08-08 — correction 0 in `profiling/00-GOAL-TARGETS.md`); a per-site + behavioural gate. **Rev 4 replaces the `option_env!`/per-crate `build.rs` route with the single shared `BOYKO_PROFILE` axis read by `crates/boyko_diag/build.rs`** (S9), and `crates/boyko_ecs/build.rs` is NOT created. **Three amendments from landing it.** (i) The `profiling-analysis` "split" is not a column of this axis and cannot be: cargo resolves features before any build script runs, so the axis emits `ANALYSIS_ADMITTED` and `boyko_ecs` refuses a disagreement — and the feature became **opt-in**, because while it was default-on no command line could clear it (nine dependent manifests, none saying `default-features = false`). (ii) The per-site gate is decidable only under `lto = "fat"`; MEASURED, a default release build carries the emission symbol in **both** legs and `--gc-sections` changes nothing. (iii) `BOYKO_PROFILE=off` does **not** turn the profiler off — `ZoneTier` has no position below `Always` and the `profiling` feature the `off` row names does not exist in this workspace | D21, G14, rung 14 | profile change rebuilds the workspace from `boyko_diag` up; ~12 KiB dead `.bss`; **no** typo cost on this axis (the tier fold deletes codegen, not tokens, so a `Deep` zone-name typo is `E0425` in every profile — the "invisible at retail" cost belongs to the FEATURE axis alone); CI 1 → 5 legs (4 net new, **shared** with logging) + one census step; and a plain `cargo build` no longer carries the analysis half |
...
| **X13** | Consumption from ECS systems while the frame runs (**C-IV**) | **FOLDED, SIMPLIFIED, PARTLY SHIPPED at rung 11** — `Res<Profiler>` + `ProfiledZone` + a **published** latency table (an artifact field, not a printed line — S1). Because the fold and retire run **outside** the schedule, the extension's `ProfilerSet::{Retire,Read}` pair and its ordering edge are **not needed and are dropped** | D25, rungs 11/15 | CPU data is N−1, by construction. **The GPU row is NOT on the shipped table**: MEASURED, nothing outside `boyko_ecs` pushes a sample and `arm` has no non-test caller, so this store holds no GPU spans to have a lag about — the host's GPU channel folds into the artifact reducer instead |
