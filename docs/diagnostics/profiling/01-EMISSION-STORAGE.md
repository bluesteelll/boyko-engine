...
- **(a) The clear is `fetch_sub(observed)`, not `store(0)`.** The fold loads `overflow`, accumulates it into a `u64` total, then subtracts *exactly what it observed*. A producer increment between the load and the clear survives. `debug_assert!(observed <= REGION_CAPACITY)`. This is `boyko_diag::loss::fold_into`, and **one gate now serves both plans** (G4 = logging's G11 — S8). **Open against this claim, and it is a BLOCKER the substrate raises rather than this plan:** `fetch_sub(observed)` closes the CONSUMER side only. If the owner's increment is a plain `load; add; store` (which is what "no lock prefix" buys), a consumer `fetch_sub` landing between that load and that store is overwritten and the one loss event is counted twice — the exact double-count S8 exists to remove. The interleaving, the two candidate closures and their costs are `substrate/loss-fold`'s Q2, and `boyko_diag` ships `loss.rs` **without** `fold_into` until it is answered. This plan consumes the resolution; it does not pick one.
...
- **`overflow` `Relaxed` both sides** — a counter with no ordering obligation; the `fetch_sub(observed)` form is what makes a concurrent producer increment survive the clear (D24a, and its open producer-side half is `substrate/loss-fold`'s Q2).
