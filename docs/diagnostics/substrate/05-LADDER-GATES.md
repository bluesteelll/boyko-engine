...
| **DG12** | **No boot work** (added at the split): with both runtime flags off, a process that starts, runs N frames and exits has (a) never entered `calibrate()`, (b) never written a `LANE`/`SPARE_OWNER`/`LossCell`/`DIAG_FLAGS` byte, (c) minted no `SessionId`. Observed by a `#[cfg(test)]` counter on each entry point plus DG6's image probe over the same binary. | Call `calibrate()` from a static initialiser, or from D1's `set_lane` path ⇒ leg (a) reds on the first worker spawn. **Second:** pre-touch the lane buffers in `lib.rs` ⇒ leg (b) reds. **CANNOT CLAIM** that the pages are physically uncommitted — DG12 proves *nothing wrote them*, DG6 proves *nothing is in the image*; **whether the loader commits an untouched page is UNPROVEN in this corpus** and is not asserted by either. |
...
- **F3 — `boyko_rhi_vulkan/Cargo.toml:44-49` is the `boyko_sdf_math` PRECEDENT block, not the
  no-third-party rationale.** The rationale is at **`:7-13`** and the `boyko_diag` row goes
  there, citing `:44-49`. *(Second-order correction, this session: the substrate plan gives the
  rationale as `:7-12`; the block runs through `:13` — "it adds no third-party dependency." — and
  an insertion at `:12` lands mid-sentence.)*
...
- **F7 — `section_report` as specified VIOLATES the crate's own mute-leaf rule.** It shells out
  ...
  `llvm-tools` is **not installed** and the whole `.bss` gate family (DG6, profiling G22, logging
