...
> `512`, now at `component_registry/mod.rs:63`; W5's "IMPORTED, never redeclared"
...
> (`component_registry/mod.rs:375-376`), and so is its sibling
> `RESIDENCY_CLASS` (`:503-504`). That is deliberate, and the reason is stated at the declaration:
...
>
> The genuine `[OnceLock<T>; MAX_COMPONENTS]` payload tables are **`LAYOUTS`** (`:208`),
> **`HOOKS`** (`:226`), and — new since this snapshot — **`SERIALIZE`**
...
`set_component_raw` at `:491`, and it is still arena-rooted exactly as described.
...
> |---|---|---|
> | `merged_archetype_id_dyn` | `commands/migration_helpers.rs:1573` | **`pub(crate)`** |
> | `without_ids_archetype_id` | `:1654` | **`pub(crate)`** |
> | `migrate_entity_attach_ids` | `:1724` | **`pub(crate)`** |
> | `migrate_entity_detach_ids` | `:2022` | **`pub(crate)`** |
> | `retag_in_place` | `:2291` | **`pub(crate)`** |
>
...
- **`name`/`TypeId` → `ComponentId` resolution lives in `boyko_ecs`** (alongside
  ...
  **VINDICATED and ALREADY BUILT (2320-8-21)** — the placement call was right, and
...
- ⚠️ **A SECOND, differently-shaped TB rule landed after this snapshot and is not in
  ...
  (2320-6-21), *"re-derive the structural-write archetype pointer under the live
...
- **`repr(Rust)` is fine — no `repr(C)` requirement** (`offset_of!` reads the
  ...
  **Re-confirmed and strongly supported (2320-8-21):** `core::mem::offset_of!` is
...
plumbing (`boyko_app/src/host_dump.rs:250~`, `boyko_app/src/profiling/artifact.rs`,
...
> **The hard error is right; its BLAST RADIUS was never weighed (2144-8-21).** With
...
> `component_registry/mod.rs:325-358` · `archetype/archetype.rs:1800` ·
...
**That premise is false today.** `is_signature_storage` (`component_registry/mod.rs:343~-358`) returns true
...
//               `ecs.is_enabled_id(e, EnableTagId)`            (enable_tag_api.rs:127)
```
...
`dense_contains` (`:50`) and `dense_slot_of` (`:59`) are already **public** on
...
(`enable_tag_api.rs:114,:120,:127`), and `EnableTagId` is `#[repr(transparent)]` over
...
`component_registry/mod.rs:1678`).
...
> `EcsMaster::register_enable_tag` (`enable_tag_api.rs:61`) → `try_register_enable_tag_by_name` (`tags.rs:134`)
...
> internally and it completes the bridge `component_registry/mod.rs:1678` already tests. This is a **shipping-API
...
> (`ecs_master/component_api.rs:785-814`.) It branches on `StorageKind::Dense` → `dense_contains`,
...
> `component_registry/mod.rs:477-485` · `archetype/archetype.rs:868-903`
...
mint `assert!`s on it at `:884~-893`), and **an archetype signature mixing `Gpu` with
...
> `boyko_render/src/gpu_transform3d.rs:55-62,:84~-89` · `boyko_render/src/csm_config.rs:393`
> · `boyko_render/src/ddgi_config.rs:77` · `boyko_render/src/frustum.rs:33` ·
...
The wider render surface is the same shape: `csm_config.rs:393`
`view_proj: [[f32;4];4]`, `ddgi_config.rs:77` `origin: [f32;3]`, `frustum.rs:33`
