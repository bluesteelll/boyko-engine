...
| `N-18` (no `Or<>` filter) | **RESERVED, not built** (`D4`): the form waited on a real in-tree consumer **and** the kernel `Or`-dense fix. ⚠ rung **R0 landed 2026-08-29** and removed the second half of that ground, so the reserve now stands on the consumer clause alone. **GB-9 was RULED 2026-08-30** and did NOT disturb this reserve: it deleted the *Gaia* generated-code ban and refused to keep it by borrowing D4 (D4 reserves the Aether **surface**; that ban governed **generated code**, which Gaia's ratified GN2 says the baker does not emit as Rust) — see the notes in `DECISIONS.md` `D4` and `CONSTRUCTS.md` §`system` | DECISIONS `D4` |
