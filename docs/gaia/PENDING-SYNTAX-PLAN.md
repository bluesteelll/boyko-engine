...
| template use | **No syntax exists for USING a template**, and `Torch: intensity=2.0` is lexically a component attach. Add a keyword anchor: `apply Torch: …`. Blocks G4. **Which layer the expansion occupies is now RULED (GB-2's sibling ballot GB-1, RESOLVED 2026-08-30): NONE — expansion is one step on the inheritance-DEPTH axis, ranked exactly where `extends` ranks, and resolution is depth first then ladder.** So `apply Torch` + a local `power=2.5` in the same node is an override, not the two-writes-one-layer error. The ruling is spelling-independent and does not choose this row's anchor ([`DECISIONS.md`](DECISIONS.md) §Layers and depth) |
...
| `PointLight` field names | ⚠️ **ENGINE-REFUTED, and the defect is in this file too.** The sketch and this plan both wrote `intensity=`; the struct field is **`power`** (luminous flux Φ in lumens; `boyko_render::PointLight`). `range` has no default and a ctor requires it. `position` is **engine-derived** — `light_reconcile` computes it from `GlobalTransform`, so it must be undeclarable in the scene profile (ballot GB-5). And `color` is `[f32; 3]` documented **LINEAR rgb** (`boyko_render/src/light.rs:309-310`), against a four-component `#RRGGBBAA` literal: an arity mismatch *and* a transfer-function question. **Both halves RESOLVED 2026-08-30 (ballot GB-2, standing rule):** arity is checked against the target's arity — widening 3 → 4 supplies alpha `1.0`, narrowing 4 → 3 is a coded refusal, so the correct literal is the 6-digit **`#FFB35C`**; and the transfer function belongs to the DESTINATION field — sRGB EOTF into a linear float colour, identity into a `u32` STRAIGHT-RGBA8 UI field, coded refusal into a device-encoded packed carrier. Raw bytes into the linear field are wrong by up to 12.92× ([`DECISIONS.md`](DECISIONS.md) §Language shape). Fix every occurrence here before the M6 rewrite re-emits the defect in the new syntax |
...
Header divergence (`aether! {` vs `gaia 1 profile=scene`) — pick one version-token rule · int literals valid in Gaia, type errors in Aether's sugar positions — normalize int→float in Aether's emitted tokens; write Gaia's float-print rule citing the two opposed in-tree conventions · seed the unknown-operation refusal with the Aether construct keywords and a mirror set ("`resource` is Aether's; declare it there and reference it here") — a dozen table rows converting the wrong-language push into one redirect · colour transfer function — **no longer unstated: ballot GB-2 RESOLVED 2026-08-30**, keyed on the destination field (sRGB EOTF into a linear float colour, identity into a `u32` STRAIGHT-RGBA8 field, coded refusal into a device-encoded packed carrier), with the arity rule mirroring Aether's shipped `ColorLit`; the engine already had a measured member of this class in the normal-map convention, and now a second in `ParticleEffect::color_keys`' `0xAABBGGRR` · `/-` vs `//`: converting a slashdash on a multi-line node to `//` leaves the body live one level up — formatter warning on a `//` line ending in an unbalanced `{` · event participant contexts are statically checkable at bake — the router `debug_assert` gets a free static twin
...
| F9 | flags carrier vs coded refusal (M4). ⚠ **PARTLY RULED 2026-08-30 [delegated]** — three eliminations ruled (`FLAGS_DIRECT` never on the load path; the refusal option contradicts ratified AB-6; the spelling, if a carrier lands, is `flags (X = true)`). **The residual is still the owner's:** may a document set a flag on an individual authored object? | **G2** (the residual). ⚠ **Kept by the merge `merge/ke16-into-render`, 2026-09-10, from `feat/threadpool-ke16`:** that side's Blocks column read **G2 / G6**; the G6 half is not struck by the partial ruling |
...
| ~~**GB-2**~~ | ~~colour transfer function: `#RRGGBBAA` sRGB-decoded at bake, or raw into a LINEAR field (K12)~~ **RULED BY THE OWNER 2026-08-30: `#RRGGBB` is `#RRGGBBAA` with maximal `AA`, a DEFAULTED FIELD rather than a second literal kind; and an 8-digit literal at a 3-component field is a coded bake refusal naming the field, never a silent alpha drop** | ~~G2 / G5~~ unblocked. ⚠ **Kept by the merge `merge/ke16-into-render`, 2026-09-10, from `feat/threadpool-ke16`:** the same ruling's second half — **the transfer function is a property of the DESTINATION FIELD, not of the literal**, and the arity is settled with it |
| ~~**GB-3**~~ | ~~the reference taxonomy's **third kind**~~ **THIRD KIND ADOPTED BY THE OWNER 2026-08-30; the four-part rewrite RULED [delegated]** — sigil (uniform, position-independent) / styles are the FIRST kind / `$hole` STAYS, lexical in every position / "declared node ⇒ `@`" WITHDRAWN. ⚠ **The glyph is NOT minted**: it is closed at G2 jointly with the arithmetic operator vocabulary. **R4/R5/R7 and Tiers 1–2 here are REGENERATED by this ruling** | ~~G2 / G3~~ unblocked; Tiers 1–2 here owe the rewrite. ⚠ **Kept by the merge `merge/ke16-into-render`, 2026-09-10, from `feat/threadpool-ke16`:** the question as that side framed it — asset-ref spelling, style-ref kind, `$hole` disposition and the withdrawn “declared-node ⇒ `@`” (K3 + K15 + K16), to be settled jointly so R4 is rewritten **once**; it amends ratified §Identity as an extension consistent with its own rationale, not a reversal |
| ~~**GB-4**~~ | ~~`instance` linkage: (a) linkage-in-slot vs (b) linkage-in-head~~ **RULED 2026-08-30 [delegated]: (a), and the linkage word is MANDATORY** — `instance <name> extends\|copy <base-ref>`, no default; `from` deleted (head-position occurrences measured: zero). ⚠ Rider owed to G2: an undeclared `from=` → `source=` rename on `bind` | ~~G4 / Tier 2~~ unblocked. ⚠ **Kept by the merge `merge/ke16-into-render`, 2026-09-10, from `feat/threadpool-ke16`:** the question as that side framed it — (a) linkage-in-slot `instance wall_east extends\|copy "…"` vs (b) linkage-in-head (`instance` ≡ live link, `copy` its own head), with `from` deleted either way (K14) |
...
> **Merge note, 2026-09-10 (`merge/ke16-into-render`).** Both branches edited this table.
> `feat/multi-paradigm-render` carried the struck, ruled rows (its 2026-09-03 correction of the
> section head); `feat/threadpool-ke16` carried three rows still in their pre-ruling state, whose
> distinct wording is folded into the rows above. Three of its rows added nothing the ruled rows
> do not already say and are recorded here verbatim rather than repeated as a contradicting row:
> `| F10 | bundle expand-vs-refuse (M14) — first item: *is this a ballot at all?* Tier 3 and this
> table disagreed | owner scoping |`; GB-1 in the same wording as above but ending “Ladder
> untouched”; and `| **GB-5** | are engine-derived fields (`PointLight::position`) declarable in
> the scene profile? | G1 table freeze |`, whose question text survives struck inside the GB-5
> row above.
...
| K2 | template application double-writes a field on one unnamed ladder rung | → **GB-1**, **RESOLVED 2026-08-30 — dissolved, not diagnosed**: the two writes are at different inheritance DEPTHS, so it is an override; the two-writes error narrows to same depth AND same layer ([`DECISIONS.md`](DECISIONS.md) §Layers and depth) |
...
| K12 | colour transfer function unstated | → **GB-2**, **RESOLVED 2026-08-30**: keyed on the destination field, three carrier classes measured; the arity half (which K12 did not name and nothing carried) settled on the same ruling |
| K13 | `each par` — two kernel drivers with opposite tick behaviour, one word | Recorded determinately (drivers, `BatchingStrategy::default()`, the 1024-row inline floor, the pinned `par_for_each_chunk_entities` signature); the **choice** → ~~AB-8~~ ✅ **RULED 2026-08-30** → [`../aether-v2/DECISIONS.md`](../aether-v2/DECISIONS.md) **C5a**: `par_iter_mut`, with `each soa par` as the only route to the chunked driver and `BatchingStrategy::default()` always passed (the batching key is **not** author-visible) |
